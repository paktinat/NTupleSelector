// -*- C++ -*-
//
// Package:    NTupleSelector
// Class:      NTupleSelector
// 
/**\class NTupleSelector NTupleSelector.cc DiLeptonAnalysis/NTupleSelector/src/NTupleSelector.cc

 Description: [one line class summary]

 Implementation:
     [Notes on implementation]
*/
//
// Original Author:  ,,,
//         Created:  Tue Dec 17 15:05:04 IRST 2013
// $Id$
//
//


#include "../interface/NTupleSelector.h"
//
// constants, enums and typedefs
//

//
// static data member definitions
//

//
// constructors and destructor
//
using namespace std;
NTupleSelector::NTupleSelector(const edm::ParameterSet& iConfig)
{

  min_njets = iConfig.getUntrackedParameter<int>("min_njets" , -100);
  min_nelectrons = iConfig.getUntrackedParameter<int>("min_nelectrons" , -100);
  min_nmuons = iConfig.getUntrackedParameter<int>("min_nmuons" , -100);
  min_ntaus = iConfig.getUntrackedParameter<int>("min_ntaus" , -100);
  min_nleptons = iConfig.getUntrackedParameter<int>("min_nleptons" , -100);
  min_HT = iConfig.getUntrackedParameter<double>("min_HT" , 0.0);

  HLTPaths = iConfig.getParameter< vector<string> >("HLTPaths");
   //now do what ever initialization is needed

    tExtXSecLO                 = edm::InputTag("analyze","ExtXSecLO");
    tExtXSecNLO                = edm::InputTag("analyze","ExtXSecNLO");
    tHLTLabels                 = edm::InputTag("analyze","HLTLabels");
    tHLTNames                  = edm::InputTag("analyze","HLTNames");
    tIntXSec                   = edm::InputTag("analyze","IntXSec");
    tL1PhysMenu                = edm::InputTag("analyze","L1PhysMenu");
    tMaxElEta                  = edm::InputTag("analyze","MaxElEta");
    tMaxGenJetEta              = edm::InputTag("analyze","MaxGenJetEta");
    tMaxGenLeptEta             = edm::InputTag("analyze","MaxGenLeptEta");
    tMaxGenPhotEta             = edm::InputTag("analyze","MaxGenPhotEta");
    tMaxJEta                   = edm::InputTag("analyze","MaxJEta");
    tMaxMuEta                  = edm::InputTag("analyze","MaxMuEta");
    tMaxNConv                  = edm::InputTag("analyze","MaxNConv");
    tMaxNEBhits                = edm::InputTag("analyze","MaxNEBhits");
    tMaxNEles                  = edm::InputTag("analyze","MaxNEles");
    tMaxNGenJet                = edm::InputTag("analyze","MaxNGenJet");
    tMaxNGenLep                = edm::InputTag("analyze","MaxNGenLep");
    tMaxNGenPho                = edm::InputTag("analyze","MaxNGenPho");
    tMaxNJets                  = edm::InputTag("analyze","MaxNJets");
    tMaxNMus                   = edm::InputTag("analyze","MaxNMus");
    tMaxNPfCand                = edm::InputTag("analyze","MaxNPfCand");
    tMaxNPhotons               = edm::InputTag("analyze","MaxNPhotons");
    tMaxNPileup                = edm::InputTag("analyze","MaxNPileup");
    tMaxNSC                    = edm::InputTag("analyze","MaxNSC");
    tMaxNTrks                  = edm::InputTag("analyze","MaxNTrks");
    tMaxNVrtx                  = edm::InputTag("analyze","MaxNVrtx");
    tMaxNXtals                 = edm::InputTag("analyze","MaxNXtals");
    tMaxPhotonEta              = edm::InputTag("analyze","MaxPhotonEta");
    tMaxTrkEta                 = edm::InputTag("analyze","MaxTrkEta");
    tMaxTrkNChi2               = edm::InputTag("analyze","MaxTrkNChi2");
    tMinEBRechitE              = edm::InputTag("analyze","MinEBRechitE");
    tMinElPt                   = edm::InputTag("analyze","MinElPt");
    tMinGenJetPt               = edm::InputTag("analyze","MinGenJetPt");
    tMinGenLeptPt              = edm::InputTag("analyze","MinGenLeptPt");
    tMinGenPhotPt              = edm::InputTag("analyze","MinGenPhotPt");
    tMinJEMfrac                = edm::InputTag("analyze","MinJEMfrac");
    tMinJPt                    = edm::InputTag("analyze","MinJPt");
    tMinMuPt                   = edm::InputTag("analyze","MinMuPt");

    tMinPhotonPt               = edm::InputTag("analyze","MinPhotonPt");
    tMinRawJPt                 = edm::InputTag("analyze","MinRawJPt");
    tMinSCraw                  = edm::InputTag("analyze","MinSCraw");
    tMinTrkNHits               = edm::InputTag("analyze","MinTrkNHits");
    tMinTrkPt                  = edm::InputTag("analyze","MinTrkPt");
    tPileUpData                = edm::InputTag("analyze","PileUpData");
    tPileUpMC                  = edm::InputTag("analyze","PileUpMC");
    tEcalDeadCellTriggerPrimitiveFilter = edm::InputTag("EcalDeadCellTriggerPrimitiveFilter","");
    tecalLaserCorrFilter       = edm::InputTag("ecalLaserCorrFilter","");
    teeBadScFilter             = edm::InputTag("eeBadScFilter","");
    thcalLaserEventFilter      = edm::InputTag("hcalLaserEventFilter","");
    tlogErrorTooManyClusters   = edm::InputTag("logErrorTooManyClusters","");
    tmanystripclus53X          = edm::InputTag("manystripclus53X","");
    ttoomanystripclus53X       = edm::InputTag("toomanystripclus53X","");
    ttrackingFailureFilter     = edm::InputTag("trackingFailureFilter","");
    tA0                        = edm::InputTag("analyze","A0");
    tBeamspotx                 = edm::InputTag("analyze","Beamspotx");
    tBeamspoty                 = edm::InputTag("analyze","Beamspoty");
    tBeamspotz                 = edm::InputTag("analyze","Beamspotz");
    tCAJArea                   = edm::InputTag("analyze","CAJArea");
    tCAJChfrac                 = edm::InputTag("analyze","CAJChfrac");
    tCAJE                      = edm::InputTag("analyze","CAJE");
    tCAJEMfrac                 = edm::InputTag("analyze","CAJEMfrac");
    tCAJEt                     = edm::InputTag("analyze","CAJEt");
    tCAJEta                    = edm::InputTag("analyze","CAJEta");
    tCAJFlavour                = edm::InputTag("analyze","CAJFlavour");
    tCAJIDHPD                  = edm::InputTag("analyze","CAJIDHPD");
    tCAJIDLoose                = edm::InputTag("analyze","CAJIDLoose");
    tCAJIDRBX                  = edm::InputTag("analyze","CAJIDRBX");
    tCAJIDn90Hits              = edm::InputTag("analyze","CAJIDn90Hits");
    tCAJIDresEMF               = edm::InputTag("analyze","CAJIDresEMF");
    tCAJL1FastJetScale         = edm::InputTag("analyze","CAJL1FastJetScale");
    tCAJNAssoTracks            = edm::InputTag("analyze","CAJNAssoTracks");
    tCAJNConstituents          = edm::InputTag("analyze","CAJNConstituents");
    tCAJPhi                    = edm::InputTag("analyze","CAJPhi");
    tCAJPt                     = edm::InputTag("analyze","CAJPt");
    tCAJPx                     = edm::InputTag("analyze","CAJPx");
    tCAJPy                     = edm::InputTag("analyze","CAJPy");
    tCAJPz                     = edm::InputTag("analyze","CAJPz");
    tCAJScale                  = edm::InputTag("analyze","CAJScale");
    tCAJn90                    = edm::InputTag("analyze","CAJn90");
    tCANJets                   = edm::InputTag("analyze","CANJets");
    tCSCTightHaloID            = edm::InputTag("analyze","CSCTightHaloID");
    tConvChi2Probability       = edm::InputTag("analyze","ConvChi2Probability");
    tConvEoverP                = edm::InputTag("analyze","ConvEoverP");
    tConvNtracks               = edm::InputTag("analyze","ConvNtracks");
    tConvValidVtx              = edm::InputTag("analyze","ConvValidVtx");
    tConvZofPrimVtxFromTrks    = edm::InputTag("analyze","ConvZofPrimVtxFromTrks");
    tDiphotonsfirst            = edm::InputTag("analyze","Diphotonsfirst");
    tDiphotonssecond           = edm::InputTag("analyze","Diphotonssecond");
    tEBrechitChi2              = edm::InputTag("analyze","EBrechitChi2");
    tEBrechitE                 = edm::InputTag("analyze","EBrechitE");
    tEBrechitE2oE9             = edm::InputTag("analyze","EBrechitE2oE9");
    tEBrechitE4oE1             = edm::InputTag("analyze","EBrechitE4oE1");
    tEBrechitEta               = edm::InputTag("analyze","EBrechitEta");
    tEBrechitPhi               = edm::InputTag("analyze","EBrechitPhi");
    tEBrechitPt                = edm::InputTag("analyze","EBrechitPt");
    tEBrechitTime              = edm::InputTag("analyze","EBrechitTime");
    tECALEsumx                 = edm::InputTag("analyze","ECALEsumx");
    tECALEsumy                 = edm::InputTag("analyze","ECALEsumy");
    tECALEsumz                 = edm::InputTag("analyze","ECALEsumz");
    tECALMET                   = edm::InputTag("analyze","ECALMET");
    tECALMETEta                = edm::InputTag("analyze","ECALMETEta");
    tECALMETPhi                = edm::InputTag("analyze","ECALMETPhi");
    tECALSumEt                 = edm::InputTag("analyze","ECALSumEt");
    tElBasicClustersSize       = edm::InputTag("analyze","ElBasicClustersSize");
    tElCInfoIsGsfCtfCons       = edm::InputTag("analyze","ElCInfoIsGsfCtfCons");
    tElCInfoIsGsfCtfScPixCons  = edm::InputTag("analyze","ElCInfoIsGsfCtfScPixCons");
    tElCInfoIsGsfScPixCons     = edm::InputTag("analyze","ElCInfoIsGsfScPixCons");
    tElCaloEnergy              = edm::InputTag("analyze","ElCaloEnergy");
    tElCharge                  = edm::InputTag("analyze","ElCharge");
    tElChargeMisIDProb         = edm::InputTag("analyze","ElChargeMisIDProb");
    tElClosestCtfTrackCharge   = edm::InputTag("analyze","ElClosestCtfTrackCharge");
    tElClosestCtfTrackEta      = edm::InputTag("analyze","ElClosestCtfTrackEta");
    tElClosestCtfTrackPhi      = edm::InputTag("analyze","ElClosestCtfTrackPhi");
    tElClosestCtfTrackPt       = edm::InputTag("analyze","ElClosestCtfTrackPt");
    tElConvPartnerTrkCharge    = edm::InputTag("analyze","ElConvPartnerTrkCharge");
    tElConvPartnerTrkDCot      = edm::InputTag("analyze","ElConvPartnerTrkDCot");
    tElConvPartnerTrkDist      = edm::InputTag("analyze","ElConvPartnerTrkDist");
    tElConvPartnerTrkEta       = edm::InputTag("analyze","ElConvPartnerTrkEta");
    tElConvPartnerTrkPhi       = edm::InputTag("analyze","ElConvPartnerTrkPhi");
    tElConvPartnerTrkPt        = edm::InputTag("analyze","ElConvPartnerTrkPt");
    tElD03DE                   = edm::InputTag("analyze","ElD03DE");
    tElD03DPV                  = edm::InputTag("analyze","ElD03DPV");
    tElD0BS                    = edm::InputTag("analyze","ElD0BS");
    tElD0E                     = edm::InputTag("analyze","ElD0E");
    tElD0PV                    = edm::InputTag("analyze","ElD0PV");
    tElDR03EcalRecHitSumEt     = edm::InputTag("analyze","ElDR03EcalRecHitSumEt");
    tElDR03HcalTowerSumEt      = edm::InputTag("analyze","ElDR03HcalTowerSumEt");
    tElDR03TkSumPt             = edm::InputTag("analyze","ElDR03TkSumPt");
    tElDR04EcalRecHitSumEt     = edm::InputTag("analyze","ElDR04EcalRecHitSumEt");
    tElDR04HcalTowerSumEt      = edm::InputTag("analyze","ElDR04HcalTowerSumEt");
    tElDR04TkSumPt             = edm::InputTag("analyze","ElDR04TkSumPt");
    tElDeltaEtaSeedClusterAtCalo = edm::InputTag("analyze","ElDeltaEtaSeedClusterAtCalo");
    tElDeltaEtaSuperClusterAtVtx = edm::InputTag("analyze","ElDeltaEtaSuperClusterAtVtx");
    tElDeltaPhiSeedClusterAtCalo = edm::InputTag("analyze","ElDeltaPhiSeedClusterAtCalo");
    tElDeltaPhiSuperClusterAtVtx = edm::InputTag("analyze","ElDeltaPhiSuperClusterAtVtx");
    tElDzBS                    = edm::InputTag("analyze","ElDzBS");
    tElDzE                     = edm::InputTag("analyze","ElDzE");
    tElDzPV                    = edm::InputTag("analyze","ElDzPV");
    tElE                       = edm::InputTag("analyze","ElE");
    tElE1OverE9                = edm::InputTag("analyze","ElE1OverE9");
    tElE1x5                    = edm::InputTag("analyze","ElE1x5");
    tElE2x5Max                 = edm::InputTag("analyze","ElE2x5Max");
    tElE5x5                    = edm::InputTag("analyze","ElE5x5");
    tElESuperClusterOverP      = edm::InputTag("analyze","ElESuperClusterOverP");
    tElEcalDriven              = edm::InputTag("analyze","ElEcalDriven");
    tElEcalEnergyError         = edm::InputTag("analyze","ElEcalEnergyError");
    tElEleMomentumError        = edm::InputTag("analyze","ElEleMomentumError");
    tElEoPout                  = edm::InputTag("analyze","ElEoPout");
    tElEt                      = edm::InputTag("analyze","ElEt");
    tElEta                     = edm::InputTag("analyze","ElEta");
    tElEventelPFIsoValueCharged03PFIdStandard = edm::InputTag("analyze","ElEventelPFIsoValueCharged03PFIdStandard");
    tElEventelPFIsoValueCharged04PFIdStandard = edm::InputTag("analyze","ElEventelPFIsoValueCharged04PFIdStandard");
    tElEventelPFIsoValueGamma03PFIdStandard = edm::InputTag("analyze","ElEventelPFIsoValueGamma03PFIdStandard");
    tElEventelPFIsoValueGamma04PFIdStandard = edm::InputTag("analyze","ElEventelPFIsoValueGamma04PFIdStandard");
    tElEventelPFIsoValueNeutral03PFIdStandard = edm::InputTag("analyze","ElEventelPFIsoValueNeutral03PFIdStandard");
    tElEventelPFIsoValueNeutral04PFIdStandard = edm::InputTag("analyze","ElEventelPFIsoValueNeutral04PFIdStandard");
    tElGenE                    = edm::InputTag("analyze","ElGenE");
    tElGenEta                  = edm::InputTag("analyze","ElGenEta");
    tElGenGME                  = edm::InputTag("analyze","ElGenGME");

    tGenLeptonGMEta            = edm::InputTag("analyze","GenLeptonGMEta");
    tGenLeptonGMID             = edm::InputTag("analyze","GenLeptonGMID");
    tGenLeptonGMPhi            = edm::InputTag("analyze","GenLeptonGMPhi");
    tGenLeptonGMPt             = edm::InputTag("analyze","GenLeptonGMPt");
    tGenLeptonGMStatus         = edm::InputTag("analyze","GenLeptonGMStatus");
    tGenLeptonID               = edm::InputTag("analyze","GenLeptonID");
    tGenLeptonMEta             = edm::InputTag("analyze","GenLeptonMEta");
    tGenLeptonMID              = edm::InputTag("analyze","GenLeptonMID");
    tGenLeptonMPhi             = edm::InputTag("analyze","GenLeptonMPhi");
    tGenLeptonMPt              = edm::InputTag("analyze","GenLeptonMPt");
    tGenLeptonMStatus          = edm::InputTag("analyze","GenLeptonMStatus");
    tGenLeptonPhi              = edm::InputTag("analyze","GenLeptonPhi");
    tGenLeptonPt               = edm::InputTag("analyze","GenLeptonPt");
    tGenMET                    = edm::InputTag("analyze","GenMET");
    tGenMETphi                 = edm::InputTag("analyze","GenMETphi");
    tGenMETpx                  = edm::InputTag("analyze","GenMETpx");
    tGenMETpy                  = edm::InputTag("analyze","GenMETpy");
    tGenPhotonEta              = edm::InputTag("analyze","GenPhotonEta");
    tGenPhotonIsoDR03          = edm::InputTag("analyze","GenPhotonIsoDR03");
    tGenPhotonIsoDR04          = edm::InputTag("analyze","GenPhotonIsoDR04");
    tGenPhotonMotherID         = edm::InputTag("analyze","GenPhotonMotherID");
    tGenPhotonMotherStatus     = edm::InputTag("analyze","GenPhotonMotherStatus");
    tGenPhotonPartonMindR      = edm::InputTag("analyze","GenPhotonPartonMindR");
    tGenPhotonPhi              = edm::InputTag("analyze","GenPhotonPhi");
    tGenPhotonPt               = edm::InputTag("analyze","GenPhotonPt");
    tGenWeight                 = edm::InputTag("analyze","GenWeight");
    tGoodEvent                 = edm::InputTag("analyze","GoodEvent");
    tGoodSCEnergy              = edm::InputTag("analyze","GoodSCEnergy");
    tGoodSCEta                 = edm::InputTag("analyze","GoodSCEta");
    tGoodSCPhi                 = edm::InputTag("analyze","GoodSCPhi");
    tHBHENoiseFilterResult     = edm::InputTag("HBHENoiseFilterResultProducer","HBHENoiseFilterResult");
    tHCALEsumx                 = edm::InputTag("analyze","HCALEsumx");
    tHCALEsumy                 = edm::InputTag("analyze","HCALEsumy");
    tHCALEsumz                 = edm::InputTag("analyze","HCALEsumz");
    tHCALMET                   = edm::InputTag("analyze","HCALMET");
    tHCALMETPhi                = edm::InputTag("analyze","HCALMETPhi");
    tHCALMETeta                = edm::InputTag("analyze","HCALMETeta");
    tHCALSumEt                 = edm::InputTag("analyze","HCALSumEt");
    tHLTObjectEta0             = edm::InputTag("analyze","HLTObjectEta0");
    tHLTObjectEta1             = edm::InputTag("analyze","HLTObjectEta1");
    tHLTObjectEta2             = edm::InputTag("analyze","HLTObjectEta2");
    tHLTObjectEta3             = edm::InputTag("analyze","HLTObjectEta3");
    tHLTObjectEta4             = edm::InputTag("analyze","HLTObjectEta4");
    tHLTObjectEta5             = edm::InputTag("analyze","HLTObjectEta5");
    tHLTObjectEta6             = edm::InputTag("analyze","HLTObjectEta6");
    tHLTObjectEta7             = edm::InputTag("analyze","HLTObjectEta7");
    tHLTObjectEta8             = edm::InputTag("analyze","HLTObjectEta8");
    tHLTObjectEta9             = edm::InputTag("analyze","HLTObjectEta9");
    tHLTObjectID0              = edm::InputTag("analyze","HLTObjectID0");
    tHLTObjectID1              = edm::InputTag("analyze","HLTObjectID1");
    tHLTObjectID2              = edm::InputTag("analyze","HLTObjectID2");
    tHLTObjectID3              = edm::InputTag("analyze","HLTObjectID3");
    tHLTObjectID4              = edm::InputTag("analyze","HLTObjectID4");
    tHLTObjectID5              = edm::InputTag("analyze","HLTObjectID5");
    tHLTObjectID6              = edm::InputTag("analyze","HLTObjectID6");
    tHLTObjectID7              = edm::InputTag("analyze","HLTObjectID7");
    tHLTObjectID8              = edm::InputTag("analyze","HLTObjectID8");
    tHLTObjectID9              = edm::InputTag("analyze","HLTObjectID9");
    tHLTObjectPhi0             = edm::InputTag("analyze","HLTObjectPhi0");
    tHLTObjectPhi1             = edm::InputTag("analyze","HLTObjectPhi1");
    tHLTObjectPhi2             = edm::InputTag("analyze","HLTObjectPhi2");
    tHLTObjectPhi3             = edm::InputTag("analyze","HLTObjectPhi3");
    tHLTObjectPhi4             = edm::InputTag("analyze","HLTObjectPhi4");
    tHLTObjectPhi5             = edm::InputTag("analyze","HLTObjectPhi5");
    tHLTObjectPhi6             = edm::InputTag("analyze","HLTObjectPhi6");
    tHLTObjectPhi7             = edm::InputTag("analyze","HLTObjectPhi7");
    tHLTObjectPhi8             = edm::InputTag("analyze","HLTObjectPhi8");
    tHLTObjectPhi9             = edm::InputTag("analyze","HLTObjectPhi9");
    tHLTObjectPt0              = edm::InputTag("analyze","HLTObjectPt0");
    tHLTObjectPt1              = edm::InputTag("analyze","HLTObjectPt1");
    tHLTObjectPt2              = edm::InputTag("analyze","HLTObjectPt2");
    tHLTObjectPt3              = edm::InputTag("analyze","HLTObjectPt3");
    tHLTObjectPt4              = edm::InputTag("analyze","HLTObjectPt4");
    tHLTObjectPt5              = edm::InputTag("analyze","HLTObjectPt5");
    tHLTObjectPt6              = edm::InputTag("analyze","HLTObjectPt6");
    tHLTObjectPt7              = edm::InputTag("analyze","HLTObjectPt7");
    tHLTObjectPt8              = edm::InputTag("analyze","HLTObjectPt8");
    tHLTObjectPt9              = edm::InputTag("analyze","HLTObjectPt9");
    tHLTPrescale               = edm::InputTag("analyze","HLTPrescale");
    tHLTResults                = edm::InputTag("analyze","HLTResults");
    tJArea                     = edm::InputTag("analyze","JArea");
    tJBeta                     = edm::InputTag("analyze","JBeta");
    tJBetaSq                   = edm::InputTag("analyze","JBetaSq");
    tJBetaStar                 = edm::InputTag("analyze","JBetaStar");
    tJChargedEmFrac            = edm::InputTag("analyze","JChargedEmFrac");
    tJChargedHadFrac           = edm::InputTag("analyze","JChargedHadFrac");
    tJChargedMuEnergyFrac      = edm::InputTag("analyze","JChargedMuEnergyFrac");
    tJE                        = edm::InputTag("analyze","JE");
    tJEcorr                    = edm::InputTag("analyze","JEcorr");
    tJEt                       = edm::InputTag("analyze","JEt");
    tJEta                      = edm::InputTag("analyze","JEta");
    tJEtaRms                   = edm::InputTag("analyze","JEtaRms");
    tJGenJetIndex              = edm::InputTag("analyze","JGenJetIndex");
    tJGood                     = edm::InputTag("analyze","JGood");
    tJHFEMFrac                 = edm::InputTag("analyze","JHFEMFrac");
    tJHFHadFrac                = edm::InputTag("analyze","JHFHadFrac");
    tJMass                     = edm::InputTag("analyze","JMass");
    tJMetCorrArea              = edm::InputTag("analyze","JMetCorrArea");
    tJMetCorrEMF               = edm::InputTag("analyze","JMetCorrEMF");
    tJMetCorrNoMuPt            = edm::InputTag("analyze","JMetCorrNoMuPt");
    tJMetCorrPhi               = edm::InputTag("analyze","JMetCorrPhi");
    tJMetCorrRawEta            = edm::InputTag("analyze","JMetCorrRawEta");
    tJMetCorrRawPt             = edm::InputTag("analyze","JMetCorrRawPt");
    tJNAssoTracks              = edm::InputTag("analyze","JNAssoTracks");
    tJNConstituents            = edm::InputTag("analyze","JNConstituents");
    tJNNeutrals                = edm::InputTag("analyze","JNNeutrals");
    tJNeutralEmFrac            = edm::InputTag("analyze","JNeutralEmFrac");
    tJNeutralHadFrac           = edm::InputTag("analyze","JNeutralHadFrac");
    tJPartonFlavour            = edm::InputTag("analyze","JPartonFlavour");
    tJPhi                      = edm::InputTag("analyze","JPhi");
    tJPhiRms                   = edm::InputTag("analyze","JPhiRms");
    tJPhoFrac                  = edm::InputTag("analyze","JPhoFrac");
    tJPt                       = edm::InputTag("analyze","JPt");
    tJPtD                      = edm::InputTag("analyze","JPtD");
    tJPx                       = edm::InputTag("analyze","JPx");
    tJPy                       = edm::InputTag("analyze","JPy");
    tJPz                       = edm::InputTag("analyze","JPz");
    tJRMSCand                  = edm::InputTag("analyze","JRMSCand");
    tJVtxExx                   = edm::InputTag("analyze","JVtxExx");
    tJVtxEyx                   = edm::InputTag("analyze","JVtxEyx");
    tJVtxEyy                   = edm::InputTag("analyze","JVtxEyy");
    tJVtxEzx                   = edm::InputTag("analyze","JVtxEzx");
    tJVtxEzy                   = edm::InputTag("analyze","JVtxEzy");
    tJVtxEzz                   = edm::InputTag("analyze","JVtxEzz");
    tJVtxNChi2                 = edm::InputTag("analyze","JVtxNChi2");
    tJVtxx                     = edm::InputTag("analyze","JVtxx");
    tJVtxy                     = edm::InputTag("analyze","JVtxy");
    tJVtxz                     = edm::InputTag("analyze","JVtxz");
    tJeMinDR                   = edm::InputTag("analyze","JeMinDR");
    tJnewPFCombinedSecondaryVertexBPFJetTags = edm::InputTag("analyze","JnewPFCombinedSecondaryVertexBPFJetTags");
    tJnewPFCombinedSecondaryVertexMVABPFJetTags = edm::InputTag("analyze","JnewPFCombinedSecondaryVertexMVABPFJetTags");
    tJnewPFJetBProbabilityBPFJetTags = edm::InputTag("analyze","JnewPFJetBProbabilityBPFJetTags");
    tJnewPFJetProbabilityBPFJetTags = edm::InputTag("analyze","JnewPFJetProbabilityBPFJetTags");
    tJnewPFSimpleSecondaryVertexHighEffBJetTags = edm::InputTag("analyze","JnewPFSimpleSecondaryVertexHighEffBJetTags");
    tJnewPFSimpleSecondaryVertexHighPurBJetTags = edm::InputTag("analyze","JnewPFSimpleSecondaryVertexHighPurBJetTags");
    tJnewPFTrackCountingHighEffBJetTags = edm::InputTag("analyze","JnewPFTrackCountingHighEffBJetTags");
    tJnewPFTrackCountingHighPurBJetTags = edm::InputTag("analyze","JnewPFTrackCountingHighPurBJetTags");
    tJtrk1px                   = edm::InputTag("analyze","Jtrk1px");
    tJtrk1py                   = edm::InputTag("analyze","Jtrk1py");
    tJtrk1pz                   = edm::InputTag("analyze","Jtrk1pz");
    tJtrk2px                   = edm::InputTag("analyze","Jtrk2px");
    tJtrk2py                   = edm::InputTag("analyze","Jtrk2py");
    tJtrk2pz                   = edm::InputTag("analyze","Jtrk2pz");
    tJtrk3px                   = edm::InputTag("analyze","Jtrk3px");
    tJtrk3py                   = edm::InputTag("analyze","Jtrk3py");
    tJtrk3pz                   = edm::InputTag("analyze","Jtrk3pz");
    tL1PhysResults             = edm::InputTag("analyze","L1PhysResults");
    tL1TechResults             = edm::InputTag("analyze","L1TechResults");
    tLHEEventE                 = edm::InputTag("analyze","LHEEventE");
    tLHEEventID                = edm::InputTag("analyze","LHEEventID");
    tLHEEventM                 = edm::InputTag("analyze","LHEEventM");
    tLHEEventMotherFirst       = edm::InputTag("analyze","LHEEventMotherFirst");
    tLHEEventMotherSecond      = edm::InputTag("analyze","LHEEventMotherSecond");
    tLHEEventPx                = edm::InputTag("analyze","LHEEventPx");
    tLHEEventPy                = edm::InputTag("analyze","LHEEventPy");
    tLHEEventPz                = edm::InputTag("analyze","LHEEventPz");
    tLHEEventStatus            = edm::InputTag("analyze","LHEEventStatus");
    tLumiSection               = edm::InputTag("analyze","LumiSection");
    tM0                        = edm::InputTag("analyze","M0");
    tM12                       = edm::InputTag("analyze","M12");
    tMETR12                    = edm::InputTag("analyze","METR12");
    tMETR21                    = edm::InputTag("analyze","METR21");
    tMassChi                   = edm::InputTag("analyze","MassChi");
    tMassGlu                   = edm::InputTag("analyze","MassGlu");
    tMassLSP                   = edm::InputTag("analyze","MassLSP");
    tMaxElExceed               = edm::InputTag("analyze","MaxElExceed");
    tMaxGenJetExceed           = edm::InputTag("analyze","MaxGenJetExceed");
    tMaxGenLepExceed           = edm::InputTag("analyze","MaxGenLepExceed");
    tMaxGenPartExceed          = edm::InputTag("analyze","MaxGenPartExceed");
    tMaxGenPhoExceed           = edm::InputTag("analyze","MaxGenPhoExceed");
    tMaxJetExceed              = edm::InputTag("analyze","MaxJetExceed");
    tMaxMuExceed               = edm::InputTag("analyze","MaxMuExceed");
    tMaxPhotonsExceed          = edm::InputTag("analyze","MaxPhotonsExceed");
    tMaxTrkExceed              = edm::InputTag("analyze","MaxTrkExceed");
    tMaxUncJetExceed           = edm::InputTag("analyze","MaxUncJetExceed");
    tMaxVerticesExceed         = edm::InputTag("analyze","MaxVerticesExceed");
    tMuCaloComp                = edm::InputTag("analyze","MuCaloComp");
    tMuCharge                  = edm::InputTag("analyze","MuCharge");
    tMuD03DE                   = edm::InputTag("analyze","MuD03DE");
    tMuD03DPV                  = edm::InputTag("analyze","MuD03DPV");
    tMuD0BS                    = edm::InputTag("analyze","MuD0BS");
    tMuD0E                     = edm::InputTag("analyze","MuD0E");
    tMuD0PV                    = edm::InputTag("analyze","MuD0PV");
    tMuDzBS                    = edm::InputTag("analyze","MuDzBS");
    tMuDzE                     = edm::InputTag("analyze","MuDzE");
    tMuDzPV                    = edm::InputTag("analyze","MuDzPV");
    tMuE                       = edm::InputTag("analyze","MuE");
    tMuEem                     = edm::InputTag("analyze","MuEem");
    tMuEhad                    = edm::InputTag("analyze","MuEhad");
    tMuEt                      = edm::InputTag("analyze","MuEt");
    tMuEta                     = edm::InputTag("analyze","MuEta");
    tMuGenE                    = edm::InputTag("analyze","MuGenE");
    tMuGenEta                  = edm::InputTag("analyze","MuGenEta");
    tMuGenGME                  = edm::InputTag("analyze","MuGenGME");
    tMuGenGMEta                = edm::InputTag("analyze","MuGenGMEta");
    tMuGenGMID                 = edm::InputTag("analyze","MuGenGMID");
    tMuGenGMPhi                = edm::InputTag("analyze","MuGenGMPhi");
    tMuGenGMPt                 = edm::InputTag("analyze","MuGenGMPt");
    tMuGenGMStatus             = edm::InputTag("analyze","MuGenGMStatus");
    tMuGenID                   = edm::InputTag("analyze","MuGenID");
    tMuGenME                   = edm::InputTag("analyze","MuGenME");
    tMuGenMEta                 = edm::InputTag("analyze","MuGenMEta");
    tMuGenMID                  = edm::InputTag("analyze","MuGenMID");
    tMuGenMPhi                 = edm::InputTag("analyze","MuGenMPhi");
    tMuGenMPt                  = edm::InputTag("analyze","MuGenMPt");
    tMuGenMStatus              = edm::InputTag("analyze","MuGenMStatus");
    tMuGenPhi                  = edm::InputTag("analyze","MuGenPhi");
    tMuGenPt                   = edm::InputTag("analyze","MuGenPt");
    tMuGenStatus               = edm::InputTag("analyze","MuGenStatus");
    tMuGood                    = edm::InputTag("analyze","MuGood");
    tMuInnerTkNChi2            = edm::InputTag("analyze","MuInnerTkNChi2");
    tMuInnerTkPt               = edm::InputTag("analyze","MuInnerTkPt");
    tMuIsAllArbitrated         = edm::InputTag("analyze","MuIsAllArbitrated");
    tMuIsAllStaMuons           = edm::InputTag("analyze","MuIsAllStaMuons");
    tMuIsAllTrkMuons           = edm::InputTag("analyze","MuIsAllTrkMuons");
    tMuIsGMPT                  = edm::InputTag("analyze","MuIsGMPT");
    tMuIsGMStaChiComp          = edm::InputTag("analyze","MuIsGMStaChiComp");
    tMuIsGMTkChiComp           = edm::InputTag("analyze","MuIsGMTkChiComp");
    tMuIsGMTkKinkTight         = edm::InputTag("analyze","MuIsGMTkKinkTight");
    tMuIsGlobalMuon            = edm::InputTag("analyze","MuIsGlobalMuon");
    tMuIsIso                   = edm::InputTag("analyze","MuIsIso");
    tMuIsPFMuon                = edm::InputTag("analyze","MuIsPFMuon");
    tMuIsStandaloneMuon        = edm::InputTag("analyze","MuIsStandaloneMuon");
    tMuIsTM2DCompLoose         = edm::InputTag("analyze","MuIsTM2DCompLoose");
    tMuIsTM2DCompTight         = edm::InputTag("analyze","MuIsTM2DCompTight");
    tMuIsTMLSAngLoose          = edm::InputTag("analyze","MuIsTMLSAngLoose");
    tMuIsTMLSAngTight          = edm::InputTag("analyze","MuIsTMLSAngTight");
    tMuIsTMLSLoose             = edm::InputTag("analyze","MuIsTMLSLoose");
    tMuIsTMLSOptLowPtLoose     = edm::InputTag("analyze","MuIsTMLSOptLowPtLoose");
    tMuIsTMLSTight             = edm::InputTag("analyze","MuIsTMLSTight");
    tMuIsTMOneStationAngLoose  = edm::InputTag("analyze","MuIsTMOneStationAngLoose");
    tMuIsTMOneStationAngTight  = edm::InputTag("analyze","MuIsTMOneStationAngTight");
    tMuIsTMOneStationLoose     = edm::InputTag("analyze","MuIsTMOneStationLoose");
    tMuIsTMOneStationTight     = edm::InputTag("analyze","MuIsTMOneStationTight");
    tMuIsTrackerMuon           = edm::InputTag("analyze","MuIsTrackerMuon");
    tMuIsTrkMuonArbitrated     = edm::InputTag("analyze","MuIsTrkMuonArbitrated");
    tMuIso03EMVetoEt           = edm::InputTag("analyze","MuIso03EMVetoEt");
    tMuIso03EmEt               = edm::InputTag("analyze","MuIso03EmEt");
    tMuIso03HadEt              = edm::InputTag("analyze","MuIso03HadEt");
    tMuIso03HadVetoEt          = edm::InputTag("analyze","MuIso03HadVetoEt");
    tMuIso03SumPt              = edm::InputTag("analyze","MuIso03SumPt");
    tMuIso05EmEt               = edm::InputTag("analyze","MuIso05EmEt");
    tMuIso05HadEt              = edm::InputTag("analyze","MuIso05HadEt");
    tMuIso05SumPt              = edm::InputTag("analyze","MuIso05SumPt");
    tMuIsoMVA                  = edm::InputTag("analyze","MuIsoMVA");
    tMuJESCorrMET              = edm::InputTag("analyze","MuJESCorrMET");
    tMuJESCorrMETphi           = edm::InputTag("analyze","MuJESCorrMETphi");
    tMuJESCorrMETpx            = edm::InputTag("analyze","MuJESCorrMETpx");
    tMuJESCorrMETpy            = edm::InputTag("analyze","MuJESCorrMETpy");
    tMuNChambers               = edm::InputTag("analyze","MuNChambers");
    tMuNChi2                   = edm::InputTag("analyze","MuNChi2");
    tMuNGlHits                 = edm::InputTag("analyze","MuNGlHits");
    tMuNGlMuHits               = edm::InputTag("analyze","MuNGlMuHits");
    tMuNMatchedStations        = edm::InputTag("analyze","MuNMatchedStations");
    tMuNMatches                = edm::InputTag("analyze","MuNMatches");
    tMuNMuHits                 = edm::InputTag("analyze","MuNMuHits");
    tMuNPxHits                 = edm::InputTag("analyze","MuNPxHits");
    tMuNSiLayers               = edm::InputTag("analyze","MuNSiLayers");
    tMuNTkHits                 = edm::InputTag("analyze","MuNTkHits");
    tMuPfIsoR03ChHad           = edm::InputTag("analyze","MuPfIsoR03ChHad");
    tMuPfIsoR03NeHad           = edm::InputTag("analyze","MuPfIsoR03NeHad");
    tMuPfIsoR03NeHadHighThresh = edm::InputTag("analyze","MuPfIsoR03NeHadHighThresh");
    tMuPfIsoR03Photon          = edm::InputTag("analyze","MuPfIsoR03Photon");
    tMuPfIsoR03PhotonHighThresh = edm::InputTag("analyze","MuPfIsoR03PhotonHighThresh");
    tMuPfIsoR03SumPUPt         = edm::InputTag("analyze","MuPfIsoR03SumPUPt");
    tMuPfIsoR04ChHad           = edm::InputTag("analyze","MuPfIsoR04ChHad");
    tMuPfIsoR04NeHad           = edm::InputTag("analyze","MuPfIsoR04NeHad");
    tMuPfIsoR04NeHadHighThresh = edm::InputTag("analyze","MuPfIsoR04NeHadHighThresh");
    tMuPfIsoR04Photon          = edm::InputTag("analyze","MuPfIsoR04Photon");
    tMuPfIsoR04PhotonHighThresh = edm::InputTag("analyze","MuPfIsoR04PhotonHighThresh");
    tMuPfIsoR04SumPUPt         = edm::InputTag("analyze","MuPfIsoR04SumPUPt");
    tMuPhi                     = edm::InputTag("analyze","MuPhi");
    tMuPt                      = edm::InputTag("analyze","MuPt");
    tMuPtE                     = edm::InputTag("analyze","MuPtE");
    tMuPx                      = edm::InputTag("analyze","MuPx");
    tMuPy                      = edm::InputTag("analyze","MuPy");
    tMuPz                      = edm::InputTag("analyze","MuPz");
    tMuRelIso03                = edm::InputTag("analyze","MuRelIso03");
    tMuSegmComp                = edm::InputTag("analyze","MuSegmComp");
    tMuTkD0E                   = edm::InputTag("analyze","MuTkD0E");
    tMuTkDzE                   = edm::InputTag("analyze","MuTkDzE");
    tMuTkPtE                   = edm::InputTag("analyze","MuTkPtE");
    tMumuonPFIsoChHad03        = edm::InputTag("analyze","MumuonPFIsoChHad03");
    tMumuonPFIsoChHad04        = edm::InputTag("analyze","MumuonPFIsoChHad04");
    tMumuonPFIsoNHad03         = edm::InputTag("analyze","MumuonPFIsoNHad03");
    tMumuonPFIsoNHad04         = edm::InputTag("analyze","MumuonPFIsoNHad04");
    tMumuonPFIsoPhoton03       = edm::InputTag("analyze","MumuonPFIsoPhoton03");
    tMumuonPFIsoPhoton04       = edm::InputTag("analyze","MumuonPFIsoPhoton04");
    tMumuonRadPFIsoChHad03     = edm::InputTag("analyze","MumuonRadPFIsoChHad03");
    tMumuonRadPFIsoChHad04     = edm::InputTag("analyze","MumuonRadPFIsoChHad04");
    tMumuonRadPFIsoNHad03      = edm::InputTag("analyze","MumuonRadPFIsoNHad03");
    tMumuonRadPFIsoNHad04      = edm::InputTag("analyze","MumuonRadPFIsoNHad04");
    tMumuonRadPFIsoPhoton03    = edm::InputTag("analyze","MumuonRadPFIsoPhoton03");
    tMumuonRadPFIsoPhoton04    = edm::InputTag("analyze","MumuonRadPFIsoPhoton04");
    tNCaloTowers               = edm::InputTag("analyze","NCaloTowers");
    tNEBhits                   = edm::InputTag("analyze","NEBhits");
    tNEles                     = edm::InputTag("analyze","NEles");
    tNElesTot                  = edm::InputTag("analyze","NElesTot");
    tNGMus                     = edm::InputTag("analyze","NGMus");
    tNGenJets                  = edm::InputTag("analyze","NGenJets");
    tNGenLeptons               = edm::InputTag("analyze","NGenLeptons");
    tNGenPhotons               = edm::InputTag("analyze","NGenPhotons");
    tNGoodSuperClusters        = edm::InputTag("analyze","NGoodSuperClusters");
    tNHLTObjs                  = edm::InputTag("analyze","NHLTObjs");
    tNJets                     = edm::InputTag("analyze","NJets");
    tNJetsTot                  = edm::InputTag("analyze","NJetsTot");
    tNMus                      = edm::InputTag("analyze","NMus");
    tNMusTot                   = edm::InputTag("analyze","NMusTot");
    tNPdfs                     = edm::InputTag("analyze","NPdfs");
    tNPfCand                   = edm::InputTag("analyze","NPfCand");
    tNPhotons                  = edm::InputTag("analyze","NPhotons");
    tNPhotonsTot               = edm::InputTag("analyze","NPhotonsTot");
    tNSuperClusters            = edm::InputTag("analyze","NSuperClusters");
    tNTMus                     = edm::InputTag("analyze","NTMus");
    tNTracks                   = edm::InputTag("analyze","NTracks");
    tNTracksTot                = edm::InputTag("analyze","NTracksTot");
    tNVrtx                     = edm::InputTag("analyze","NVrtx");
    tNXtals                    = edm::InputTag("analyze","NXtals");
    tNconv                     = edm::InputTag("analyze","Nconv");
    tNgv                       = edm::InputTag("analyze","Ngv");
    tPDFID1                    = edm::InputTag("analyze","PDFID1");
    tPDFID2                    = edm::InputTag("analyze","PDFID2");
    tPDFScalePDF               = edm::InputTag("analyze","PDFScalePDF");
    tPDFx1                     = edm::InputTag("analyze","PDFx1");
    tPDFx2                     = edm::InputTag("analyze","PDFx2");
    tPDFxPDF1                  = edm::InputTag("analyze","PDFxPDF1");
    tPDFxPDF2                  = edm::InputTag("analyze","PDFxPDF2");
    tPFCE                      = edm::InputTag("analyze","PFCE");
    tPFCEt                     = edm::InputTag("analyze","PFCEt");
    tPFCEta                    = edm::InputTag("analyze","PFCEta");
    tPFCHSJArea                = edm::InputTag("analyze","PFCHSJArea");
    tPFCHSJChEmfrac            = edm::InputTag("analyze","PFCHSJChEmfrac");
    tPFCHSJChHadfrac           = edm::InputTag("analyze","PFCHSJChHadfrac");
    tPFCHSJChMufrac            = edm::InputTag("analyze","PFCHSJChMufrac");
    tPFCHSJChMult              = edm::InputTag("analyze","PFCHSJChMult");
    tPFCHSJE                   = edm::InputTag("analyze","PFCHSJE");
    tPFCHSJEt                  = edm::InputTag("analyze","PFCHSJEt");
    tPFCHSJEta                 = edm::InputTag("analyze","PFCHSJEta");
    tPFCHSJFlavour             = edm::InputTag("analyze","PFCHSJFlavour");
    tPFCHSJHFEMfrac            = edm::InputTag("analyze","PFCHSJHFEMfrac");
    tPFCHSJHFHadfrac           = edm::InputTag("analyze","PFCHSJHFHadfrac");
    tPFCHSJIDLoose             = edm::InputTag("analyze","PFCHSJIDLoose");
    tPFCHSJL1FastJetScale      = edm::InputTag("analyze","PFCHSJL1FastJetScale");
    tPFCHSJNConstituents       = edm::InputTag("analyze","PFCHSJNConstituents");
    tPFCHSJNeuEmfrac           = edm::InputTag("analyze","PFCHSJNeuEmfrac");
    tPFCHSJNeuHadfrac          = edm::InputTag("analyze","PFCHSJNeuHadfrac");
    tPFCHSJNeuMult             = edm::InputTag("analyze","PFCHSJNeuMult");
    tPFCHSJPhi                 = edm::InputTag("analyze","PFCHSJPhi");
    tPFCHSJPhofrac             = edm::InputTag("analyze","PFCHSJPhofrac");
    tPFCHSJPt                  = edm::InputTag("analyze","PFCHSJPt");
    tPFCHSJPx                  = edm::InputTag("analyze","PFCHSJPx");
    tPFCHSJPy                  = edm::InputTag("analyze","PFCHSJPy");
    tPFCHSJPz                  = edm::InputTag("analyze","PFCHSJPz");
    tPFCHSJScale               = edm::InputTag("analyze","PFCHSJScale");
    tPFCHSJcombinedSecondaryVertexBJetTags = edm::InputTag("analyze","PFCHSJcombinedSecondaryVertexBJetTags");
    tPFCHSJcombinedSecondaryVertexMVABJetTags = edm::InputTag("analyze","PFCHSJcombinedSecondaryVertexMVABJetTags");
    tPFCHSJjetBProbabilityBJetTags = edm::InputTag("analyze","PFCHSJjetBProbabilityBJetTags");
    tPFCHSJjetProbabilityBJetTags = edm::InputTag("analyze","PFCHSJjetProbabilityBJetTags");
    tPFCHSJsimpleSecondaryVertexHighEffBJetTags = edm::InputTag("analyze","PFCHSJsimpleSecondaryVertexHighEffBJetTags");
    tPFCHSJsimpleSecondaryVertexHighPurBJetTags = edm::InputTag("analyze","PFCHSJsimpleSecondaryVertexHighPurBJetTags");
    tPFCHSJtrackCountingHighEffBJetTags = edm::InputTag("analyze","PFCHSJtrackCountingHighEffBJetTags");
    tPFCHSJtrackCountingHighPurBJetTags = edm::InputTag("analyze","PFCHSJtrackCountingHighPurBJetTags");
    tPFCHSNJets                = edm::InputTag("analyze","PFCHSNJets");
    tPFCNCandidates            = edm::InputTag("analyze","PFCNCandidates");
    tPFCPhi                    = edm::InputTag("analyze","PFCPhi");
    tPFCPt                     = edm::InputTag("analyze","PFCPt");
    tPFCPx                     = edm::InputTag("analyze","PFCPx");
    tPFCPy                     = edm::InputTag("analyze","PFCPy");
    tPFCPz                     = edm::InputTag("analyze","PFCPz");
    tPFCType                   = edm::InputTag("analyze","PFCType");
    tPFCVx                     = edm::InputTag("analyze","PFCVx");
    tPFCVy                     = edm::InputTag("analyze","PFCVy");
    tPFCVz                     = edm::InputTag("analyze","PFCVz");
    tPFMET                     = edm::InputTag("analyze","PFMET");
    tPFMETSignificance         = edm::InputTag("analyze","PFMETSignificance");
    tPFMETphi                  = edm::InputTag("analyze","PFMETphi");
    tPFMETpx                   = edm::InputTag("analyze","PFMETpx");
    tPFMETpy                   = edm::InputTag("analyze","PFMETpy");
    tPFSumEt                   = edm::InputTag("analyze","PFSumEt");
    tPFType1MET                = edm::InputTag("analyze","PFType1MET");
    tPFType1METSignificance    = edm::InputTag("analyze","PFType1METSignificance");
    tPFType1METphi             = edm::InputTag("analyze","PFType1METphi");
    tPFType1METpx              = edm::InputTag("analyze","PFType1METpx");
    tPFType1METpy              = edm::InputTag("analyze","PFType1METpy");
    tPFType1SumEt              = edm::InputTag("analyze","PFType1SumEt");
    tPUOOTnumInteractionsEarly = edm::InputTag("analyze","PUOOTnumInteractionsEarly");
    tPUOOTnumInteractionsLate  = edm::InputTag("analyze","PUOOTnumInteractionsLate");
    tPUWeightInTime            = edm::InputTag("analyze","PUWeightInTime");
    tPUWeightTotal             = edm::InputTag("analyze","PUWeightTotal");
    tPUnTrksHighPt             = edm::InputTag("analyze","PUnTrksHighPt");
    tPUnTrksLowPt              = edm::InputTag("analyze","PUnTrksLowPt");
    tPUnumFilled               = edm::InputTag("analyze","PUnumFilled");
    tPUnumInteractions         = edm::InputTag("analyze","PUnumInteractions");
    tPUnumTrueInteractions     = edm::InputTag("analyze","PUnumTrueInteractions");
    tPUsumPtHighPt             = edm::InputTag("analyze","PUsumPtHighPt");
    tPUsumPtLowPt              = edm::InputTag("analyze","PUsumPtLowPt");
    tPUzPositions              = edm::InputTag("analyze","PUzPositions");
    tPfCandEnergy              = edm::InputTag("analyze","PfCandEnergy");
    tPfCandEta                 = edm::InputTag("analyze","PfCandEta");
    tPfCandHasHitInFirstPixelLayer = edm::InputTag("analyze","PfCandHasHitInFirstPixelLayer");
    tPfCandPdgId               = edm::InputTag("analyze","PfCandPdgId");
    tPfCandPhi                 = edm::InputTag("analyze","PfCandPhi");
    tPfCandPt                  = edm::InputTag("analyze","PfCandPt");
    tPfCandTrackRefPx          = edm::InputTag("analyze","PfCandTrackRefPx");
    tPfCandTrackRefPy          = edm::InputTag("analyze","PfCandTrackRefPy");
    tPfCandTrackRefPz          = edm::InputTag("analyze","PfCandTrackRefPz");
    tPfCandVx                  = edm::InputTag("analyze","PfCandVx");
    tPfCandVy                  = edm::InputTag("analyze","PfCandVy");
    tPfCandVz                  = edm::InputTag("analyze","PfCandVz");
    tPhoCaloPositionX          = edm::InputTag("analyze","PhoCaloPositionX");
    tPhoCaloPositionY          = edm::InputTag("analyze","PhoCaloPositionY");
    tPhoCaloPositionZ          = edm::InputTag("analyze","PhoCaloPositionZ");
    tPhoChargedHadronIso       = edm::InputTag("analyze","PhoChargedHadronIso");
    tPhoCone01ChargedHadronIsodR015dEta0pt0PFnoPU = edm::InputTag("analyze","PhoCone01ChargedHadronIsodR015dEta0pt0PFnoPU");
    tPhoCone01ChargedHadronIsodR015dEta0pt0dz0 = edm::InputTag("analyze","PhoCone01ChargedHadronIsodR015dEta0pt0dz0");
    tPhoCone01ChargedHadronIsodR015dEta0pt0dz1dxy01 = edm::InputTag("analyze","PhoCone01ChargedHadronIsodR015dEta0pt0dz1dxy01");
    tPhoCone01ChargedHadronIsodR02dz02dxy01 = edm::InputTag("analyze","PhoCone01ChargedHadronIsodR02dz02dxy01");
    tPhoCone01ChargedHadronIsodR0dEta0pt0PFnoPU = edm::InputTag("analyze","PhoCone01ChargedHadronIsodR0dEta0pt0PFnoPU");
    tPhoCone01ChargedHadronIsodR0dEta0pt0dz0 = edm::InputTag("analyze","PhoCone01ChargedHadronIsodR0dEta0pt0dz0");
    tPhoCone01ChargedHadronIsodR0dEta0pt0dz1dxy01 = edm::InputTag("analyze","PhoCone01ChargedHadronIsodR0dEta0pt0dz1dxy01");
    tPhoCone01NeutralHadronIsodR0dEta0pt0mvVtx = edm::InputTag("analyze","PhoCone01NeutralHadronIsodR0dEta0pt0mvVtx");
    tPhoCone01NeutralHadronIsomvVtx = edm::InputTag("analyze","PhoCone01NeutralHadronIsomvVtx");
    tPhoCone01PhotonIsodEta015EBdR070EEmvVtx = edm::InputTag("analyze","PhoCone01PhotonIsodEta015EBdR070EEmvVtx");
    tPhoCone01PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx = edm::InputTag("analyze","PhoCone01PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx");
    tPhoCone02ChargedHadronIsodR015dEta0pt0PFnoPU = edm::InputTag("analyze","PhoCone02ChargedHadronIsodR015dEta0pt0PFnoPU");
    tPhoCone02ChargedHadronIsodR015dEta0pt0dz0 = edm::InputTag("analyze","PhoCone02ChargedHadronIsodR015dEta0pt0dz0");
    tPhoCone02ChargedHadronIsodR015dEta0pt0dz1dxy01 = edm::InputTag("analyze","PhoCone02ChargedHadronIsodR015dEta0pt0dz1dxy01");
    tPhoCone02ChargedHadronIsodR02dz02dxy01 = edm::InputTag("analyze","PhoCone02ChargedHadronIsodR02dz02dxy01");
    tPhoCone02ChargedHadronIsodR0dEta0pt0PFnoPU = edm::InputTag("analyze","PhoCone02ChargedHadronIsodR0dEta0pt0PFnoPU");
    tPhoCone02ChargedHadronIsodR0dEta0pt0dz0 = edm::InputTag("analyze","PhoCone02ChargedHadronIsodR0dEta0pt0dz0");
    tPhoCone02ChargedHadronIsodR0dEta0pt0dz1dxy01 = edm::InputTag("analyze","PhoCone02ChargedHadronIsodR0dEta0pt0dz1dxy01");
    tPhoCone02NeutralHadronIsodR0dEta0pt0mvVtx = edm::InputTag("analyze","PhoCone02NeutralHadronIsodR0dEta0pt0mvVtx");
    tPhoCone02NeutralHadronIsomvVtx = edm::InputTag("analyze","PhoCone02NeutralHadronIsomvVtx");
    tPhoCone02PhotonIsodEta015EBdR070EEmvVtx = edm::InputTag("analyze","PhoCone02PhotonIsodEta015EBdR070EEmvVtx");
    tPhoCone02PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx = edm::InputTag("analyze","PhoCone02PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx");
    tPhoCone03ChargedHadronIsodR015dEta0pt0PFnoPU = edm::InputTag("analyze","PhoCone03ChargedHadronIsodR015dEta0pt0PFnoPU");
    tPhoCone03ChargedHadronIsodR015dEta0pt0dz0 = edm::InputTag("analyze","PhoCone03ChargedHadronIsodR015dEta0pt0dz0");
    tPhoCone03ChargedHadronIsodR015dEta0pt0dz1dxy01 = edm::InputTag("analyze","PhoCone03ChargedHadronIsodR015dEta0pt0dz1dxy01");
    tPhoCone03ChargedHadronIsodR02dz02dxy01 = edm::InputTag("analyze","PhoCone03ChargedHadronIsodR02dz02dxy01");
    tPhoCone03ChargedHadronIsodR0dEta0pt0PFnoPU = edm::InputTag("analyze","PhoCone03ChargedHadronIsodR0dEta0pt0PFnoPU");
    tPhoCone03ChargedHadronIsodR0dEta0pt0dz0 = edm::InputTag("analyze","PhoCone03ChargedHadronIsodR0dEta0pt0dz0");
    tPhoCone03ChargedHadronIsodR0dEta0pt0dz1dxy01 = edm::InputTag("analyze","PhoCone03ChargedHadronIsodR0dEta0pt0dz1dxy01");
    tPhoCone03NeutralHadronIsodR0dEta0pt0mvVtx = edm::InputTag("analyze","PhoCone03NeutralHadronIsodR0dEta0pt0mvVtx");
    tPhoCone03NeutralHadronIsomvVtx = edm::InputTag("analyze","PhoCone03NeutralHadronIsomvVtx");
    tPhoCone03PFCombinedIso    = edm::InputTag("analyze","PhoCone03PFCombinedIso");
    tPhoCone03PhotonIsodEta015EBdR070EEmvVtx = edm::InputTag("analyze","PhoCone03PhotonIsodEta015EBdR070EEmvVtx");
    tPhoCone03PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx = edm::InputTag("analyze","PhoCone03PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx");
    tPhoCone04ChargedHadronIsodR015dEta0pt0PFnoPU = edm::InputTag("analyze","PhoCone04ChargedHadronIsodR015dEta0pt0PFnoPU");
    tPhoCone04ChargedHadronIsodR015dEta0pt0PFnoPUold = edm::InputTag("analyze","PhoCone04ChargedHadronIsodR015dEta0pt0PFnoPUold");
    tPhoCone04ChargedHadronIsodR015dEta0pt0dz0 = edm::InputTag("analyze","PhoCone04ChargedHadronIsodR015dEta0pt0dz0");
    tPhoCone04ChargedHadronIsodR015dEta0pt0dz0old = edm::InputTag("analyze","PhoCone04ChargedHadronIsodR015dEta0pt0dz0old");
    tPhoCone04ChargedHadronIsodR015dEta0pt0dz1dxy01 = edm::InputTag("analyze","PhoCone04ChargedHadronIsodR015dEta0pt0dz1dxy01");
    tPhoCone04ChargedHadronIsodR02dz02dxy01 = edm::InputTag("analyze","PhoCone04ChargedHadronIsodR02dz02dxy01");
    tPhoCone04ChargedHadronIsodR0dEta0pt0PFnoPU = edm::InputTag("analyze","PhoCone04ChargedHadronIsodR0dEta0pt0PFnoPU");
    tPhoCone04ChargedHadronIsodR0dEta0pt0PFnoPUold = edm::InputTag("analyze","PhoCone04ChargedHadronIsodR0dEta0pt0PFnoPUold");
    tPhoCone04ChargedHadronIsodR0dEta0pt0dz0 = edm::InputTag("analyze","PhoCone04ChargedHadronIsodR0dEta0pt0dz0");
    tPhoCone04ChargedHadronIsodR0dEta0pt0dz0old = edm::InputTag("analyze","PhoCone04ChargedHadronIsodR0dEta0pt0dz0old");
    tPhoCone04ChargedHadronIsodR0dEta0pt0dz1dxy01 = edm::InputTag("analyze","PhoCone04ChargedHadronIsodR0dEta0pt0dz1dxy01");
    tPhoCone04NeutralHadronIsodR0dEta0pt0 = edm::InputTag("analyze","PhoCone04NeutralHadronIsodR0dEta0pt0");
    tPhoCone04NeutralHadronIsodR0dEta0pt0mvVtx = edm::InputTag("analyze","PhoCone04NeutralHadronIsodR0dEta0pt0mvVtx");
    tPhoCone04NeutralHadronIsodR0dEta0pt0nocracks = edm::InputTag("analyze","PhoCone04NeutralHadronIsodR0dEta0pt0nocracks");
    tPhoCone04NeutralHadronIsodR0dEta0pt5 = edm::InputTag("analyze","PhoCone04NeutralHadronIsodR0dEta0pt5");
    tPhoCone04NeutralHadronIsodR0dEta0pt5nocracks = edm::InputTag("analyze","PhoCone04NeutralHadronIsodR0dEta0pt5nocracks");
    tPhoCone04NeutralHadronIsodR7dEta0pt0 = edm::InputTag("analyze","PhoCone04NeutralHadronIsodR7dEta0pt0");
    tPhoCone04NeutralHadronIsodR7dEta0pt5 = edm::InputTag("analyze","PhoCone04NeutralHadronIsodR7dEta0pt5");
    tPhoCone04NeutralHadronIsomvVtx = edm::InputTag("analyze","PhoCone04NeutralHadronIsomvVtx");
    tPhoCone04PFCombinedIso    = edm::InputTag("analyze","PhoCone04PFCombinedIso");
    tPhoCone04PhotonIsodEta015EBdR070EEmvVtx = edm::InputTag("analyze","PhoCone04PhotonIsodEta015EBdR070EEmvVtx");
    tPhoCone04PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx = edm::InputTag("analyze","PhoCone04PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx");
    tPhoCone04PhotonIsodR0dEta0pt0 = edm::InputTag("analyze","PhoCone04PhotonIsodR0dEta0pt0");
    tPhoCone04PhotonIsodR0dEta0pt5 = edm::InputTag("analyze","PhoCone04PhotonIsodR0dEta0pt5");
    tPhoCone04PhotonIsodR8dEta0pt0 = edm::InputTag("analyze","PhoCone04PhotonIsodR8dEta0pt0");
    tPhoCone04PhotonIsodR8dEta0pt5 = edm::InputTag("analyze","PhoCone04PhotonIsodR8dEta0pt5");
    tPhoConvChi2Probability    = edm::InputTag("analyze","PhoConvChi2Probability");
    tPhoConvEoverP             = edm::InputTag("analyze","PhoConvEoverP");
    tPhoConvNtracks            = edm::InputTag("analyze","PhoConvNtracks");
    tPhoConvValidVtx           = edm::InputTag("analyze","PhoConvValidVtx");
    tPhoE1OverE9               = edm::InputTag("analyze","PhoE1OverE9");
    tPhoE1x5                   = edm::InputTag("analyze","PhoE1x5");
    tPhoE2x5                   = edm::InputTag("analyze","PhoE2x5");
    tPhoE3x3                   = edm::InputTag("analyze","PhoE3x3");
    tPhoE5x5                   = edm::InputTag("analyze","PhoE5x5");
    tPhoEnergy                 = edm::InputTag("analyze","PhoEnergy");
    tPhoEta                    = edm::InputTag("analyze","PhoEta");
    tPhoGood                   = edm::InputTag("analyze","PhoGood");
    tPhoH1overE                = edm::InputTag("analyze","PhoH1overE");
    tPhoH2overE                = edm::InputTag("analyze","PhoH2overE");
    tPhoHCalIso2012ConeDR03    = edm::InputTag("analyze","PhoHCalIso2012ConeDR03");
    tPhoHasConvTrks            = edm::InputTag("analyze","PhoHasConvTrks");
    tPhoHasPixSeed             = edm::InputTag("analyze","PhoHasPixSeed");
    tPhoHoverE                 = edm::InputTag("analyze","PhoHoverE");
    tPhoHoverE2012             = edm::InputTag("analyze","PhoHoverE2012");
    tPhoIsIso                  = edm::InputTag("analyze","PhoIsIso");
    tPhoIso03                  = edm::InputTag("analyze","PhoIso03");
    tPhoIso03Ecal              = edm::InputTag("analyze","PhoIso03Ecal");
    tPhoIso03Hcal              = edm::InputTag("analyze","PhoIso03Hcal");
    tPhoIso03HcalDepth1        = edm::InputTag("analyze","PhoIso03HcalDepth1");
    tPhoIso03HcalDepth2        = edm::InputTag("analyze","PhoIso03HcalDepth2");
    tPhoIso03TrkHollow         = edm::InputTag("analyze","PhoIso03TrkHollow");
    tPhoIso03TrkSolid          = edm::InputTag("analyze","PhoIso03TrkSolid");
    tPhoIso03nTrksHollow       = edm::InputTag("analyze","PhoIso03nTrksHollow");
    tPhoIso03nTrksSolid        = edm::InputTag("analyze","PhoIso03nTrksSolid");
    tPhoIso04                  = edm::InputTag("analyze","PhoIso04");
    tPhoIso04Ecal              = edm::InputTag("analyze","PhoIso04Ecal");
    tPhoIso04Hcal              = edm::InputTag("analyze","PhoIso04Hcal");
    tPhoIso04HcalDepth1        = edm::InputTag("analyze","PhoIso04HcalDepth1");
    tPhoIso04HcalDepth2        = edm::InputTag("analyze","PhoIso04HcalDepth2");
    tPhoIso04TrkHollow         = edm::InputTag("analyze","PhoIso04TrkHollow");
    tPhoIso04TrkSolid          = edm::InputTag("analyze","PhoIso04TrkSolid");
    tPhoIso04nTrksHollow       = edm::InputTag("analyze","PhoIso04nTrksHollow");
    tPhoIso04nTrksSolid        = edm::InputTag("analyze","PhoIso04nTrksSolid");
    tPhoMCmatchexitcode        = edm::InputTag("analyze","PhoMCmatchexitcode");
    tPhoMCmatchindex           = edm::InputTag("analyze","PhoMCmatchindex");
    tPhoMatchedPFElectronCand  = edm::InputTag("analyze","PhoMatchedPFElectronCand");
    tPhoMatchedPFPhotonCand    = edm::InputTag("analyze","PhoMatchedPFPhotonCand");
    tPhoNeutralHadronIso       = edm::InputTag("analyze","PhoNeutralHadronIso");
    tPhoNewIsoPFCharged        = edm::InputTag("analyze","PhoNewIsoPFCharged");
    tPhoNewIsoPFNeutral        = edm::InputTag("analyze","PhoNewIsoPFNeutral");
    tPhoNewIsoPFPhoton         = edm::InputTag("analyze","PhoNewIsoPFPhoton");
    tPhoPassConversionVeto     = edm::InputTag("analyze","PhoPassConversionVeto");
    tPhoPhi                    = edm::InputTag("analyze","PhoPhi");
    tPhoPhotonIso              = edm::InputTag("analyze","PhoPhotonIso");
    tPhoPt                     = edm::InputTag("analyze","PhoPt");
    tPhoPx                     = edm::InputTag("analyze","PhoPx");
    tPhoPy                     = edm::InputTag("analyze","PhoPy");
    tPhoPz                     = edm::InputTag("analyze","PhoPz");
    tPhoR9                     = edm::InputTag("analyze","PhoR9");
    tPhoS4OverS1               = edm::InputTag("analyze","PhoS4OverS1");
    tPhoSCEtaWidth             = edm::InputTag("analyze","PhoSCEtaWidth");
    tPhoSCRawEnergy            = edm::InputTag("analyze","PhoSCRawEnergy");
    tPhoSCRemovalPFIsoCharged  = edm::InputTag("analyze","PhoSCRemovalPFIsoCharged");
    tPhoSCRemovalPFIsoNeutral  = edm::InputTag("analyze","PhoSCRemovalPFIsoNeutral");
    tPhoSCRemovalPFIsoPhoton   = edm::InputTag("analyze","PhoSCRemovalPFIsoPhoton");
    tPhoSCSigmaPhiPhi          = edm::InputTag("analyze","PhoSCSigmaPhiPhi");
    tPhoScSeedSeverity         = edm::InputTag("analyze","PhoScSeedSeverity");
    tPhoSigmaEtaEta            = edm::InputTag("analyze","PhoSigmaEtaEta");
    tPhoSigmaIetaIeta          = edm::InputTag("analyze","PhoSigmaIetaIeta");
    tPhoVx                     = edm::InputTag("analyze","PhoVx");
    tPhoVy                     = edm::InputTag("analyze","PhoVy");
    tPhoVz                     = edm::InputTag("analyze","PhoVz");
    tPhoisEB                   = edm::InputTag("analyze","PhoisEB");
    tPhoisEBEEGap              = edm::InputTag("analyze","PhoisEBEEGap");
    tPhoisEBEtaGap             = edm::InputTag("analyze","PhoisEBEtaGap");
    tPhoisEBPhiGap             = edm::InputTag("analyze","PhoisEBPhiGap");
    tPhoisEE                   = edm::InputTag("analyze","PhoisEE");
    tPhoisEEDeeGap             = edm::InputTag("analyze","PhoisEEDeeGap");
    tPhoisEERingGap            = edm::InputTag("analyze","PhoisEERingGap");
    tPhoisPFElectron           = edm::InputTag("analyze","PhoisPFElectron");
    tPhoisPFPhoton             = edm::InputTag("analyze","PhoisPFPhoton");
    tPhoisPFlowPhoton          = edm::InputTag("analyze","PhoisPFlowPhoton");
    tPhoisStandardPhoton       = edm::InputTag("analyze","PhoisStandardPhoton");
    tPhomaxEnergyXtal          = edm::InputTag("analyze","PhomaxEnergyXtal");
    tPhotSCindex               = edm::InputTag("analyze","PhotSCindex");
    tPrimVtxGood               = edm::InputTag("analyze","PrimVtxGood");
    tPrimVtxIsFake             = edm::InputTag("analyze","PrimVtxIsFake");
    tPrimVtxNChi2              = edm::InputTag("analyze","PrimVtxNChi2");
    tPrimVtxNdof               = edm::InputTag("analyze","PrimVtxNdof");
    tPrimVtxPtSum              = edm::InputTag("analyze","PrimVtxPtSum");
    tPrimVtxRho                = edm::InputTag("analyze","PrimVtxRho");
    tPrimVtxx                  = edm::InputTag("analyze","PrimVtxx");
    tPrimVtxxE                 = edm::InputTag("analyze","PrimVtxxE");
    tPrimVtxy                  = edm::InputTag("analyze","PrimVtxy");
    tPrimVtxyE                 = edm::InputTag("analyze","PrimVtxyE");
    tPrimVtxz                  = edm::InputTag("analyze","PrimVtxz");
    tPrimVtxzE                 = edm::InputTag("analyze","PrimVtxzE");
    tPromptnessLevel           = edm::InputTag("analyze","PromptnessLevel");
    tPtHat                     = edm::InputTag("analyze","PtHat");
    tQCDPartonicHT             = edm::InputTag("analyze","QCDPartonicHT");
    tRawMET                    = edm::InputTag("analyze","RawMET");
    tRawMETSignificance        = edm::InputTag("analyze","RawMETSignificance");
    tRawMETemEtFrac            = edm::InputTag("analyze","RawMETemEtFrac");
    tRawMETemEtInEB            = edm::InputTag("analyze","RawMETemEtInEB");
    tRawMETemEtInEE            = edm::InputTag("analyze","RawMETemEtInEE");
    tRawMETemEtInHF            = edm::InputTag("analyze","RawMETemEtInHF");
    tRawMEThadEtFrac           = edm::InputTag("analyze","RawMEThadEtFrac");
    tRawMEThadEtInHB           = edm::InputTag("analyze","RawMEThadEtInHB");
    tRawMEThadEtInHE           = edm::InputTag("analyze","RawMEThadEtInHE");
    tRawMEThadEtInHF           = edm::InputTag("analyze","RawMEThadEtInHF");
    tRawMETphi                 = edm::InputTag("analyze","RawMETphi");
    tRawMETpx                  = edm::InputTag("analyze","RawMETpx");
    tRawMETpy                  = edm::InputTag("analyze","RawMETpy");
    tRho                       = edm::InputTag("analyze","Rho");
    tRhoForIso                 = edm::InputTag("analyze","RhoForIso");
    tRun                       = edm::InputTag("analyze","Run");
    tSCBrem                    = edm::InputTag("analyze","SCBrem");
    tSCEnergy                  = edm::InputTag("analyze","SCEnergy");
    tSCEta                     = edm::InputTag("analyze","SCEta");
    tSCEtaWidth                = edm::InputTag("analyze","SCEtaWidth");
    tSCNXtals                  = edm::InputTag("analyze","SCNXtals");
    tSCPhi                     = edm::InputTag("analyze","SCPhi");
    tSCPhiWidth                = edm::InputTag("analyze","SCPhiWidth");
    tSCPre                     = edm::InputTag("analyze","SCPre");
    tSCR9                      = edm::InputTag("analyze","SCR9");
    tSCRaw                     = edm::InputTag("analyze","SCRaw");
    tSCX                       = edm::InputTag("analyze","SCX");
    tSCXtalListStart           = edm::InputTag("analyze","SCXtalListStart");
    tSCY                       = edm::InputTag("analyze","SCY");
    tSCZ                       = edm::InputTag("analyze","SCZ");
    tSCcrackcorr               = edm::InputTag("analyze","SCcrackcorr");
    tSCcrackcorrseed           = edm::InputTag("analyze","SCcrackcorrseed");
    tSCcrackcorrseedfactor     = edm::InputTag("analyze","SCcrackcorrseedfactor");
    tSClocalcorr               = edm::InputTag("analyze","SClocalcorr");
    tSClocalcorrseed           = edm::InputTag("analyze","SClocalcorrseed");
    tSClocalcorrseedfactor     = edm::InputTag("analyze","SClocalcorrseedfactor");
    tSigProcID                 = edm::InputTag("analyze","SigProcID");
    tSigma                     = edm::InputTag("analyze","Sigma");
    tSumEt                     = edm::InputTag("analyze","SumEt");
    tTCMET                     = edm::InputTag("analyze","TCMET");
    tTCMETSignificance         = edm::InputTag("analyze","TCMETSignificance");
    tTCMETphi                  = edm::InputTag("analyze","TCMETphi");
    tTCMETpx                   = edm::InputTag("analyze","TCMETpx");
    tTCMETpy                   = edm::InputTag("analyze","TCMETpy");
    tTauCharge                 = edm::InputTag("analyze","TauCharge");
    tTauChargedHadronIso       = edm::InputTag("analyze","TauChargedHadronIso");
    tTauDecayMode              = edm::InputTag("analyze","TauDecayMode");
    tTauDecayModeFinding       = edm::InputTag("analyze","TauDecayModeFinding");
    tTauE                      = edm::InputTag("analyze","TauE");
    tTauElectronMVARejection   = edm::InputTag("analyze","TauElectronMVARejection");
    tTauEmFraction             = edm::InputTag("analyze","TauEmFraction");
    tTauEt                     = edm::InputTag("analyze","TauEt");
    tTauEta                    = edm::InputTag("analyze","TauEta");
    tTauIsPFTau                = edm::InputTag("analyze","TauIsPFTau");
    tTauIsolationMVA2raw       = edm::InputTag("analyze","TauIsolationMVA2raw");
    tTauIsolationMVAraw        = edm::InputTag("analyze","TauIsolationMVAraw");
    tTauJetEta                 = edm::InputTag("analyze","TauJetEta");
    tTauJetMass                = edm::InputTag("analyze","TauJetMass");
    tTauJetPhi                 = edm::InputTag("analyze","TauJetPhi");
    tTauJetPt                  = edm::InputTag("analyze","TauJetPt");
    tTauLeadingNeuPt           = edm::InputTag("analyze","TauLeadingNeuPt");
    tTauLeadingTkEcalenergy    = edm::InputTag("analyze","TauLeadingTkEcalenergy");
    tTauLeadingTkHcalenergy    = edm::InputTag("analyze","TauLeadingTkHcalenergy");
    tTauLeadingTkPt            = edm::InputTag("analyze","TauLeadingTkPt");
    tTauLooseChargedIso        = edm::InputTag("analyze","TauLooseChargedIso");
    tTauLooseCombinedIsoDBSumPtCorr = edm::InputTag("analyze","TauLooseCombinedIsoDBSumPtCorr");
    tTauLooseCombinedIsoDBSumPtCorr3Hits = edm::InputTag("analyze","TauLooseCombinedIsoDBSumPtCorr3Hits");
    tTauLooseElectronMVA3Rejection = edm::InputTag("analyze","TauLooseElectronMVA3Rejection");
    tTauLooseElectronRejection = edm::InputTag("analyze","TauLooseElectronRejection");
    tTauLooseIso               = edm::InputTag("analyze","TauLooseIso");
    tTauLooseIsoDBSumPtCorr    = edm::InputTag("analyze","TauLooseIsoDBSumPtCorr");
    tTauLooseIsolationMVA      = edm::InputTag("analyze","TauLooseIsolationMVA");
    tTauLooseIsolationMVA2     = edm::InputTag("analyze","TauLooseIsolationMVA2");
    tTauLooseMuon2Rejection    = edm::InputTag("analyze","TauLooseMuon2Rejection");
    tTauLooseMuonRejection     = edm::InputTag("analyze","TauLooseMuonRejection");
    tTauMaxLepExc              = edm::InputTag("analyze","TauMaxLepExc");
    tTauMediumChargedIso       = edm::InputTag("analyze","TauMediumChargedIso");
    tTauMediumCombinedIsoDBSumPtCorr = edm::InputTag("analyze","TauMediumCombinedIsoDBSumPtCorr");
    tTauMediumCombinedIsoDBSumPtCorr3Hits = edm::InputTag("analyze","TauMediumCombinedIsoDBSumPtCorr3Hits");
    tTauMediumElectronMVA3Rejection = edm::InputTag("analyze","TauMediumElectronMVA3Rejection");
    tTauMediumElectronRejection = edm::InputTag("analyze","TauMediumElectronRejection");
    tTauMediumIso              = edm::InputTag("analyze","TauMediumIso");
    tTauMediumIsoDBSumPtCorr   = edm::InputTag("analyze","TauMediumIsoDBSumPtCorr");
    tTauMediumIsolationMVA     = edm::InputTag("analyze","TauMediumIsolationMVA");
    tTauMediumIsolationMVA2    = edm::InputTag("analyze","TauMediumIsolationMVA2");
    tTauMediumMuon2Rejection   = edm::InputTag("analyze","TauMediumMuon2Rejection");
    tTauMediumMuonRejection    = edm::InputTag("analyze","TauMediumMuonRejection");
    tTauNObjs                  = edm::InputTag("analyze","TauNObjs");
    tTauNObjsTot               = edm::InputTag("analyze","TauNObjsTot");
    tTauNeutralHadronIso       = edm::InputTag("analyze","TauNeutralHadronIso");
    tTauNumChargedHadronsIsoCone = edm::InputTag("analyze","TauNumChargedHadronsIsoCone");
    tTauNumChargedHadronsSignalCone = edm::InputTag("analyze","TauNumChargedHadronsSignalCone");
    tTauNumNeutralHadronsIsoCone = edm::InputTag("analyze","TauNumNeutralHadronsIsoCone");
    tTauNumNeutralHadronsSignalCone = edm::InputTag("analyze","TauNumNeutralHadronsSignalCone");
    tTauNumParticlesIsolationCone = edm::InputTag("analyze","TauNumParticlesIsolationCone");
    tTauNumParticlesSignalCone = edm::InputTag("analyze","TauNumParticlesSignalCone");
    tTauNumPhotonsIsolationCone = edm::InputTag("analyze","TauNumPhotonsIsolationCone");
    tTauNumPhotonsSignalCone   = edm::InputTag("analyze","TauNumPhotonsSignalCone");
    tTauParticleIso            = edm::InputTag("analyze","TauParticleIso");
    tTauPhi                    = edm::InputTag("analyze","TauPhi");
    tTauPhotonIso              = edm::InputTag("analyze","TauPhotonIso");
    tTauPt                     = edm::InputTag("analyze","TauPt");
    tTauPtSumChargedParticlesIsoCone = edm::InputTag("analyze","TauPtSumChargedParticlesIsoCone");
    tTauPtSumPhotonsIsoCone    = edm::InputTag("analyze","TauPtSumPhotonsIsoCone");
    tTauPx                     = edm::InputTag("analyze","TauPx");
    tTauPy                     = edm::InputTag("analyze","TauPy");
    tTauPz                     = edm::InputTag("analyze","TauPz");
    tTauTightChargedIso        = edm::InputTag("analyze","TauTightChargedIso");
    tTauTightCombinedIsoDBSumPtCorr = edm::InputTag("analyze","TauTightCombinedIsoDBSumPtCorr");
    tTauTightCombinedIsoDBSumPtCorr3Hits = edm::InputTag("analyze","TauTightCombinedIsoDBSumPtCorr3Hits");
    tTauTightElectronMVA3Rejection = edm::InputTag("analyze","TauTightElectronMVA3Rejection");
    tTauTightElectronRejection = edm::InputTag("analyze","TauTightElectronRejection");
    tTauTightIso               = edm::InputTag("analyze","TauTightIso");
    tTauTightIsoDBSumPtCorr    = edm::InputTag("analyze","TauTightIsoDBSumPtCorr");
    tTauTightIsolationMVA      = edm::InputTag("analyze","TauTightIsolationMVA");
    tTauTightIsolationMVA2     = edm::InputTag("analyze","TauTightIsolationMVA2");
    tTauTightMuon2Rejection    = edm::InputTag("analyze","TauTightMuon2Rejection");
    tTauTightMuonRejection     = edm::InputTag("analyze","TauTightMuonRejection");
    tTauVLooseChargedIso       = edm::InputTag("analyze","TauVLooseChargedIso");
    tTauVLooseCombinedIsoDBSumPtCorr = edm::InputTag("analyze","TauVLooseCombinedIsoDBSumPtCorr");
    tTauVLooseIso              = edm::InputTag("analyze","TauVLooseIso");
    tTauVLooseIsoDBSumPtCorr   = edm::InputTag("analyze","TauVLooseIsoDBSumPtCorr");
    tTauVTightElectronMVA3Rejection = edm::InputTag("analyze","TauVTightElectronMVA3Rejection");
    tTauVz                     = edm::InputTag("analyze","TauVz");
    tTrkEta                    = edm::InputTag("analyze","TrkEta");
    tTrkGood                   = edm::InputTag("analyze","TrkGood");
    tTrkNChi2                  = edm::InputTag("analyze","TrkNChi2");
    tTrkNHits                  = edm::InputTag("analyze","TrkNHits");
    tTrkPhi                    = edm::InputTag("analyze","TrkPhi");
    tTrkPt                     = edm::InputTag("analyze","TrkPt");
    tTrkPtSum                  = edm::InputTag("analyze","TrkPtSum");
    tTrkPtSumPhi               = edm::InputTag("analyze","TrkPtSumPhi");
    tTrkPtSumx                 = edm::InputTag("analyze","TrkPtSumx");
    tTrkPtSumy                 = edm::InputTag("analyze","TrkPtSumy");
    tTrkVtxDxy                 = edm::InputTag("analyze","TrkVtxDxy");
    tTrkVtxDz                  = edm::InputTag("analyze","TrkVtxDz");
    tVrtxChi2                  = edm::InputTag("analyze","VrtxChi2");
    tVrtxIsFake                = edm::InputTag("analyze","VrtxIsFake");
    tVrtxNdof                  = edm::InputTag("analyze","VrtxNdof");
    tVrtxNtrks                 = edm::InputTag("analyze","VrtxNtrks");
    tVrtxSumPt                 = edm::InputTag("analyze","VrtxSumPt");
    tVrtxX                     = edm::InputTag("analyze","VrtxX");
    tVrtxXE                    = edm::InputTag("analyze","VrtxXE");
    tVrtxY                     = edm::InputTag("analyze","VrtxY");
    tVrtxYE                    = edm::InputTag("analyze","VrtxYE");
    tVrtxZ                     = edm::InputTag("analyze","VrtxZ");
    tVrtxZE                    = edm::InputTag("analyze","VrtxZE");
    tVtxdiphoh2gglobe          = edm::InputTag("analyze","Vtxdiphoh2gglobe");
    tVtxdiphomva               = edm::InputTag("analyze","Vtxdiphomva");
    tVtxdiphoproductrank       = edm::InputTag("analyze","Vtxdiphoproductrank");
    tWeight                    = edm::InputTag("analyze","Weight");
    tXtalEtaWidth              = edm::InputTag("analyze","XtalEtaWidth");
    tXtalFront1X               = edm::InputTag("analyze","XtalFront1X");
    tXtalFront1Y               = edm::InputTag("analyze","XtalFront1Y");
    tXtalFront1Z               = edm::InputTag("analyze","XtalFront1Z");
    tXtalFront2X               = edm::InputTag("analyze","XtalFront2X");
    tXtalFront2Y               = edm::InputTag("analyze","XtalFront2Y");
    tXtalFront2Z               = edm::InputTag("analyze","XtalFront2Z");
    tXtalFront3X               = edm::InputTag("analyze","XtalFront3X");
    tXtalFront3Y               = edm::InputTag("analyze","XtalFront3Y");
    tXtalFront3Z               = edm::InputTag("analyze","XtalFront3Z");
    tXtalFront4X               = edm::InputTag("analyze","XtalFront4X");
    tXtalFront4Y               = edm::InputTag("analyze","XtalFront4Y");
    tXtalFront4Z               = edm::InputTag("analyze","XtalFront4Z");
    tXtalPhiWidth              = edm::InputTag("analyze","XtalPhiWidth");
    tXtalX                     = edm::InputTag("analyze","XtalX");
    tXtalY                     = edm::InputTag("analyze","XtalY");
    tXtalZ                     = edm::InputTag("analyze","XtalZ");
    tgenInfoEta                = edm::InputTag("analyze","genInfoEta");
    tgenInfoId                 = edm::InputTag("analyze","genInfoId");
    tgenInfoM                  = edm::InputTag("analyze","genInfoM");
    tgenInfoMo1                = edm::InputTag("analyze","genInfoMo1");
    tgenInfoMo2                = edm::InputTag("analyze","genInfoMo2");
    tgenInfoNMo                = edm::InputTag("analyze","genInfoNMo");
    tgenInfoPhi                = edm::InputTag("analyze","genInfoPhi");
    tgenInfoPromptFlag         = edm::InputTag("analyze","genInfoPromptFlag");
    tgenInfoPt                 = edm::InputTag("analyze","genInfoPt");
    tgenInfoPx                 = edm::InputTag("analyze","genInfoPx");
    tgenInfoPy                 = edm::InputTag("analyze","genInfoPy");
    tgenInfoPz                 = edm::InputTag("analyze","genInfoPz");
    tgenInfoStatus             = edm::InputTag("analyze","genInfoStatus");
    tgvNTkHi                   = edm::InputTag("analyze","gvNTkHi");
    tgvNTkLo                   = edm::InputTag("analyze","gvNTkLo");
    tgvSumPtHi                 = edm::InputTag("analyze","gvSumPtHi");
    tgvSumPtLo                 = edm::InputTag("analyze","gvSumPtLo");
    tnGenParticles             = edm::InputTag("analyze","nGenParticles");
    tpdfW                      = edm::InputTag("analyze","pdfW");
    tpdfWsum                   = edm::InputTag("analyze","pdfWsum");
    tprocess                   = edm::InputTag("analyze","process");
    tsignMu                    = edm::InputTag("analyze","signMu");
    txSMS                      = edm::InputTag("analyze","xSMS");
    txbarSMS                   = edm::InputTag("analyze","xbarSMS");


    


}


NTupleSelector::~NTupleSelector()
{
 
   // do anything here that needs to be done at desctruction time
   // (e.g. close files, deallocate resources etc.)

}


//
// member functions
//

// ------------ method called on each new Event  ------------
bool
NTupleSelector::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{

  bool result = true;


    // Get all event handles and assign to members
    result &= iEvent.getByLabel( tEcalDeadCellTriggerPrimitiveFilter, hEcalDeadCellTriggerPrimitiveFilter );
    if ( hEcalDeadCellTriggerPrimitiveFilter.isValid() ) EcalDeadCellTriggerPrimitiveFilter = *hEcalDeadCellTriggerPrimitiveFilter;
    result &= iEvent.getByLabel( tecalLaserCorrFilter, hecalLaserCorrFilter );
    if ( hecalLaserCorrFilter.isValid() ) ecalLaserCorrFilter = *hecalLaserCorrFilter;
    result &= iEvent.getByLabel( teeBadScFilter, heeBadScFilter );
    if ( heeBadScFilter.isValid() ) eeBadScFilter = *heeBadScFilter;
    result &= iEvent.getByLabel( thcalLaserEventFilter, hhcalLaserEventFilter );
    if ( hhcalLaserEventFilter.isValid() ) hcalLaserEventFilter = *hhcalLaserEventFilter;
    result &= iEvent.getByLabel( tlogErrorTooManyClusters, hlogErrorTooManyClusters );
    if ( hlogErrorTooManyClusters.isValid() ) logErrorTooManyClusters = *hlogErrorTooManyClusters;
    result &= iEvent.getByLabel( tmanystripclus53X, hmanystripclus53X );
    if ( hmanystripclus53X.isValid() ) manystripclus53X = *hmanystripclus53X;
    result &= iEvent.getByLabel( ttoomanystripclus53X, htoomanystripclus53X );
    if ( htoomanystripclus53X.isValid() ) toomanystripclus53X = *htoomanystripclus53X;
    result &= iEvent.getByLabel( ttrackingFailureFilter, htrackingFailureFilter );
    if ( htrackingFailureFilter.isValid() ) trackingFailureFilter = *htrackingFailureFilter;
    result &= iEvent.getByLabel( tA0, hA0 );
    if ( hA0.isValid() ) A0 = *hA0;
    result &= iEvent.getByLabel( tBeamspotx, hBeamspotx );
    if ( hBeamspotx.isValid() ) Beamspotx = *hBeamspotx;
    result &= iEvent.getByLabel( tBeamspoty, hBeamspoty );
    if ( hBeamspoty.isValid() ) Beamspoty = *hBeamspoty;
    result &= iEvent.getByLabel( tBeamspotz, hBeamspotz );
    if ( hBeamspotz.isValid() ) Beamspotz = *hBeamspotz;
    result &= iEvent.getByLabel( tCAJArea, hCAJArea );
    if ( hCAJArea.isValid() ) CAJArea = *hCAJArea;
    result &= iEvent.getByLabel( tCAJChfrac, hCAJChfrac );
    if ( hCAJChfrac.isValid() ) CAJChfrac = *hCAJChfrac;
    result &= iEvent.getByLabel( tCAJE, hCAJE );
    if ( hCAJE.isValid() ) CAJE = *hCAJE;
    result &= iEvent.getByLabel( tCAJEMfrac, hCAJEMfrac );
    if ( hCAJEMfrac.isValid() ) CAJEMfrac = *hCAJEMfrac;
    result &= iEvent.getByLabel( tCAJEt, hCAJEt );
    if ( hCAJEt.isValid() ) CAJEt = *hCAJEt;
    result &= iEvent.getByLabel( tCAJEta, hCAJEta );
    if ( hCAJEta.isValid() ) CAJEta = *hCAJEta;
    result &= iEvent.getByLabel( tCAJFlavour, hCAJFlavour );
    if ( hCAJFlavour.isValid() ) CAJFlavour = *hCAJFlavour;
    result &= iEvent.getByLabel( tCAJIDHPD, hCAJIDHPD );
    if ( hCAJIDHPD.isValid() ) CAJIDHPD = *hCAJIDHPD;
    result &= iEvent.getByLabel( tCAJIDLoose, hCAJIDLoose );
    if ( hCAJIDLoose.isValid() ) CAJIDLoose = *hCAJIDLoose;
    result &= iEvent.getByLabel( tCAJIDRBX, hCAJIDRBX );
    if ( hCAJIDRBX.isValid() ) CAJIDRBX = *hCAJIDRBX;
    result &= iEvent.getByLabel( tCAJIDn90Hits, hCAJIDn90Hits );
    if ( hCAJIDn90Hits.isValid() ) CAJIDn90Hits = *hCAJIDn90Hits;
    result &= iEvent.getByLabel( tCAJIDresEMF, hCAJIDresEMF );
    if ( hCAJIDresEMF.isValid() ) CAJIDresEMF = *hCAJIDresEMF;
    result &= iEvent.getByLabel( tCAJL1FastJetScale, hCAJL1FastJetScale );
    if ( hCAJL1FastJetScale.isValid() ) CAJL1FastJetScale = *hCAJL1FastJetScale;
    result &= iEvent.getByLabel( tCAJNAssoTracks, hCAJNAssoTracks );
    if ( hCAJNAssoTracks.isValid() ) CAJNAssoTracks = *hCAJNAssoTracks;
    result &= iEvent.getByLabel( tCAJNConstituents, hCAJNConstituents );
    if ( hCAJNConstituents.isValid() ) CAJNConstituents = *hCAJNConstituents;
    result &= iEvent.getByLabel( tCAJPhi, hCAJPhi );
    if ( hCAJPhi.isValid() ) CAJPhi = *hCAJPhi;
    result &= iEvent.getByLabel( tCAJPt, hCAJPt );
    if ( hCAJPt.isValid() ) CAJPt = *hCAJPt;
    result &= iEvent.getByLabel( tCAJPx, hCAJPx );
    if ( hCAJPx.isValid() ) CAJPx = *hCAJPx;
    result &= iEvent.getByLabel( tCAJPy, hCAJPy );
    if ( hCAJPy.isValid() ) CAJPy = *hCAJPy;
    result &= iEvent.getByLabel( tCAJPz, hCAJPz );
    if ( hCAJPz.isValid() ) CAJPz = *hCAJPz;
    result &= iEvent.getByLabel( tCAJScale, hCAJScale );
    if ( hCAJScale.isValid() ) CAJScale = *hCAJScale;
    result &= iEvent.getByLabel( tCAJn90, hCAJn90 );
    if ( hCAJn90.isValid() ) CAJn90 = *hCAJn90;
    result &= iEvent.getByLabel( tCANJets, hCANJets );
    if ( hCANJets.isValid() ) CANJets = *hCANJets;
    result &= iEvent.getByLabel( tCSCTightHaloID, hCSCTightHaloID );
    if ( hCSCTightHaloID.isValid() ) CSCTightHaloID = *hCSCTightHaloID;
    result &= iEvent.getByLabel( tConvChi2Probability, hConvChi2Probability );
    if ( hConvChi2Probability.isValid() ) ConvChi2Probability = *hConvChi2Probability;
    result &= iEvent.getByLabel( tConvEoverP, hConvEoverP );
    if ( hConvEoverP.isValid() ) ConvEoverP = *hConvEoverP;
    result &= iEvent.getByLabel( tConvNtracks, hConvNtracks );
    if ( hConvNtracks.isValid() ) ConvNtracks = *hConvNtracks;
    result &= iEvent.getByLabel( tConvValidVtx, hConvValidVtx );
    if ( hConvValidVtx.isValid() ) ConvValidVtx = *hConvValidVtx;
    result &= iEvent.getByLabel( tConvZofPrimVtxFromTrks, hConvZofPrimVtxFromTrks );
    if ( hConvZofPrimVtxFromTrks.isValid() ) ConvZofPrimVtxFromTrks = *hConvZofPrimVtxFromTrks;
    result &= iEvent.getByLabel( tDiphotonsfirst, hDiphotonsfirst );
    if ( hDiphotonsfirst.isValid() ) Diphotonsfirst = *hDiphotonsfirst;
    result &= iEvent.getByLabel( tDiphotonssecond, hDiphotonssecond );
    if ( hDiphotonssecond.isValid() ) Diphotonssecond = *hDiphotonssecond;
    result &= iEvent.getByLabel( tEBrechitChi2, hEBrechitChi2 );
    if ( hEBrechitChi2.isValid() ) EBrechitChi2 = *hEBrechitChi2;
    result &= iEvent.getByLabel( tEBrechitE, hEBrechitE );
    if ( hEBrechitE.isValid() ) EBrechitE = *hEBrechitE;
    result &= iEvent.getByLabel( tEBrechitE2oE9, hEBrechitE2oE9 );
    if ( hEBrechitE2oE9.isValid() ) EBrechitE2oE9 = *hEBrechitE2oE9;
    result &= iEvent.getByLabel( tEBrechitE4oE1, hEBrechitE4oE1 );
    if ( hEBrechitE4oE1.isValid() ) EBrechitE4oE1 = *hEBrechitE4oE1;
    result &= iEvent.getByLabel( tEBrechitEta, hEBrechitEta );
    if ( hEBrechitEta.isValid() ) EBrechitEta = *hEBrechitEta;
    result &= iEvent.getByLabel( tEBrechitPhi, hEBrechitPhi );
    if ( hEBrechitPhi.isValid() ) EBrechitPhi = *hEBrechitPhi;
    result &= iEvent.getByLabel( tEBrechitPt, hEBrechitPt );
    if ( hEBrechitPt.isValid() ) EBrechitPt = *hEBrechitPt;
    result &= iEvent.getByLabel( tEBrechitTime, hEBrechitTime );
    if ( hEBrechitTime.isValid() ) EBrechitTime = *hEBrechitTime;
    result &= iEvent.getByLabel( tECALEsumx, hECALEsumx );
    if ( hECALEsumx.isValid() ) ECALEsumx = *hECALEsumx;
    result &= iEvent.getByLabel( tECALEsumy, hECALEsumy );
    if ( hECALEsumy.isValid() ) ECALEsumy = *hECALEsumy;
    result &= iEvent.getByLabel( tECALEsumz, hECALEsumz );
    if ( hECALEsumz.isValid() ) ECALEsumz = *hECALEsumz;
    result &= iEvent.getByLabel( tECALMET, hECALMET );
    if ( hECALMET.isValid() ) ECALMET = *hECALMET;
    result &= iEvent.getByLabel( tECALMETEta, hECALMETEta );
    if ( hECALMETEta.isValid() ) ECALMETEta = *hECALMETEta;
    result &= iEvent.getByLabel( tECALMETPhi, hECALMETPhi );
    if ( hECALMETPhi.isValid() ) ECALMETPhi = *hECALMETPhi;
    result &= iEvent.getByLabel( tECALSumEt, hECALSumEt );
    if ( hECALSumEt.isValid() ) ECALSumEt = *hECALSumEt;
    result &= iEvent.getByLabel( tElBasicClustersSize, hElBasicClustersSize );
    if ( hElBasicClustersSize.isValid() ) ElBasicClustersSize = *hElBasicClustersSize;
    result &= iEvent.getByLabel( tElCInfoIsGsfCtfCons, hElCInfoIsGsfCtfCons );
    if ( hElCInfoIsGsfCtfCons.isValid() ) ElCInfoIsGsfCtfCons = *hElCInfoIsGsfCtfCons;
    result &= iEvent.getByLabel( tElCInfoIsGsfCtfScPixCons, hElCInfoIsGsfCtfScPixCons );
    if ( hElCInfoIsGsfCtfScPixCons.isValid() ) ElCInfoIsGsfCtfScPixCons = *hElCInfoIsGsfCtfScPixCons;
    result &= iEvent.getByLabel( tElCInfoIsGsfScPixCons, hElCInfoIsGsfScPixCons );
    if ( hElCInfoIsGsfScPixCons.isValid() ) ElCInfoIsGsfScPixCons = *hElCInfoIsGsfScPixCons;
    result &= iEvent.getByLabel( tElCaloEnergy, hElCaloEnergy );
    if ( hElCaloEnergy.isValid() ) ElCaloEnergy = *hElCaloEnergy;
    result &= iEvent.getByLabel( tElCharge, hElCharge );
    if ( hElCharge.isValid() ) ElCharge = *hElCharge;
    result &= iEvent.getByLabel( tElChargeMisIDProb, hElChargeMisIDProb );
    if ( hElChargeMisIDProb.isValid() ) ElChargeMisIDProb = *hElChargeMisIDProb;
    result &= iEvent.getByLabel( tElClosestCtfTrackCharge, hElClosestCtfTrackCharge );
    if ( hElClosestCtfTrackCharge.isValid() ) ElClosestCtfTrackCharge = *hElClosestCtfTrackCharge;
    result &= iEvent.getByLabel( tElClosestCtfTrackEta, hElClosestCtfTrackEta );
    if ( hElClosestCtfTrackEta.isValid() ) ElClosestCtfTrackEta = *hElClosestCtfTrackEta;
    result &= iEvent.getByLabel( tElClosestCtfTrackPhi, hElClosestCtfTrackPhi );
    if ( hElClosestCtfTrackPhi.isValid() ) ElClosestCtfTrackPhi = *hElClosestCtfTrackPhi;
    result &= iEvent.getByLabel( tElClosestCtfTrackPt, hElClosestCtfTrackPt );
    if ( hElClosestCtfTrackPt.isValid() ) ElClosestCtfTrackPt = *hElClosestCtfTrackPt;
    result &= iEvent.getByLabel( tElConvPartnerTrkCharge, hElConvPartnerTrkCharge );
    if ( hElConvPartnerTrkCharge.isValid() ) ElConvPartnerTrkCharge = *hElConvPartnerTrkCharge;
    result &= iEvent.getByLabel( tElConvPartnerTrkDCot, hElConvPartnerTrkDCot );
    if ( hElConvPartnerTrkDCot.isValid() ) ElConvPartnerTrkDCot = *hElConvPartnerTrkDCot;
    result &= iEvent.getByLabel( tElConvPartnerTrkDist, hElConvPartnerTrkDist );
    if ( hElConvPartnerTrkDist.isValid() ) ElConvPartnerTrkDist = *hElConvPartnerTrkDist;
    result &= iEvent.getByLabel( tElConvPartnerTrkEta, hElConvPartnerTrkEta );
    if ( hElConvPartnerTrkEta.isValid() ) ElConvPartnerTrkEta = *hElConvPartnerTrkEta;
    result &= iEvent.getByLabel( tElConvPartnerTrkPhi, hElConvPartnerTrkPhi );
    if ( hElConvPartnerTrkPhi.isValid() ) ElConvPartnerTrkPhi = *hElConvPartnerTrkPhi;
    result &= iEvent.getByLabel( tElConvPartnerTrkPt, hElConvPartnerTrkPt );
    if ( hElConvPartnerTrkPt.isValid() ) ElConvPartnerTrkPt = *hElConvPartnerTrkPt;
    result &= iEvent.getByLabel( tElD03DE, hElD03DE );
    if ( hElD03DE.isValid() ) ElD03DE = *hElD03DE;
    result &= iEvent.getByLabel( tElD03DPV, hElD03DPV );
    if ( hElD03DPV.isValid() ) ElD03DPV = *hElD03DPV;
    result &= iEvent.getByLabel( tElD0BS, hElD0BS );
    if ( hElD0BS.isValid() ) ElD0BS = *hElD0BS;
    result &= iEvent.getByLabel( tElD0E, hElD0E );
    if ( hElD0E.isValid() ) ElD0E = *hElD0E;
    result &= iEvent.getByLabel( tElD0PV, hElD0PV );
    if ( hElD0PV.isValid() ) ElD0PV = *hElD0PV;
    result &= iEvent.getByLabel( tElDR03EcalRecHitSumEt, hElDR03EcalRecHitSumEt );
    if ( hElDR03EcalRecHitSumEt.isValid() ) ElDR03EcalRecHitSumEt = *hElDR03EcalRecHitSumEt;
    result &= iEvent.getByLabel( tElDR03HcalTowerSumEt, hElDR03HcalTowerSumEt );
    if ( hElDR03HcalTowerSumEt.isValid() ) ElDR03HcalTowerSumEt = *hElDR03HcalTowerSumEt;
    result &= iEvent.getByLabel( tElDR03TkSumPt, hElDR03TkSumPt );
    if ( hElDR03TkSumPt.isValid() ) ElDR03TkSumPt = *hElDR03TkSumPt;
    result &= iEvent.getByLabel( tElDR04EcalRecHitSumEt, hElDR04EcalRecHitSumEt );
    if ( hElDR04EcalRecHitSumEt.isValid() ) ElDR04EcalRecHitSumEt = *hElDR04EcalRecHitSumEt;
    result &= iEvent.getByLabel( tElDR04HcalTowerSumEt, hElDR04HcalTowerSumEt );
    if ( hElDR04HcalTowerSumEt.isValid() ) ElDR04HcalTowerSumEt = *hElDR04HcalTowerSumEt;
    result &= iEvent.getByLabel( tElDR04TkSumPt, hElDR04TkSumPt );
    if ( hElDR04TkSumPt.isValid() ) ElDR04TkSumPt = *hElDR04TkSumPt;
    result &= iEvent.getByLabel( tElDeltaEtaSeedClusterAtCalo, hElDeltaEtaSeedClusterAtCalo );
    if ( hElDeltaEtaSeedClusterAtCalo.isValid() ) ElDeltaEtaSeedClusterAtCalo = *hElDeltaEtaSeedClusterAtCalo;
    result &= iEvent.getByLabel( tElDeltaEtaSuperClusterAtVtx, hElDeltaEtaSuperClusterAtVtx );
    if ( hElDeltaEtaSuperClusterAtVtx.isValid() ) ElDeltaEtaSuperClusterAtVtx = *hElDeltaEtaSuperClusterAtVtx;
    result &= iEvent.getByLabel( tElDeltaPhiSeedClusterAtCalo, hElDeltaPhiSeedClusterAtCalo );
    if ( hElDeltaPhiSeedClusterAtCalo.isValid() ) ElDeltaPhiSeedClusterAtCalo = *hElDeltaPhiSeedClusterAtCalo;
    result &= iEvent.getByLabel( tElDeltaPhiSuperClusterAtVtx, hElDeltaPhiSuperClusterAtVtx );
    if ( hElDeltaPhiSuperClusterAtVtx.isValid() ) ElDeltaPhiSuperClusterAtVtx = *hElDeltaPhiSuperClusterAtVtx;
    result &= iEvent.getByLabel( tElDzBS, hElDzBS );
    if ( hElDzBS.isValid() ) ElDzBS = *hElDzBS;
    result &= iEvent.getByLabel( tElDzE, hElDzE );
    if ( hElDzE.isValid() ) ElDzE = *hElDzE;
    result &= iEvent.getByLabel( tElDzPV, hElDzPV );
    if ( hElDzPV.isValid() ) ElDzPV = *hElDzPV;
    result &= iEvent.getByLabel( tElE, hElE );
    if ( hElE.isValid() ) ElE = *hElE;
    result &= iEvent.getByLabel( tElE1OverE9, hElE1OverE9 );
    if ( hElE1OverE9.isValid() ) ElE1OverE9 = *hElE1OverE9;
    result &= iEvent.getByLabel( tElE1x5, hElE1x5 );
    if ( hElE1x5.isValid() ) ElE1x5 = *hElE1x5;
    result &= iEvent.getByLabel( tElE2x5Max, hElE2x5Max );
    if ( hElE2x5Max.isValid() ) ElE2x5Max = *hElE2x5Max;
    result &= iEvent.getByLabel( tElE5x5, hElE5x5 );
    if ( hElE5x5.isValid() ) ElE5x5 = *hElE5x5;
    result &= iEvent.getByLabel( tElESuperClusterOverP, hElESuperClusterOverP );
    if ( hElESuperClusterOverP.isValid() ) ElESuperClusterOverP = *hElESuperClusterOverP;
    result &= iEvent.getByLabel( tElEcalDriven, hElEcalDriven );
    if ( hElEcalDriven.isValid() ) ElEcalDriven = *hElEcalDriven;
    result &= iEvent.getByLabel( tElEcalEnergyError, hElEcalEnergyError );
    if ( hElEcalEnergyError.isValid() ) ElEcalEnergyError = *hElEcalEnergyError;
    result &= iEvent.getByLabel( tElEleMomentumError, hElEleMomentumError );
    if ( hElEleMomentumError.isValid() ) ElEleMomentumError = *hElEleMomentumError;
    result &= iEvent.getByLabel( tElEoPout, hElEoPout );
    if ( hElEoPout.isValid() ) ElEoPout = *hElEoPout;
    result &= iEvent.getByLabel( tElEt, hElEt );
    if ( hElEt.isValid() ) ElEt = *hElEt;
    result &= iEvent.getByLabel( tElEta, hElEta );
    if ( hElEta.isValid() ) ElEta = *hElEta;
    result &= iEvent.getByLabel( tElEventelPFIsoValueCharged03PFIdStandard, hElEventelPFIsoValueCharged03PFIdStandard );
    if ( hElEventelPFIsoValueCharged03PFIdStandard.isValid() ) ElEventelPFIsoValueCharged03PFIdStandard = *hElEventelPFIsoValueCharged03PFIdStandard;
    result &= iEvent.getByLabel( tElEventelPFIsoValueCharged04PFIdStandard, hElEventelPFIsoValueCharged04PFIdStandard );
    if ( hElEventelPFIsoValueCharged04PFIdStandard.isValid() ) ElEventelPFIsoValueCharged04PFIdStandard = *hElEventelPFIsoValueCharged04PFIdStandard;
    result &= iEvent.getByLabel( tElEventelPFIsoValueGamma03PFIdStandard, hElEventelPFIsoValueGamma03PFIdStandard );
    if ( hElEventelPFIsoValueGamma03PFIdStandard.isValid() ) ElEventelPFIsoValueGamma03PFIdStandard = *hElEventelPFIsoValueGamma03PFIdStandard;
    result &= iEvent.getByLabel( tElEventelPFIsoValueGamma04PFIdStandard, hElEventelPFIsoValueGamma04PFIdStandard );
    if ( hElEventelPFIsoValueGamma04PFIdStandard.isValid() ) ElEventelPFIsoValueGamma04PFIdStandard = *hElEventelPFIsoValueGamma04PFIdStandard;
    result &= iEvent.getByLabel( tElEventelPFIsoValueNeutral03PFIdStandard, hElEventelPFIsoValueNeutral03PFIdStandard );
    if ( hElEventelPFIsoValueNeutral03PFIdStandard.isValid() ) ElEventelPFIsoValueNeutral03PFIdStandard = *hElEventelPFIsoValueNeutral03PFIdStandard;
    result &= iEvent.getByLabel( tElEventelPFIsoValueNeutral04PFIdStandard, hElEventelPFIsoValueNeutral04PFIdStandard );
    if ( hElEventelPFIsoValueNeutral04PFIdStandard.isValid() ) ElEventelPFIsoValueNeutral04PFIdStandard = *hElEventelPFIsoValueNeutral04PFIdStandard;
    result &= iEvent.getByLabel( tElGenE, hElGenE );
    if ( hElGenE.isValid() ) ElGenE = *hElGenE;
    result &= iEvent.getByLabel( tElGenEta, hElGenEta );
    if ( hElGenEta.isValid() ) ElGenEta = *hElGenEta;
    result &= iEvent.getByLabel( tElGenGME, hElGenGME );
    if ( hElGenGME.isValid() ) ElGenGME = *hElGenGME;
    result &= iEvent.getByLabel( tElGenGMEta, hElGenGMEta );
    if ( hElGenGMEta.isValid() ) ElGenGMEta = *hElGenGMEta;
    result &= iEvent.getByLabel( tElGenGMID, hElGenGMID );
    if ( hElGenGMID.isValid() ) ElGenGMID = *hElGenGMID;
    result &= iEvent.getByLabel( tElGenGMPhi, hElGenGMPhi );
    if ( hElGenGMPhi.isValid() ) ElGenGMPhi = *hElGenGMPhi;
    result &= iEvent.getByLabel( tElGenGMPt, hElGenGMPt );
    if ( hElGenGMPt.isValid() ) ElGenGMPt = *hElGenGMPt;
    result &= iEvent.getByLabel( tElGenGMStatus, hElGenGMStatus );
    if ( hElGenGMStatus.isValid() ) ElGenGMStatus = *hElGenGMStatus;
    result &= iEvent.getByLabel( tElGenID, hElGenID );
    if ( hElGenID.isValid() ) ElGenID = *hElGenID;
    result &= iEvent.getByLabel( tElGenME, hElGenME );
    if ( hElGenME.isValid() ) ElGenME = *hElGenME;
    result &= iEvent.getByLabel( tElGenMEta, hElGenMEta );
    if ( hElGenMEta.isValid() ) ElGenMEta = *hElGenMEta;
    result &= iEvent.getByLabel( tElGenMID, hElGenMID );
    if ( hElGenMID.isValid() ) ElGenMID = *hElGenMID;
    result &= iEvent.getByLabel( tElGenMPhi, hElGenMPhi );
    if ( hElGenMPhi.isValid() ) ElGenMPhi = *hElGenMPhi;
    result &= iEvent.getByLabel( tElGenMPt, hElGenMPt );
    if ( hElGenMPt.isValid() ) ElGenMPt = *hElGenMPt;
    result &= iEvent.getByLabel( tElGenMStatus, hElGenMStatus );
    if ( hElGenMStatus.isValid() ) ElGenMStatus = *hElGenMStatus;
    result &= iEvent.getByLabel( tElGenPhi, hElGenPhi );
    if ( hElGenPhi.isValid() ) ElGenPhi = *hElGenPhi;
    result &= iEvent.getByLabel( tElGenPt, hElGenPt );
    if ( hElGenPt.isValid() ) ElGenPt = *hElGenPt;
    result &= iEvent.getByLabel( tElGenStatus, hElGenStatus );
    if ( hElGenStatus.isValid() ) ElGenStatus = *hElGenStatus;
    result &= iEvent.getByLabel( tElGood, hElGood );
    if ( hElGood.isValid() ) ElGood = *hElGood;
    result &= iEvent.getByLabel( tElGsfTkEta, hElGsfTkEta );
    if ( hElGsfTkEta.isValid() ) ElGsfTkEta = *hElGsfTkEta;
    result &= iEvent.getByLabel( tElGsfTkPhi, hElGsfTkPhi );
    if ( hElGsfTkPhi.isValid() ) ElGsfTkPhi = *hElGsfTkPhi;
    result &= iEvent.getByLabel( tElGsfTkPt, hElGsfTkPt );
    if ( hElGsfTkPt.isValid() ) ElGsfTkPt = *hElGsfTkPt;
    result &= iEvent.getByLabel( tElHcalOverEcal, hElHcalOverEcal );
    if ( hElHcalOverEcal.isValid() ) ElHcalOverEcal = *hElHcalOverEcal;
    result &= iEvent.getByLabel( tElHcalOverEcalBc, hElHcalOverEcalBc );
    if ( hElHcalOverEcalBc.isValid() ) ElHcalOverEcalBc = *hElHcalOverEcalBc;
    result &= iEvent.getByLabel( tElIDMVANoTrig, hElIDMVANoTrig );
    if ( hElIDMVANoTrig.isValid() ) ElIDMVANoTrig = *hElIDMVANoTrig;
    result &= iEvent.getByLabel( tElIDMVATrig, hElIDMVATrig );
    if ( hElIDMVATrig.isValid() ) ElIDMVATrig = *hElIDMVATrig;
    result &= iEvent.getByLabel( tElIDMva, hElIDMva );
    if ( hElIDMva.isValid() ) ElIDMva = *hElIDMva;
    result &= iEvent.getByLabel( tElInGap, hElInGap );
    if ( hElInGap.isValid() ) ElInGap = *hElInGap;
    result &= iEvent.getByLabel( tElIoEmIoP, hElIoEmIoP );
    if ( hElIoEmIoP.isValid() ) ElIoEmIoP = *hElIoEmIoP;
    result &= iEvent.getByLabel( tElIsEB, hElIsEB );
    if ( hElIsEB.isValid() ) ElIsEB = *hElIsEB;
    result &= iEvent.getByLabel( tElIsEE, hElIsEE );
    if ( hElIsEE.isValid() ) ElIsEE = *hElIsEE;
    result &= iEvent.getByLabel( tElIsIso, hElIsIso );
    if ( hElIsIso.isValid() ) ElIsIso = *hElIsIso;
    result &= iEvent.getByLabel( tElKfTrkchi2, hElKfTrkchi2 );
    if ( hElKfTrkchi2.isValid() ) ElKfTrkchi2 = *hElKfTrkchi2;
    result &= iEvent.getByLabel( tElKfTrkhits, hElKfTrkhits );
    if ( hElKfTrkhits.isValid() ) ElKfTrkhits = *hElKfTrkhits;
    result &= iEvent.getByLabel( tElNBrems, hElNBrems );
    if ( hElNBrems.isValid() ) ElNBrems = *hElNBrems;
    result &= iEvent.getByLabel( tElNChi2, hElNChi2 );
    if ( hElNChi2.isValid() ) ElNChi2 = *hElNChi2;
    result &= iEvent.getByLabel( tElNumberOfMissingInnerHits, hElNumberOfMissingInnerHits );
    if ( hElNumberOfMissingInnerHits.isValid() ) ElNumberOfMissingInnerHits = *hElNumberOfMissingInnerHits;
    result &= iEvent.getByLabel( tElPassConversionVeto, hElPassConversionVeto );
    if ( hElPassConversionVeto.isValid() ) ElPassConversionVeto = *hElPassConversionVeto;
    result &= iEvent.getByLabel( tElPfIsoChHad03, hElPfIsoChHad03 );
    if ( hElPfIsoChHad03.isValid() ) ElPfIsoChHad03 = *hElPfIsoChHad03;
    result &= iEvent.getByLabel( tElPfIsoNeHad03, hElPfIsoNeHad03 );
    if ( hElPfIsoNeHad03.isValid() ) ElPfIsoNeHad03 = *hElPfIsoNeHad03;
    result &= iEvent.getByLabel( tElPfIsoPhoton03, hElPfIsoPhoton03 );
    if ( hElPfIsoPhoton03.isValid() ) ElPfIsoPhoton03 = *hElPfIsoPhoton03;
    result &= iEvent.getByLabel( tElPhi, hElPhi );
    if ( hElPhi.isValid() ) ElPhi = *hElPhi;
    result &= iEvent.getByLabel( tElPreShowerOverRaw, hElPreShowerOverRaw );
    if ( hElPreShowerOverRaw.isValid() ) ElPreShowerOverRaw = *hElPreShowerOverRaw;
    result &= iEvent.getByLabel( tElPt, hElPt );
    if ( hElPt.isValid() ) ElPt = *hElPt;
    result &= iEvent.getByLabel( tElPtE, hElPtE );
    if ( hElPtE.isValid() ) ElPtE = *hElPtE;
    result &= iEvent.getByLabel( tElPx, hElPx );
    if ( hElPx.isValid() ) ElPx = *hElPx;
    result &= iEvent.getByLabel( tElPy, hElPy );
    if ( hElPy.isValid() ) ElPy = *hElPy;
    result &= iEvent.getByLabel( tElPz, hElPz );
    if ( hElPz.isValid() ) ElPz = *hElPz;
    result &= iEvent.getByLabel( tElR9, hElR9 );
    if ( hElR9.isValid() ) ElR9 = *hElR9;
    result &= iEvent.getByLabel( tElRelIso03, hElRelIso03 );
    if ( hElRelIso03.isValid() ) ElRelIso03 = *hElRelIso03;
    result &= iEvent.getByLabel( tElRelIso04, hElRelIso04 );
    if ( hElRelIso04.isValid() ) ElRelIso04 = *hElRelIso04;
    result &= iEvent.getByLabel( tElS4OverS1, hElS4OverS1 );
    if ( hElS4OverS1.isValid() ) ElS4OverS1 = *hElS4OverS1;
    result &= iEvent.getByLabel( tElSCEta, hElSCEta );
    if ( hElSCEta.isValid() ) ElSCEta = *hElSCEta;
    result &= iEvent.getByLabel( tElSCindex, hElSCindex );
    if ( hElSCindex.isValid() ) ElSCindex = *hElSCindex;
    result &= iEvent.getByLabel( tElScEtaWidth, hElScEtaWidth );
    if ( hElScEtaWidth.isValid() ) ElScEtaWidth = *hElScEtaWidth;
    result &= iEvent.getByLabel( tElScPhiWidth, hElScPhiWidth );
    if ( hElScPhiWidth.isValid() ) ElScPhiWidth = *hElScPhiWidth;
    result &= iEvent.getByLabel( tElScPixCharge, hElScPixCharge );
    if ( hElScPixCharge.isValid() ) ElScPixCharge = *hElScPixCharge;
    result &= iEvent.getByLabel( tElScSeedSeverity, hElScSeedSeverity );
    if ( hElScSeedSeverity.isValid() ) ElScSeedSeverity = *hElScSeedSeverity;
    result &= iEvent.getByLabel( tElSigmaIetaIeta, hElSigmaIetaIeta );
    if ( hElSigmaIetaIeta.isValid() ) ElSigmaIetaIeta = *hElSigmaIetaIeta;
    result &= iEvent.getByLabel( tElSigmaIphiIphi, hElSigmaIphiIphi );
    if ( hElSigmaIphiIphi.isValid() ) ElSigmaIphiIphi = *hElSigmaIphiIphi;
    result &= iEvent.getByLabel( tElTheta, hElTheta );
    if ( hElTheta.isValid() ) ElTheta = *hElTheta;
    result &= iEvent.getByLabel( tElTrackerDriven, hElTrackerDriven );
    if ( hElTrackerDriven.isValid() ) ElTrackerDriven = *hElTrackerDriven;
    result &= iEvent.getByLabel( tElTrkMomAtVtx, hElTrkMomAtVtx );
    if ( hElTrkMomAtVtx.isValid() ) ElTrkMomAtVtx = *hElTrkMomAtVtx;
    result &= iEvent.getByLabel( tElTrkMomentumError, hElTrkMomentumError );
    if ( hElTrkMomentumError.isValid() ) ElTrkMomentumError = *hElTrkMomentumError;
    result &= iEvent.getByLabel( tElelectronPFIsoChHad03, hElelectronPFIsoChHad03 );
    if ( hElelectronPFIsoChHad03.isValid() ) ElelectronPFIsoChHad03 = *hElelectronPFIsoChHad03;
    result &= iEvent.getByLabel( tElelectronPFIsoChHad04, hElelectronPFIsoChHad04 );
    if ( hElelectronPFIsoChHad04.isValid() ) ElelectronPFIsoChHad04 = *hElelectronPFIsoChHad04;
    result &= iEvent.getByLabel( tElelectronPFIsoNHad03, hElelectronPFIsoNHad03 );
    if ( hElelectronPFIsoNHad03.isValid() ) ElelectronPFIsoNHad03 = *hElelectronPFIsoNHad03;
    result &= iEvent.getByLabel( tElelectronPFIsoNHad04, hElelectronPFIsoNHad04 );
    if ( hElelectronPFIsoNHad04.isValid() ) ElelectronPFIsoNHad04 = *hElelectronPFIsoNHad04;
    result &= iEvent.getByLabel( tElelectronPFIsoPhoton03, hElelectronPFIsoPhoton03 );
    if ( hElelectronPFIsoPhoton03.isValid() ) ElelectronPFIsoPhoton03 = *hElelectronPFIsoPhoton03;
    result &= iEvent.getByLabel( tElelectronPFIsoPhoton04, hElelectronPFIsoPhoton04 );
    if ( hElelectronPFIsoPhoton04.isValid() ) ElelectronPFIsoPhoton04 = *hElelectronPFIsoPhoton04;
    result &= iEvent.getByLabel( tElelectronRadPFIsoChHad03, hElelectronRadPFIsoChHad03 );
    if ( hElelectronRadPFIsoChHad03.isValid() ) ElelectronRadPFIsoChHad03 = *hElelectronRadPFIsoChHad03;
    result &= iEvent.getByLabel( tElelectronRadPFIsoChHad04, hElelectronRadPFIsoChHad04 );
    if ( hElelectronRadPFIsoChHad04.isValid() ) ElelectronRadPFIsoChHad04 = *hElelectronRadPFIsoChHad04;
    result &= iEvent.getByLabel( tElelectronRadPFIsoNHad03, hElelectronRadPFIsoNHad03 );
    if ( hElelectronRadPFIsoNHad03.isValid() ) ElelectronRadPFIsoNHad03 = *hElelectronRadPFIsoNHad03;
    result &= iEvent.getByLabel( tElelectronRadPFIsoNHad04, hElelectronRadPFIsoNHad04 );
    if ( hElelectronRadPFIsoNHad04.isValid() ) ElelectronRadPFIsoNHad04 = *hElelectronRadPFIsoNHad04;
    result &= iEvent.getByLabel( tElelectronRadPFIsoPhoton03, hElelectronRadPFIsoPhoton03 );
    if ( hElelectronRadPFIsoPhoton03.isValid() ) ElelectronRadPFIsoPhoton03 = *hElelectronRadPFIsoPhoton03;
    result &= iEvent.getByLabel( tElelectronRadPFIsoPhoton04, hElelectronRadPFIsoPhoton04 );
    if ( hElelectronRadPFIsoPhoton04.isValid() ) ElelectronRadPFIsoPhoton04 = *hElelectronRadPFIsoPhoton04;
    result &= iEvent.getByLabel( tElfbrem, hElfbrem );
    if ( hElfbrem.isValid() ) Elfbrem = *hElfbrem;
    result &= iEvent.getByLabel( tEvent, hEvent );
    if ( hEvent.isValid() ) Event = *hEvent;
    result &= iEvent.getByLabel( tGenJetE, hGenJetE );
    if ( hGenJetE.isValid() ) GenJetE = *hGenJetE;
    result &= iEvent.getByLabel( tGenJetEmE, hGenJetEmE );
    if ( hGenJetEmE.isValid() ) GenJetEmE = *hGenJetEmE;
    result &= iEvent.getByLabel( tGenJetEta, hGenJetEta );
    if ( hGenJetEta.isValid() ) GenJetEta = *hGenJetEta;
    result &= iEvent.getByLabel( tGenJetHadE, hGenJetHadE );
    if ( hGenJetHadE.isValid() ) GenJetHadE = *hGenJetHadE;
    result &= iEvent.getByLabel( tGenJetInvE, hGenJetInvE );
    if ( hGenJetInvE.isValid() ) GenJetInvE = *hGenJetInvE;
    result &= iEvent.getByLabel( tGenJetPhi, hGenJetPhi );
    if ( hGenJetPhi.isValid() ) GenJetPhi = *hGenJetPhi;
    result &= iEvent.getByLabel( tGenJetPt, hGenJetPt );
    if ( hGenJetPt.isValid() ) GenJetPt = *hGenJetPt;
    result &= iEvent.getByLabel( tGenLeptonEta, hGenLeptonEta );
    if ( hGenLeptonEta.isValid() ) GenLeptonEta = *hGenLeptonEta;
    result &= iEvent.getByLabel( tGenLeptonGMEta, hGenLeptonGMEta );
    if ( hGenLeptonGMEta.isValid() ) GenLeptonGMEta = *hGenLeptonGMEta;
    result &= iEvent.getByLabel( tGenLeptonGMID, hGenLeptonGMID );
    if ( hGenLeptonGMID.isValid() ) GenLeptonGMID = *hGenLeptonGMID;
    result &= iEvent.getByLabel( tGenLeptonGMPhi, hGenLeptonGMPhi );
    if ( hGenLeptonGMPhi.isValid() ) GenLeptonGMPhi = *hGenLeptonGMPhi;
    result &= iEvent.getByLabel( tGenLeptonGMPt, hGenLeptonGMPt );
    if ( hGenLeptonGMPt.isValid() ) GenLeptonGMPt = *hGenLeptonGMPt;
    result &= iEvent.getByLabel( tGenLeptonGMStatus, hGenLeptonGMStatus );
    if ( hGenLeptonGMStatus.isValid() ) GenLeptonGMStatus = *hGenLeptonGMStatus;
    result &= iEvent.getByLabel( tGenLeptonID, hGenLeptonID );
    if ( hGenLeptonID.isValid() ) GenLeptonID = *hGenLeptonID;
    result &= iEvent.getByLabel( tGenLeptonMEta, hGenLeptonMEta );
    if ( hGenLeptonMEta.isValid() ) GenLeptonMEta = *hGenLeptonMEta;
    result &= iEvent.getByLabel( tGenLeptonMID, hGenLeptonMID );
    if ( hGenLeptonMID.isValid() ) GenLeptonMID = *hGenLeptonMID;
    result &= iEvent.getByLabel( tGenLeptonMPhi, hGenLeptonMPhi );
    if ( hGenLeptonMPhi.isValid() ) GenLeptonMPhi = *hGenLeptonMPhi;
    result &= iEvent.getByLabel( tGenLeptonMPt, hGenLeptonMPt );
    if ( hGenLeptonMPt.isValid() ) GenLeptonMPt = *hGenLeptonMPt;
    result &= iEvent.getByLabel( tGenLeptonMStatus, hGenLeptonMStatus );
    if ( hGenLeptonMStatus.isValid() ) GenLeptonMStatus = *hGenLeptonMStatus;
    result &= iEvent.getByLabel( tGenLeptonPhi, hGenLeptonPhi );
    if ( hGenLeptonPhi.isValid() ) GenLeptonPhi = *hGenLeptonPhi;
    result &= iEvent.getByLabel( tGenLeptonPt, hGenLeptonPt );
    if ( hGenLeptonPt.isValid() ) GenLeptonPt = *hGenLeptonPt;
    result &= iEvent.getByLabel( tGenMET, hGenMET );
    if ( hGenMET.isValid() ) GenMET = *hGenMET;
    result &= iEvent.getByLabel( tGenMETphi, hGenMETphi );
    if ( hGenMETphi.isValid() ) GenMETphi = *hGenMETphi;
    result &= iEvent.getByLabel( tGenMETpx, hGenMETpx );
    if ( hGenMETpx.isValid() ) GenMETpx = *hGenMETpx;
    result &= iEvent.getByLabel( tGenMETpy, hGenMETpy );
    if ( hGenMETpy.isValid() ) GenMETpy = *hGenMETpy;
    result &= iEvent.getByLabel( tGenPhotonEta, hGenPhotonEta );
    if ( hGenPhotonEta.isValid() ) GenPhotonEta = *hGenPhotonEta;
    result &= iEvent.getByLabel( tGenPhotonIsoDR03, hGenPhotonIsoDR03 );
    if ( hGenPhotonIsoDR03.isValid() ) GenPhotonIsoDR03 = *hGenPhotonIsoDR03;
    result &= iEvent.getByLabel( tGenPhotonIsoDR04, hGenPhotonIsoDR04 );
    if ( hGenPhotonIsoDR04.isValid() ) GenPhotonIsoDR04 = *hGenPhotonIsoDR04;
    result &= iEvent.getByLabel( tGenPhotonMotherID, hGenPhotonMotherID );
    if ( hGenPhotonMotherID.isValid() ) GenPhotonMotherID = *hGenPhotonMotherID;
    result &= iEvent.getByLabel( tGenPhotonMotherStatus, hGenPhotonMotherStatus );
    if ( hGenPhotonMotherStatus.isValid() ) GenPhotonMotherStatus = *hGenPhotonMotherStatus;
    result &= iEvent.getByLabel( tGenPhotonPartonMindR, hGenPhotonPartonMindR );
    if ( hGenPhotonPartonMindR.isValid() ) GenPhotonPartonMindR = *hGenPhotonPartonMindR;
    result &= iEvent.getByLabel( tGenPhotonPhi, hGenPhotonPhi );
    if ( hGenPhotonPhi.isValid() ) GenPhotonPhi = *hGenPhotonPhi;
    result &= iEvent.getByLabel( tGenPhotonPt, hGenPhotonPt );
    if ( hGenPhotonPt.isValid() ) GenPhotonPt = *hGenPhotonPt;
    result &= iEvent.getByLabel( tGenWeight, hGenWeight );
    if ( hGenWeight.isValid() ) GenWeight = *hGenWeight;
    result &= iEvent.getByLabel( tGoodEvent, hGoodEvent );
    if ( hGoodEvent.isValid() ) GoodEvent = *hGoodEvent;
    result &= iEvent.getByLabel( tGoodSCEnergy, hGoodSCEnergy );
    if ( hGoodSCEnergy.isValid() ) GoodSCEnergy = *hGoodSCEnergy;
    result &= iEvent.getByLabel( tGoodSCEta, hGoodSCEta );
    if ( hGoodSCEta.isValid() ) GoodSCEta = *hGoodSCEta;
    result &= iEvent.getByLabel( tGoodSCPhi, hGoodSCPhi );
    if ( hGoodSCPhi.isValid() ) GoodSCPhi = *hGoodSCPhi;
    result &= iEvent.getByLabel( tHBHENoiseFilterResult, hHBHENoiseFilterResult );
    if ( hHBHENoiseFilterResult.isValid() ) HBHENoiseFilterResult = *hHBHENoiseFilterResult;
    result &= iEvent.getByLabel( tHCALEsumx, hHCALEsumx );
    if ( hHCALEsumx.isValid() ) HCALEsumx = *hHCALEsumx;
    result &= iEvent.getByLabel( tHCALEsumy, hHCALEsumy );
    if ( hHCALEsumy.isValid() ) HCALEsumy = *hHCALEsumy;
    result &= iEvent.getByLabel( tHCALEsumz, hHCALEsumz );
    if ( hHCALEsumz.isValid() ) HCALEsumz = *hHCALEsumz;
    result &= iEvent.getByLabel( tHCALMET, hHCALMET );
    if ( hHCALMET.isValid() ) HCALMET = *hHCALMET;
    result &= iEvent.getByLabel( tHCALMETPhi, hHCALMETPhi );
    if ( hHCALMETPhi.isValid() ) HCALMETPhi = *hHCALMETPhi;
    result &= iEvent.getByLabel( tHCALMETeta, hHCALMETeta );
    if ( hHCALMETeta.isValid() ) HCALMETeta = *hHCALMETeta;
    result &= iEvent.getByLabel( tHCALSumEt, hHCALSumEt );
    if ( hHCALSumEt.isValid() ) HCALSumEt = *hHCALSumEt;
    result &= iEvent.getByLabel( tHLTObjectEta0, hHLTObjectEta0 );
    if ( hHLTObjectEta0.isValid() ) HLTObjectEta0 = *hHLTObjectEta0;
    result &= iEvent.getByLabel( tHLTObjectEta1, hHLTObjectEta1 );
    if ( hHLTObjectEta1.isValid() ) HLTObjectEta1 = *hHLTObjectEta1;
    result &= iEvent.getByLabel( tHLTObjectEta2, hHLTObjectEta2 );
    if ( hHLTObjectEta2.isValid() ) HLTObjectEta2 = *hHLTObjectEta2;
    result &= iEvent.getByLabel( tHLTObjectEta3, hHLTObjectEta3 );
    if ( hHLTObjectEta3.isValid() ) HLTObjectEta3 = *hHLTObjectEta3;
    result &= iEvent.getByLabel( tHLTObjectEta4, hHLTObjectEta4 );
    if ( hHLTObjectEta4.isValid() ) HLTObjectEta4 = *hHLTObjectEta4;
    result &= iEvent.getByLabel( tHLTObjectEta5, hHLTObjectEta5 );
    if ( hHLTObjectEta5.isValid() ) HLTObjectEta5 = *hHLTObjectEta5;
    result &= iEvent.getByLabel( tHLTObjectEta6, hHLTObjectEta6 );
    if ( hHLTObjectEta6.isValid() ) HLTObjectEta6 = *hHLTObjectEta6;
    result &= iEvent.getByLabel( tHLTObjectEta7, hHLTObjectEta7 );
    if ( hHLTObjectEta7.isValid() ) HLTObjectEta7 = *hHLTObjectEta7;
    result &= iEvent.getByLabel( tHLTObjectEta8, hHLTObjectEta8 );
    if ( hHLTObjectEta8.isValid() ) HLTObjectEta8 = *hHLTObjectEta8;
    result &= iEvent.getByLabel( tHLTObjectEta9, hHLTObjectEta9 );
    if ( hHLTObjectEta9.isValid() ) HLTObjectEta9 = *hHLTObjectEta9;
    result &= iEvent.getByLabel( tHLTObjectID0, hHLTObjectID0 );
    if ( hHLTObjectID0.isValid() ) HLTObjectID0 = *hHLTObjectID0;
    result &= iEvent.getByLabel( tHLTObjectID1, hHLTObjectID1 );
    if ( hHLTObjectID1.isValid() ) HLTObjectID1 = *hHLTObjectID1;
    result &= iEvent.getByLabel( tHLTObjectID2, hHLTObjectID2 );
    if ( hHLTObjectID2.isValid() ) HLTObjectID2 = *hHLTObjectID2;
    result &= iEvent.getByLabel( tHLTObjectID3, hHLTObjectID3 );
    if ( hHLTObjectID3.isValid() ) HLTObjectID3 = *hHLTObjectID3;
    result &= iEvent.getByLabel( tHLTObjectID4, hHLTObjectID4 );
    if ( hHLTObjectID4.isValid() ) HLTObjectID4 = *hHLTObjectID4;
    result &= iEvent.getByLabel( tHLTObjectID5, hHLTObjectID5 );
    if ( hHLTObjectID5.isValid() ) HLTObjectID5 = *hHLTObjectID5;
    result &= iEvent.getByLabel( tHLTObjectID6, hHLTObjectID6 );
    if ( hHLTObjectID6.isValid() ) HLTObjectID6 = *hHLTObjectID6;
    result &= iEvent.getByLabel( tHLTObjectID7, hHLTObjectID7 );
    if ( hHLTObjectID7.isValid() ) HLTObjectID7 = *hHLTObjectID7;
    result &= iEvent.getByLabel( tHLTObjectID8, hHLTObjectID8 );
    if ( hHLTObjectID8.isValid() ) HLTObjectID8 = *hHLTObjectID8;
    result &= iEvent.getByLabel( tHLTObjectID9, hHLTObjectID9 );
    if ( hHLTObjectID9.isValid() ) HLTObjectID9 = *hHLTObjectID9;
    result &= iEvent.getByLabel( tHLTObjectPhi0, hHLTObjectPhi0 );
    if ( hHLTObjectPhi0.isValid() ) HLTObjectPhi0 = *hHLTObjectPhi0;
    result &= iEvent.getByLabel( tHLTObjectPhi1, hHLTObjectPhi1 );
    if ( hHLTObjectPhi1.isValid() ) HLTObjectPhi1 = *hHLTObjectPhi1;
    result &= iEvent.getByLabel( tHLTObjectPhi2, hHLTObjectPhi2 );
    if ( hHLTObjectPhi2.isValid() ) HLTObjectPhi2 = *hHLTObjectPhi2;
    result &= iEvent.getByLabel( tHLTObjectPhi3, hHLTObjectPhi3 );
    if ( hHLTObjectPhi3.isValid() ) HLTObjectPhi3 = *hHLTObjectPhi3;
    result &= iEvent.getByLabel( tHLTObjectPhi4, hHLTObjectPhi4 );
    if ( hHLTObjectPhi4.isValid() ) HLTObjectPhi4 = *hHLTObjectPhi4;
    result &= iEvent.getByLabel( tHLTObjectPhi5, hHLTObjectPhi5 );
    if ( hHLTObjectPhi5.isValid() ) HLTObjectPhi5 = *hHLTObjectPhi5;
    result &= iEvent.getByLabel( tHLTObjectPhi6, hHLTObjectPhi6 );
    if ( hHLTObjectPhi6.isValid() ) HLTObjectPhi6 = *hHLTObjectPhi6;
    result &= iEvent.getByLabel( tHLTObjectPhi7, hHLTObjectPhi7 );
    if ( hHLTObjectPhi7.isValid() ) HLTObjectPhi7 = *hHLTObjectPhi7;
    result &= iEvent.getByLabel( tHLTObjectPhi8, hHLTObjectPhi8 );
    if ( hHLTObjectPhi8.isValid() ) HLTObjectPhi8 = *hHLTObjectPhi8;
    result &= iEvent.getByLabel( tHLTObjectPhi9, hHLTObjectPhi9 );
    if ( hHLTObjectPhi9.isValid() ) HLTObjectPhi9 = *hHLTObjectPhi9;
    result &= iEvent.getByLabel( tHLTObjectPt0, hHLTObjectPt0 );
    if ( hHLTObjectPt0.isValid() ) HLTObjectPt0 = *hHLTObjectPt0;
    result &= iEvent.getByLabel( tHLTObjectPt1, hHLTObjectPt1 );
    if ( hHLTObjectPt1.isValid() ) HLTObjectPt1 = *hHLTObjectPt1;
    result &= iEvent.getByLabel( tHLTObjectPt2, hHLTObjectPt2 );
    if ( hHLTObjectPt2.isValid() ) HLTObjectPt2 = *hHLTObjectPt2;
    result &= iEvent.getByLabel( tHLTObjectPt3, hHLTObjectPt3 );
    if ( hHLTObjectPt3.isValid() ) HLTObjectPt3 = *hHLTObjectPt3;
    result &= iEvent.getByLabel( tHLTObjectPt4, hHLTObjectPt4 );
    if ( hHLTObjectPt4.isValid() ) HLTObjectPt4 = *hHLTObjectPt4;
    result &= iEvent.getByLabel( tHLTObjectPt5, hHLTObjectPt5 );
    if ( hHLTObjectPt5.isValid() ) HLTObjectPt5 = *hHLTObjectPt5;
    result &= iEvent.getByLabel( tHLTObjectPt6, hHLTObjectPt6 );
    if ( hHLTObjectPt6.isValid() ) HLTObjectPt6 = *hHLTObjectPt6;
    result &= iEvent.getByLabel( tHLTObjectPt7, hHLTObjectPt7 );
    if ( hHLTObjectPt7.isValid() ) HLTObjectPt7 = *hHLTObjectPt7;
    result &= iEvent.getByLabel( tHLTObjectPt8, hHLTObjectPt8 );
    if ( hHLTObjectPt8.isValid() ) HLTObjectPt8 = *hHLTObjectPt8;
    result &= iEvent.getByLabel( tHLTObjectPt9, hHLTObjectPt9 );
    if ( hHLTObjectPt9.isValid() ) HLTObjectPt9 = *hHLTObjectPt9;
    result &= iEvent.getByLabel( tHLTPrescale, hHLTPrescale );
    if ( hHLTPrescale.isValid() ) HLTPrescale = *hHLTPrescale;
    result &= iEvent.getByLabel( tHLTResults, hHLTResults );
    if ( hHLTResults.isValid() ) HLTResults = *hHLTResults;
    result &= iEvent.getByLabel( tJArea, hJArea );
    if ( hJArea.isValid() ) JArea = *hJArea;
    result &= iEvent.getByLabel( tJBeta, hJBeta );
    if ( hJBeta.isValid() ) JBeta = *hJBeta;
    result &= iEvent.getByLabel( tJBetaSq, hJBetaSq );
    if ( hJBetaSq.isValid() ) JBetaSq = *hJBetaSq;
    result &= iEvent.getByLabel( tJBetaStar, hJBetaStar );
    if ( hJBetaStar.isValid() ) JBetaStar = *hJBetaStar;
    result &= iEvent.getByLabel( tJChargedEmFrac, hJChargedEmFrac );
    if ( hJChargedEmFrac.isValid() ) JChargedEmFrac = *hJChargedEmFrac;
    result &= iEvent.getByLabel( tJChargedHadFrac, hJChargedHadFrac );
    if ( hJChargedHadFrac.isValid() ) JChargedHadFrac = *hJChargedHadFrac;
    result &= iEvent.getByLabel( tJChargedMuEnergyFrac, hJChargedMuEnergyFrac );
    if ( hJChargedMuEnergyFrac.isValid() ) JChargedMuEnergyFrac = *hJChargedMuEnergyFrac;
    result &= iEvent.getByLabel( tJE, hJE );
    if ( hJE.isValid() ) JE = *hJE;
    result &= iEvent.getByLabel( tJEcorr, hJEcorr );
    if ( hJEcorr.isValid() ) JEcorr = *hJEcorr;
    result &= iEvent.getByLabel( tJEt, hJEt );
    if ( hJEt.isValid() ) JEt = *hJEt;
    result &= iEvent.getByLabel( tJEta, hJEta );
    if ( hJEta.isValid() ) JEta = *hJEta;
    result &= iEvent.getByLabel( tJEtaRms, hJEtaRms );
    if ( hJEtaRms.isValid() ) JEtaRms = *hJEtaRms;
    result &= iEvent.getByLabel( tJGenJetIndex, hJGenJetIndex );
    if ( hJGenJetIndex.isValid() ) JGenJetIndex = *hJGenJetIndex;
    result &= iEvent.getByLabel( tJGood, hJGood );
    if ( hJGood.isValid() ) JGood = *hJGood;
    result &= iEvent.getByLabel( tJHFEMFrac, hJHFEMFrac );
    if ( hJHFEMFrac.isValid() ) JHFEMFrac = *hJHFEMFrac;
    result &= iEvent.getByLabel( tJHFHadFrac, hJHFHadFrac );
    if ( hJHFHadFrac.isValid() ) JHFHadFrac = *hJHFHadFrac;
    result &= iEvent.getByLabel( tJMass, hJMass );
    if ( hJMass.isValid() ) JMass = *hJMass;
    result &= iEvent.getByLabel( tJMetCorrArea, hJMetCorrArea );
    if ( hJMetCorrArea.isValid() ) JMetCorrArea = *hJMetCorrArea;
    result &= iEvent.getByLabel( tJMetCorrEMF, hJMetCorrEMF );
    if ( hJMetCorrEMF.isValid() ) JMetCorrEMF = *hJMetCorrEMF;
    result &= iEvent.getByLabel( tJMetCorrNoMuPt, hJMetCorrNoMuPt );
    if ( hJMetCorrNoMuPt.isValid() ) JMetCorrNoMuPt = *hJMetCorrNoMuPt;
    result &= iEvent.getByLabel( tJMetCorrPhi, hJMetCorrPhi );
    if ( hJMetCorrPhi.isValid() ) JMetCorrPhi = *hJMetCorrPhi;
    result &= iEvent.getByLabel( tJMetCorrRawEta, hJMetCorrRawEta );
    if ( hJMetCorrRawEta.isValid() ) JMetCorrRawEta = *hJMetCorrRawEta;
    result &= iEvent.getByLabel( tJMetCorrRawPt, hJMetCorrRawPt );
    if ( hJMetCorrRawPt.isValid() ) JMetCorrRawPt = *hJMetCorrRawPt;
    result &= iEvent.getByLabel( tJNAssoTracks, hJNAssoTracks );
    if ( hJNAssoTracks.isValid() ) JNAssoTracks = *hJNAssoTracks;
    result &= iEvent.getByLabel( tJNConstituents, hJNConstituents );
    if ( hJNConstituents.isValid() ) JNConstituents = *hJNConstituents;
    result &= iEvent.getByLabel( tJNNeutrals, hJNNeutrals );
    if ( hJNNeutrals.isValid() ) JNNeutrals = *hJNNeutrals;
    result &= iEvent.getByLabel( tJNeutralEmFrac, hJNeutralEmFrac );
    if ( hJNeutralEmFrac.isValid() ) JNeutralEmFrac = *hJNeutralEmFrac;
    result &= iEvent.getByLabel( tJNeutralHadFrac, hJNeutralHadFrac );
    if ( hJNeutralHadFrac.isValid() ) JNeutralHadFrac = *hJNeutralHadFrac;
    result &= iEvent.getByLabel( tJPartonFlavour, hJPartonFlavour );
    if ( hJPartonFlavour.isValid() ) JPartonFlavour = *hJPartonFlavour;
    result &= iEvent.getByLabel( tJPhi, hJPhi );
    if ( hJPhi.isValid() ) JPhi = *hJPhi;
    result &= iEvent.getByLabel( tJPhiRms, hJPhiRms );
    if ( hJPhiRms.isValid() ) JPhiRms = *hJPhiRms;
    result &= iEvent.getByLabel( tJPhoFrac, hJPhoFrac );
    if ( hJPhoFrac.isValid() ) JPhoFrac = *hJPhoFrac;
    result &= iEvent.getByLabel( tJPt, hJPt );
    if ( hJPt.isValid() ) JPt = *hJPt;
    result &= iEvent.getByLabel( tJPtD, hJPtD );
    if ( hJPtD.isValid() ) JPtD = *hJPtD;
    result &= iEvent.getByLabel( tJPx, hJPx );
    if ( hJPx.isValid() ) JPx = *hJPx;
    result &= iEvent.getByLabel( tJPy, hJPy );
    if ( hJPy.isValid() ) JPy = *hJPy;
    result &= iEvent.getByLabel( tJPz, hJPz );
    if ( hJPz.isValid() ) JPz = *hJPz;
    result &= iEvent.getByLabel( tJRMSCand, hJRMSCand );
    if ( hJRMSCand.isValid() ) JRMSCand = *hJRMSCand;
    result &= iEvent.getByLabel( tJVtxExx, hJVtxExx );
    if ( hJVtxExx.isValid() ) JVtxExx = *hJVtxExx;
    result &= iEvent.getByLabel( tJVtxEyx, hJVtxEyx );
    if ( hJVtxEyx.isValid() ) JVtxEyx = *hJVtxEyx;
    result &= iEvent.getByLabel( tJVtxEyy, hJVtxEyy );
    if ( hJVtxEyy.isValid() ) JVtxEyy = *hJVtxEyy;
    result &= iEvent.getByLabel( tJVtxEzx, hJVtxEzx );
    if ( hJVtxEzx.isValid() ) JVtxEzx = *hJVtxEzx;
    result &= iEvent.getByLabel( tJVtxEzy, hJVtxEzy );
    if ( hJVtxEzy.isValid() ) JVtxEzy = *hJVtxEzy;
    result &= iEvent.getByLabel( tJVtxEzz, hJVtxEzz );
    if ( hJVtxEzz.isValid() ) JVtxEzz = *hJVtxEzz;
    result &= iEvent.getByLabel( tJVtxNChi2, hJVtxNChi2 );
    if ( hJVtxNChi2.isValid() ) JVtxNChi2 = *hJVtxNChi2;
    result &= iEvent.getByLabel( tJVtxx, hJVtxx );
    if ( hJVtxx.isValid() ) JVtxx = *hJVtxx;
    result &= iEvent.getByLabel( tJVtxy, hJVtxy );
    if ( hJVtxy.isValid() ) JVtxy = *hJVtxy;
    result &= iEvent.getByLabel( tJVtxz, hJVtxz );
    if ( hJVtxz.isValid() ) JVtxz = *hJVtxz;
    result &= iEvent.getByLabel( tJeMinDR, hJeMinDR );
    if ( hJeMinDR.isValid() ) JeMinDR = *hJeMinDR;
    result &= iEvent.getByLabel( tJnewPFCombinedSecondaryVertexBPFJetTags, hJnewPFCombinedSecondaryVertexBPFJetTags );
    if ( hJnewPFCombinedSecondaryVertexBPFJetTags.isValid() ) JnewPFCombinedSecondaryVertexBPFJetTags = *hJnewPFCombinedSecondaryVertexBPFJetTags;
    result &= iEvent.getByLabel( tJnewPFCombinedSecondaryVertexMVABPFJetTags, hJnewPFCombinedSecondaryVertexMVABPFJetTags );
    if ( hJnewPFCombinedSecondaryVertexMVABPFJetTags.isValid() ) JnewPFCombinedSecondaryVertexMVABPFJetTags = *hJnewPFCombinedSecondaryVertexMVABPFJetTags;
    result &= iEvent.getByLabel( tJnewPFJetBProbabilityBPFJetTags, hJnewPFJetBProbabilityBPFJetTags );
    if ( hJnewPFJetBProbabilityBPFJetTags.isValid() ) JnewPFJetBProbabilityBPFJetTags = *hJnewPFJetBProbabilityBPFJetTags;
    result &= iEvent.getByLabel( tJnewPFJetProbabilityBPFJetTags, hJnewPFJetProbabilityBPFJetTags );
    if ( hJnewPFJetProbabilityBPFJetTags.isValid() ) JnewPFJetProbabilityBPFJetTags = *hJnewPFJetProbabilityBPFJetTags;
    result &= iEvent.getByLabel( tJnewPFSimpleSecondaryVertexHighEffBJetTags, hJnewPFSimpleSecondaryVertexHighEffBJetTags );
    if ( hJnewPFSimpleSecondaryVertexHighEffBJetTags.isValid() ) JnewPFSimpleSecondaryVertexHighEffBJetTags = *hJnewPFSimpleSecondaryVertexHighEffBJetTags;
    result &= iEvent.getByLabel( tJnewPFSimpleSecondaryVertexHighPurBJetTags, hJnewPFSimpleSecondaryVertexHighPurBJetTags );
    if ( hJnewPFSimpleSecondaryVertexHighPurBJetTags.isValid() ) JnewPFSimpleSecondaryVertexHighPurBJetTags = *hJnewPFSimpleSecondaryVertexHighPurBJetTags;
    result &= iEvent.getByLabel( tJnewPFTrackCountingHighEffBJetTags, hJnewPFTrackCountingHighEffBJetTags );
    if ( hJnewPFTrackCountingHighEffBJetTags.isValid() ) JnewPFTrackCountingHighEffBJetTags = *hJnewPFTrackCountingHighEffBJetTags;
    result &= iEvent.getByLabel( tJnewPFTrackCountingHighPurBJetTags, hJnewPFTrackCountingHighPurBJetTags );
    if ( hJnewPFTrackCountingHighPurBJetTags.isValid() ) JnewPFTrackCountingHighPurBJetTags = *hJnewPFTrackCountingHighPurBJetTags;
    result &= iEvent.getByLabel( tJtrk1px, hJtrk1px );
    if ( hJtrk1px.isValid() ) Jtrk1px = *hJtrk1px;
    result &= iEvent.getByLabel( tJtrk1py, hJtrk1py );
    if ( hJtrk1py.isValid() ) Jtrk1py = *hJtrk1py;
    result &= iEvent.getByLabel( tJtrk1pz, hJtrk1pz );
    if ( hJtrk1pz.isValid() ) Jtrk1pz = *hJtrk1pz;
    result &= iEvent.getByLabel( tJtrk2px, hJtrk2px );
    if ( hJtrk2px.isValid() ) Jtrk2px = *hJtrk2px;
    result &= iEvent.getByLabel( tJtrk2py, hJtrk2py );
    if ( hJtrk2py.isValid() ) Jtrk2py = *hJtrk2py;
    result &= iEvent.getByLabel( tJtrk2pz, hJtrk2pz );
    if ( hJtrk2pz.isValid() ) Jtrk2pz = *hJtrk2pz;
    result &= iEvent.getByLabel( tJtrk3px, hJtrk3px );
    if ( hJtrk3px.isValid() ) Jtrk3px = *hJtrk3px;
    result &= iEvent.getByLabel( tJtrk3py, hJtrk3py );
    if ( hJtrk3py.isValid() ) Jtrk3py = *hJtrk3py;
    result &= iEvent.getByLabel( tJtrk3pz, hJtrk3pz );
    if ( hJtrk3pz.isValid() ) Jtrk3pz = *hJtrk3pz;
    result &= iEvent.getByLabel( tL1PhysResults, hL1PhysResults );
    if ( hL1PhysResults.isValid() ) L1PhysResults = *hL1PhysResults;
    result &= iEvent.getByLabel( tL1TechResults, hL1TechResults );
    if ( hL1TechResults.isValid() ) L1TechResults = *hL1TechResults;
    result &= iEvent.getByLabel( tLHEEventE, hLHEEventE );
    if ( hLHEEventE.isValid() ) LHEEventE = *hLHEEventE;
    result &= iEvent.getByLabel( tLHEEventID, hLHEEventID );
    if ( hLHEEventID.isValid() ) LHEEventID = *hLHEEventID;
    result &= iEvent.getByLabel( tLHEEventM, hLHEEventM );
    if ( hLHEEventM.isValid() ) LHEEventM = *hLHEEventM;
    result &= iEvent.getByLabel( tLHEEventMotherFirst, hLHEEventMotherFirst );
    if ( hLHEEventMotherFirst.isValid() ) LHEEventMotherFirst = *hLHEEventMotherFirst;
    result &= iEvent.getByLabel( tLHEEventMotherSecond, hLHEEventMotherSecond );
    if ( hLHEEventMotherSecond.isValid() ) LHEEventMotherSecond = *hLHEEventMotherSecond;
    result &= iEvent.getByLabel( tLHEEventPx, hLHEEventPx );
    if ( hLHEEventPx.isValid() ) LHEEventPx = *hLHEEventPx;
    result &= iEvent.getByLabel( tLHEEventPy, hLHEEventPy );
    if ( hLHEEventPy.isValid() ) LHEEventPy = *hLHEEventPy;
    result &= iEvent.getByLabel( tLHEEventPz, hLHEEventPz );
    if ( hLHEEventPz.isValid() ) LHEEventPz = *hLHEEventPz;
    result &= iEvent.getByLabel( tLHEEventStatus, hLHEEventStatus );
    if ( hLHEEventStatus.isValid() ) LHEEventStatus = *hLHEEventStatus;
    result &= iEvent.getByLabel( tLumiSection, hLumiSection );
    if ( hLumiSection.isValid() ) LumiSection = *hLumiSection;
    result &= iEvent.getByLabel( tM0, hM0 );
    if ( hM0.isValid() ) M0 = *hM0;
    result &= iEvent.getByLabel( tM12, hM12 );
    if ( hM12.isValid() ) M12 = *hM12;
    result &= iEvent.getByLabel( tMETR12, hMETR12 );
    if ( hMETR12.isValid() ) METR12 = *hMETR12;
    result &= iEvent.getByLabel( tMETR21, hMETR21 );
    if ( hMETR21.isValid() ) METR21 = *hMETR21;
    result &= iEvent.getByLabel( tMassChi, hMassChi );
    if ( hMassChi.isValid() ) MassChi = *hMassChi;
    result &= iEvent.getByLabel( tMassGlu, hMassGlu );
    if ( hMassGlu.isValid() ) MassGlu = *hMassGlu;
    result &= iEvent.getByLabel( tMassLSP, hMassLSP );
    if ( hMassLSP.isValid() ) MassLSP = *hMassLSP;
    result &= iEvent.getByLabel( tMaxElExceed, hMaxElExceed );
    if ( hMaxElExceed.isValid() ) MaxElExceed = *hMaxElExceed;
    result &= iEvent.getByLabel( tMaxGenJetExceed, hMaxGenJetExceed );
    if ( hMaxGenJetExceed.isValid() ) MaxGenJetExceed = *hMaxGenJetExceed;
    result &= iEvent.getByLabel( tMaxGenLepExceed, hMaxGenLepExceed );
    if ( hMaxGenLepExceed.isValid() ) MaxGenLepExceed = *hMaxGenLepExceed;
    result &= iEvent.getByLabel( tMaxGenPartExceed, hMaxGenPartExceed );
    if ( hMaxGenPartExceed.isValid() ) MaxGenPartExceed = *hMaxGenPartExceed;
    result &= iEvent.getByLabel( tMaxGenPhoExceed, hMaxGenPhoExceed );
    if ( hMaxGenPhoExceed.isValid() ) MaxGenPhoExceed = *hMaxGenPhoExceed;
    result &= iEvent.getByLabel( tMaxJetExceed, hMaxJetExceed );
    if ( hMaxJetExceed.isValid() ) MaxJetExceed = *hMaxJetExceed;
    result &= iEvent.getByLabel( tMaxMuExceed, hMaxMuExceed );
    if ( hMaxMuExceed.isValid() ) MaxMuExceed = *hMaxMuExceed;
    result &= iEvent.getByLabel( tMaxPhotonsExceed, hMaxPhotonsExceed );
    if ( hMaxPhotonsExceed.isValid() ) MaxPhotonsExceed = *hMaxPhotonsExceed;
    result &= iEvent.getByLabel( tMaxTrkExceed, hMaxTrkExceed );
    if ( hMaxTrkExceed.isValid() ) MaxTrkExceed = *hMaxTrkExceed;
    result &= iEvent.getByLabel( tMaxUncJetExceed, hMaxUncJetExceed );
    if ( hMaxUncJetExceed.isValid() ) MaxUncJetExceed = *hMaxUncJetExceed;
    result &= iEvent.getByLabel( tMaxVerticesExceed, hMaxVerticesExceed );
    if ( hMaxVerticesExceed.isValid() ) MaxVerticesExceed = *hMaxVerticesExceed;
    result &= iEvent.getByLabel( tMuCaloComp, hMuCaloComp );
    if ( hMuCaloComp.isValid() ) MuCaloComp = *hMuCaloComp;
    result &= iEvent.getByLabel( tMuCharge, hMuCharge );
    if ( hMuCharge.isValid() ) MuCharge = *hMuCharge;
    result &= iEvent.getByLabel( tMuD03DE, hMuD03DE );
    if ( hMuD03DE.isValid() ) MuD03DE = *hMuD03DE;
    result &= iEvent.getByLabel( tMuD03DPV, hMuD03DPV );
    if ( hMuD03DPV.isValid() ) MuD03DPV = *hMuD03DPV;
    result &= iEvent.getByLabel( tMuD0BS, hMuD0BS );
    if ( hMuD0BS.isValid() ) MuD0BS = *hMuD0BS;
    result &= iEvent.getByLabel( tMuD0E, hMuD0E );
    if ( hMuD0E.isValid() ) MuD0E = *hMuD0E;
    result &= iEvent.getByLabel( tMuD0PV, hMuD0PV );
    if ( hMuD0PV.isValid() ) MuD0PV = *hMuD0PV;
    result &= iEvent.getByLabel( tMuDzBS, hMuDzBS );
    if ( hMuDzBS.isValid() ) MuDzBS = *hMuDzBS;
    result &= iEvent.getByLabel( tMuDzE, hMuDzE );
    if ( hMuDzE.isValid() ) MuDzE = *hMuDzE;
    result &= iEvent.getByLabel( tMuDzPV, hMuDzPV );
    if ( hMuDzPV.isValid() ) MuDzPV = *hMuDzPV;
    result &= iEvent.getByLabel( tMuE, hMuE );
    if ( hMuE.isValid() ) MuE = *hMuE;
    result &= iEvent.getByLabel( tMuEem, hMuEem );
    if ( hMuEem.isValid() ) MuEem = *hMuEem;
    result &= iEvent.getByLabel( tMuEhad, hMuEhad );
    if ( hMuEhad.isValid() ) MuEhad = *hMuEhad;
    result &= iEvent.getByLabel( tMuEt, hMuEt );
    if ( hMuEt.isValid() ) MuEt = *hMuEt;
    result &= iEvent.getByLabel( tMuEta, hMuEta );
    if ( hMuEta.isValid() ) MuEta = *hMuEta;
    result &= iEvent.getByLabel( tMuGenE, hMuGenE );
    if ( hMuGenE.isValid() ) MuGenE = *hMuGenE;
    result &= iEvent.getByLabel( tMuGenEta, hMuGenEta );
    if ( hMuGenEta.isValid() ) MuGenEta = *hMuGenEta;
    result &= iEvent.getByLabel( tMuGenGME, hMuGenGME );
    if ( hMuGenGME.isValid() ) MuGenGME = *hMuGenGME;
    result &= iEvent.getByLabel( tMuGenGMEta, hMuGenGMEta );
    if ( hMuGenGMEta.isValid() ) MuGenGMEta = *hMuGenGMEta;
    result &= iEvent.getByLabel( tMuGenGMID, hMuGenGMID );
    if ( hMuGenGMID.isValid() ) MuGenGMID = *hMuGenGMID;
    result &= iEvent.getByLabel( tMuGenGMPhi, hMuGenGMPhi );
    if ( hMuGenGMPhi.isValid() ) MuGenGMPhi = *hMuGenGMPhi;
    result &= iEvent.getByLabel( tMuGenGMPt, hMuGenGMPt );
    if ( hMuGenGMPt.isValid() ) MuGenGMPt = *hMuGenGMPt;
    result &= iEvent.getByLabel( tMuGenGMStatus, hMuGenGMStatus );
    if ( hMuGenGMStatus.isValid() ) MuGenGMStatus = *hMuGenGMStatus;
    result &= iEvent.getByLabel( tMuGenID, hMuGenID );
    if ( hMuGenID.isValid() ) MuGenID = *hMuGenID;
    result &= iEvent.getByLabel( tMuGenME, hMuGenME );
    if ( hMuGenME.isValid() ) MuGenME = *hMuGenME;
    result &= iEvent.getByLabel( tMuGenMEta, hMuGenMEta );
    if ( hMuGenMEta.isValid() ) MuGenMEta = *hMuGenMEta;
    result &= iEvent.getByLabel( tMuGenMID, hMuGenMID );
    if ( hMuGenMID.isValid() ) MuGenMID = *hMuGenMID;
    result &= iEvent.getByLabel( tMuGenMPhi, hMuGenMPhi );
    if ( hMuGenMPhi.isValid() ) MuGenMPhi = *hMuGenMPhi;
    result &= iEvent.getByLabel( tMuGenMPt, hMuGenMPt );
    if ( hMuGenMPt.isValid() ) MuGenMPt = *hMuGenMPt;
    result &= iEvent.getByLabel( tMuGenMStatus, hMuGenMStatus );
    if ( hMuGenMStatus.isValid() ) MuGenMStatus = *hMuGenMStatus;
    result &= iEvent.getByLabel( tMuGenPhi, hMuGenPhi );
    if ( hMuGenPhi.isValid() ) MuGenPhi = *hMuGenPhi;
    result &= iEvent.getByLabel( tMuGenPt, hMuGenPt );
    if ( hMuGenPt.isValid() ) MuGenPt = *hMuGenPt;
    result &= iEvent.getByLabel( tMuGenStatus, hMuGenStatus );
    if ( hMuGenStatus.isValid() ) MuGenStatus = *hMuGenStatus;
    result &= iEvent.getByLabel( tMuGood, hMuGood );
    if ( hMuGood.isValid() ) MuGood = *hMuGood;
    result &= iEvent.getByLabel( tMuInnerTkNChi2, hMuInnerTkNChi2 );
    if ( hMuInnerTkNChi2.isValid() ) MuInnerTkNChi2 = *hMuInnerTkNChi2;
    result &= iEvent.getByLabel( tMuInnerTkPt, hMuInnerTkPt );
    if ( hMuInnerTkPt.isValid() ) MuInnerTkPt = *hMuInnerTkPt;
    result &= iEvent.getByLabel( tMuIsAllArbitrated, hMuIsAllArbitrated );
    if ( hMuIsAllArbitrated.isValid() ) MuIsAllArbitrated = *hMuIsAllArbitrated;
    result &= iEvent.getByLabel( tMuIsAllStaMuons, hMuIsAllStaMuons );
    if ( hMuIsAllStaMuons.isValid() ) MuIsAllStaMuons = *hMuIsAllStaMuons;
    result &= iEvent.getByLabel( tMuIsAllTrkMuons, hMuIsAllTrkMuons );
    if ( hMuIsAllTrkMuons.isValid() ) MuIsAllTrkMuons = *hMuIsAllTrkMuons;
    result &= iEvent.getByLabel( tMuIsGMPT, hMuIsGMPT );
    if ( hMuIsGMPT.isValid() ) MuIsGMPT = *hMuIsGMPT;
    result &= iEvent.getByLabel( tMuIsGMStaChiComp, hMuIsGMStaChiComp );
    if ( hMuIsGMStaChiComp.isValid() ) MuIsGMStaChiComp = *hMuIsGMStaChiComp;
    result &= iEvent.getByLabel( tMuIsGMTkChiComp, hMuIsGMTkChiComp );
    if ( hMuIsGMTkChiComp.isValid() ) MuIsGMTkChiComp = *hMuIsGMTkChiComp;
    result &= iEvent.getByLabel( tMuIsGMTkKinkTight, hMuIsGMTkKinkTight );
    if ( hMuIsGMTkKinkTight.isValid() ) MuIsGMTkKinkTight = *hMuIsGMTkKinkTight;
    result &= iEvent.getByLabel( tMuIsGlobalMuon, hMuIsGlobalMuon );
    if ( hMuIsGlobalMuon.isValid() ) MuIsGlobalMuon = *hMuIsGlobalMuon;
    result &= iEvent.getByLabel( tMuIsIso, hMuIsIso );
    if ( hMuIsIso.isValid() ) MuIsIso = *hMuIsIso;
    result &= iEvent.getByLabel( tMuIsPFMuon, hMuIsPFMuon );
    if ( hMuIsPFMuon.isValid() ) MuIsPFMuon = *hMuIsPFMuon;
    result &= iEvent.getByLabel( tMuIsStandaloneMuon, hMuIsStandaloneMuon );
    if ( hMuIsStandaloneMuon.isValid() ) MuIsStandaloneMuon = *hMuIsStandaloneMuon;
    result &= iEvent.getByLabel( tMuIsTM2DCompLoose, hMuIsTM2DCompLoose );
    if ( hMuIsTM2DCompLoose.isValid() ) MuIsTM2DCompLoose = *hMuIsTM2DCompLoose;
    result &= iEvent.getByLabel( tMuIsTM2DCompTight, hMuIsTM2DCompTight );
    if ( hMuIsTM2DCompTight.isValid() ) MuIsTM2DCompTight = *hMuIsTM2DCompTight;
    result &= iEvent.getByLabel( tMuIsTMLSAngLoose, hMuIsTMLSAngLoose );
    if ( hMuIsTMLSAngLoose.isValid() ) MuIsTMLSAngLoose = *hMuIsTMLSAngLoose;
    result &= iEvent.getByLabel( tMuIsTMLSAngTight, hMuIsTMLSAngTight );
    if ( hMuIsTMLSAngTight.isValid() ) MuIsTMLSAngTight = *hMuIsTMLSAngTight;
    result &= iEvent.getByLabel( tMuIsTMLSLoose, hMuIsTMLSLoose );
    if ( hMuIsTMLSLoose.isValid() ) MuIsTMLSLoose = *hMuIsTMLSLoose;
    result &= iEvent.getByLabel( tMuIsTMLSOptLowPtLoose, hMuIsTMLSOptLowPtLoose );
    if ( hMuIsTMLSOptLowPtLoose.isValid() ) MuIsTMLSOptLowPtLoose = *hMuIsTMLSOptLowPtLoose;
    result &= iEvent.getByLabel( tMuIsTMLSTight, hMuIsTMLSTight );
    if ( hMuIsTMLSTight.isValid() ) MuIsTMLSTight = *hMuIsTMLSTight;
    result &= iEvent.getByLabel( tMuIsTMOneStationAngLoose, hMuIsTMOneStationAngLoose );
    if ( hMuIsTMOneStationAngLoose.isValid() ) MuIsTMOneStationAngLoose = *hMuIsTMOneStationAngLoose;
    result &= iEvent.getByLabel( tMuIsTMOneStationAngTight, hMuIsTMOneStationAngTight );
    if ( hMuIsTMOneStationAngTight.isValid() ) MuIsTMOneStationAngTight = *hMuIsTMOneStationAngTight;
    result &= iEvent.getByLabel( tMuIsTMOneStationLoose, hMuIsTMOneStationLoose );
    if ( hMuIsTMOneStationLoose.isValid() ) MuIsTMOneStationLoose = *hMuIsTMOneStationLoose;
    result &= iEvent.getByLabel( tMuIsTMOneStationTight, hMuIsTMOneStationTight );
    if ( hMuIsTMOneStationTight.isValid() ) MuIsTMOneStationTight = *hMuIsTMOneStationTight;
    result &= iEvent.getByLabel( tMuIsTrackerMuon, hMuIsTrackerMuon );
    if ( hMuIsTrackerMuon.isValid() ) MuIsTrackerMuon = *hMuIsTrackerMuon;
    result &= iEvent.getByLabel( tMuIsTrkMuonArbitrated, hMuIsTrkMuonArbitrated );
    if ( hMuIsTrkMuonArbitrated.isValid() ) MuIsTrkMuonArbitrated = *hMuIsTrkMuonArbitrated;
    result &= iEvent.getByLabel( tMuIso03EMVetoEt, hMuIso03EMVetoEt );
    if ( hMuIso03EMVetoEt.isValid() ) MuIso03EMVetoEt = *hMuIso03EMVetoEt;
    result &= iEvent.getByLabel( tMuIso03EmEt, hMuIso03EmEt );
    if ( hMuIso03EmEt.isValid() ) MuIso03EmEt = *hMuIso03EmEt;
    result &= iEvent.getByLabel( tMuIso03HadEt, hMuIso03HadEt );
    if ( hMuIso03HadEt.isValid() ) MuIso03HadEt = *hMuIso03HadEt;
    result &= iEvent.getByLabel( tMuIso03HadVetoEt, hMuIso03HadVetoEt );
    if ( hMuIso03HadVetoEt.isValid() ) MuIso03HadVetoEt = *hMuIso03HadVetoEt;
    result &= iEvent.getByLabel( tMuIso03SumPt, hMuIso03SumPt );
    if ( hMuIso03SumPt.isValid() ) MuIso03SumPt = *hMuIso03SumPt;
    result &= iEvent.getByLabel( tMuIso05EmEt, hMuIso05EmEt );
    if ( hMuIso05EmEt.isValid() ) MuIso05EmEt = *hMuIso05EmEt;
    result &= iEvent.getByLabel( tMuIso05HadEt, hMuIso05HadEt );
    if ( hMuIso05HadEt.isValid() ) MuIso05HadEt = *hMuIso05HadEt;
    result &= iEvent.getByLabel( tMuIso05SumPt, hMuIso05SumPt );
    if ( hMuIso05SumPt.isValid() ) MuIso05SumPt = *hMuIso05SumPt;
    result &= iEvent.getByLabel( tMuIsoMVA, hMuIsoMVA );
    if ( hMuIsoMVA.isValid() ) MuIsoMVA = *hMuIsoMVA;
    result &= iEvent.getByLabel( tMuJESCorrMET, hMuJESCorrMET );
    if ( hMuJESCorrMET.isValid() ) MuJESCorrMET = *hMuJESCorrMET;
    result &= iEvent.getByLabel( tMuJESCorrMETphi, hMuJESCorrMETphi );
    if ( hMuJESCorrMETphi.isValid() ) MuJESCorrMETphi = *hMuJESCorrMETphi;
    result &= iEvent.getByLabel( tMuJESCorrMETpx, hMuJESCorrMETpx );
    if ( hMuJESCorrMETpx.isValid() ) MuJESCorrMETpx = *hMuJESCorrMETpx;
    result &= iEvent.getByLabel( tMuJESCorrMETpy, hMuJESCorrMETpy );
    if ( hMuJESCorrMETpy.isValid() ) MuJESCorrMETpy = *hMuJESCorrMETpy;
    result &= iEvent.getByLabel( tMuNChambers, hMuNChambers );
    if ( hMuNChambers.isValid() ) MuNChambers = *hMuNChambers;
    result &= iEvent.getByLabel( tMuNChi2, hMuNChi2 );
    if ( hMuNChi2.isValid() ) MuNChi2 = *hMuNChi2;
    result &= iEvent.getByLabel( tMuNGlHits, hMuNGlHits );
    if ( hMuNGlHits.isValid() ) MuNGlHits = *hMuNGlHits;
    result &= iEvent.getByLabel( tMuNGlMuHits, hMuNGlMuHits );
    if ( hMuNGlMuHits.isValid() ) MuNGlMuHits = *hMuNGlMuHits;
    result &= iEvent.getByLabel( tMuNMatchedStations, hMuNMatchedStations );
    if ( hMuNMatchedStations.isValid() ) MuNMatchedStations = *hMuNMatchedStations;
    result &= iEvent.getByLabel( tMuNMatches, hMuNMatches );
    if ( hMuNMatches.isValid() ) MuNMatches = *hMuNMatches;
    result &= iEvent.getByLabel( tMuNMuHits, hMuNMuHits );
    if ( hMuNMuHits.isValid() ) MuNMuHits = *hMuNMuHits;
    result &= iEvent.getByLabel( tMuNPxHits, hMuNPxHits );
    if ( hMuNPxHits.isValid() ) MuNPxHits = *hMuNPxHits;
    result &= iEvent.getByLabel( tMuNSiLayers, hMuNSiLayers );
    if ( hMuNSiLayers.isValid() ) MuNSiLayers = *hMuNSiLayers;
    result &= iEvent.getByLabel( tMuNTkHits, hMuNTkHits );
    if ( hMuNTkHits.isValid() ) MuNTkHits = *hMuNTkHits;
    result &= iEvent.getByLabel( tMuPfIsoR03ChHad, hMuPfIsoR03ChHad );
    if ( hMuPfIsoR03ChHad.isValid() ) MuPfIsoR03ChHad = *hMuPfIsoR03ChHad;
    result &= iEvent.getByLabel( tMuPfIsoR03NeHad, hMuPfIsoR03NeHad );
    if ( hMuPfIsoR03NeHad.isValid() ) MuPfIsoR03NeHad = *hMuPfIsoR03NeHad;
    result &= iEvent.getByLabel( tMuPfIsoR03NeHadHighThresh, hMuPfIsoR03NeHadHighThresh );
    if ( hMuPfIsoR03NeHadHighThresh.isValid() ) MuPfIsoR03NeHadHighThresh = *hMuPfIsoR03NeHadHighThresh;
    result &= iEvent.getByLabel( tMuPfIsoR03Photon, hMuPfIsoR03Photon );
    if ( hMuPfIsoR03Photon.isValid() ) MuPfIsoR03Photon = *hMuPfIsoR03Photon;
    result &= iEvent.getByLabel( tMuPfIsoR03PhotonHighThresh, hMuPfIsoR03PhotonHighThresh );
    if ( hMuPfIsoR03PhotonHighThresh.isValid() ) MuPfIsoR03PhotonHighThresh = *hMuPfIsoR03PhotonHighThresh;
    result &= iEvent.getByLabel( tMuPfIsoR03SumPUPt, hMuPfIsoR03SumPUPt );
    if ( hMuPfIsoR03SumPUPt.isValid() ) MuPfIsoR03SumPUPt = *hMuPfIsoR03SumPUPt;
    result &= iEvent.getByLabel( tMuPfIsoR04ChHad, hMuPfIsoR04ChHad );
    if ( hMuPfIsoR04ChHad.isValid() ) MuPfIsoR04ChHad = *hMuPfIsoR04ChHad;
    result &= iEvent.getByLabel( tMuPfIsoR04NeHad, hMuPfIsoR04NeHad );
    if ( hMuPfIsoR04NeHad.isValid() ) MuPfIsoR04NeHad = *hMuPfIsoR04NeHad;
    result &= iEvent.getByLabel( tMuPfIsoR04NeHadHighThresh, hMuPfIsoR04NeHadHighThresh );
    if ( hMuPfIsoR04NeHadHighThresh.isValid() ) MuPfIsoR04NeHadHighThresh = *hMuPfIsoR04NeHadHighThresh;
    result &= iEvent.getByLabel( tMuPfIsoR04Photon, hMuPfIsoR04Photon );
    if ( hMuPfIsoR04Photon.isValid() ) MuPfIsoR04Photon = *hMuPfIsoR04Photon;
    result &= iEvent.getByLabel( tMuPfIsoR04PhotonHighThresh, hMuPfIsoR04PhotonHighThresh );
    if ( hMuPfIsoR04PhotonHighThresh.isValid() ) MuPfIsoR04PhotonHighThresh = *hMuPfIsoR04PhotonHighThresh;
    result &= iEvent.getByLabel( tMuPfIsoR04SumPUPt, hMuPfIsoR04SumPUPt );
    if ( hMuPfIsoR04SumPUPt.isValid() ) MuPfIsoR04SumPUPt = *hMuPfIsoR04SumPUPt;
    result &= iEvent.getByLabel( tMuPhi, hMuPhi );
    if ( hMuPhi.isValid() ) MuPhi = *hMuPhi;
    result &= iEvent.getByLabel( tMuPt, hMuPt );
    if ( hMuPt.isValid() ) MuPt = *hMuPt;
    result &= iEvent.getByLabel( tMuPtE, hMuPtE );
    if ( hMuPtE.isValid() ) MuPtE = *hMuPtE;
    result &= iEvent.getByLabel( tMuPx, hMuPx );
    if ( hMuPx.isValid() ) MuPx = *hMuPx;
    result &= iEvent.getByLabel( tMuPy, hMuPy );
    if ( hMuPy.isValid() ) MuPy = *hMuPy;
    result &= iEvent.getByLabel( tMuPz, hMuPz );
    if ( hMuPz.isValid() ) MuPz = *hMuPz;
    result &= iEvent.getByLabel( tMuRelIso03, hMuRelIso03 );
    if ( hMuRelIso03.isValid() ) MuRelIso03 = *hMuRelIso03;
    result &= iEvent.getByLabel( tMuSegmComp, hMuSegmComp );
    if ( hMuSegmComp.isValid() ) MuSegmComp = *hMuSegmComp;
    result &= iEvent.getByLabel( tMuTkD0E, hMuTkD0E );
    if ( hMuTkD0E.isValid() ) MuTkD0E = *hMuTkD0E;
    result &= iEvent.getByLabel( tMuTkDzE, hMuTkDzE );
    if ( hMuTkDzE.isValid() ) MuTkDzE = *hMuTkDzE;
    result &= iEvent.getByLabel( tMuTkPtE, hMuTkPtE );
    if ( hMuTkPtE.isValid() ) MuTkPtE = *hMuTkPtE;
    result &= iEvent.getByLabel( tMumuonPFIsoChHad03, hMumuonPFIsoChHad03 );
    if ( hMumuonPFIsoChHad03.isValid() ) MumuonPFIsoChHad03 = *hMumuonPFIsoChHad03;
    result &= iEvent.getByLabel( tMumuonPFIsoChHad04, hMumuonPFIsoChHad04 );
    if ( hMumuonPFIsoChHad04.isValid() ) MumuonPFIsoChHad04 = *hMumuonPFIsoChHad04;
    result &= iEvent.getByLabel( tMumuonPFIsoNHad03, hMumuonPFIsoNHad03 );
    if ( hMumuonPFIsoNHad03.isValid() ) MumuonPFIsoNHad03 = *hMumuonPFIsoNHad03;
    result &= iEvent.getByLabel( tMumuonPFIsoNHad04, hMumuonPFIsoNHad04 );
    if ( hMumuonPFIsoNHad04.isValid() ) MumuonPFIsoNHad04 = *hMumuonPFIsoNHad04;
    result &= iEvent.getByLabel( tMumuonPFIsoPhoton03, hMumuonPFIsoPhoton03 );
    if ( hMumuonPFIsoPhoton03.isValid() ) MumuonPFIsoPhoton03 = *hMumuonPFIsoPhoton03;
    result &= iEvent.getByLabel( tMumuonPFIsoPhoton04, hMumuonPFIsoPhoton04 );
    if ( hMumuonPFIsoPhoton04.isValid() ) MumuonPFIsoPhoton04 = *hMumuonPFIsoPhoton04;
    result &= iEvent.getByLabel( tMumuonRadPFIsoChHad03, hMumuonRadPFIsoChHad03 );
    if ( hMumuonRadPFIsoChHad03.isValid() ) MumuonRadPFIsoChHad03 = *hMumuonRadPFIsoChHad03;
    result &= iEvent.getByLabel( tMumuonRadPFIsoChHad04, hMumuonRadPFIsoChHad04 );
    if ( hMumuonRadPFIsoChHad04.isValid() ) MumuonRadPFIsoChHad04 = *hMumuonRadPFIsoChHad04;
    result &= iEvent.getByLabel( tMumuonRadPFIsoNHad03, hMumuonRadPFIsoNHad03 );
    if ( hMumuonRadPFIsoNHad03.isValid() ) MumuonRadPFIsoNHad03 = *hMumuonRadPFIsoNHad03;
    result &= iEvent.getByLabel( tMumuonRadPFIsoNHad04, hMumuonRadPFIsoNHad04 );
    if ( hMumuonRadPFIsoNHad04.isValid() ) MumuonRadPFIsoNHad04 = *hMumuonRadPFIsoNHad04;
    result &= iEvent.getByLabel( tMumuonRadPFIsoPhoton03, hMumuonRadPFIsoPhoton03 );
    if ( hMumuonRadPFIsoPhoton03.isValid() ) MumuonRadPFIsoPhoton03 = *hMumuonRadPFIsoPhoton03;
    result &= iEvent.getByLabel( tMumuonRadPFIsoPhoton04, hMumuonRadPFIsoPhoton04 );
    if ( hMumuonRadPFIsoPhoton04.isValid() ) MumuonRadPFIsoPhoton04 = *hMumuonRadPFIsoPhoton04;
    result &= iEvent.getByLabel( tNCaloTowers, hNCaloTowers );
    if ( hNCaloTowers.isValid() ) NCaloTowers = *hNCaloTowers;
    result &= iEvent.getByLabel( tNEBhits, hNEBhits );
    if ( hNEBhits.isValid() ) NEBhits = *hNEBhits;
    result &= iEvent.getByLabel( tNEles, hNEles );
    if ( hNEles.isValid() ) NEles = *hNEles;
    result &= iEvent.getByLabel( tNElesTot, hNElesTot );
    if ( hNElesTot.isValid() ) NElesTot = *hNElesTot;
    result &= iEvent.getByLabel( tNGMus, hNGMus );
    if ( hNGMus.isValid() ) NGMus = *hNGMus;
    result &= iEvent.getByLabel( tNGenJets, hNGenJets );
    if ( hNGenJets.isValid() ) NGenJets = *hNGenJets;
    result &= iEvent.getByLabel( tNGenLeptons, hNGenLeptons );
    if ( hNGenLeptons.isValid() ) NGenLeptons = *hNGenLeptons;
    result &= iEvent.getByLabel( tNGenPhotons, hNGenPhotons );
    if ( hNGenPhotons.isValid() ) NGenPhotons = *hNGenPhotons;
    result &= iEvent.getByLabel( tNGoodSuperClusters, hNGoodSuperClusters );
    if ( hNGoodSuperClusters.isValid() ) NGoodSuperClusters = *hNGoodSuperClusters;
    result &= iEvent.getByLabel( tNHLTObjs, hNHLTObjs );
    if ( hNHLTObjs.isValid() ) NHLTObjs = *hNHLTObjs;
    result &= iEvent.getByLabel( tNJets, hNJets );
    if ( hNJets.isValid() ) NJets = *hNJets;
    result &= iEvent.getByLabel( tNJetsTot, hNJetsTot );
    if ( hNJetsTot.isValid() ) NJetsTot = *hNJetsTot;
    result &= iEvent.getByLabel( tNMus, hNMus );
    if ( hNMus.isValid() ) NMus = *hNMus;
    result &= iEvent.getByLabel( tNMusTot, hNMusTot );
    if ( hNMusTot.isValid() ) NMusTot = *hNMusTot;
    result &= iEvent.getByLabel( tNPdfs, hNPdfs );
    if ( hNPdfs.isValid() ) NPdfs = *hNPdfs;
    result &= iEvent.getByLabel( tNPfCand, hNPfCand );
    if ( hNPfCand.isValid() ) NPfCand = *hNPfCand;
    result &= iEvent.getByLabel( tNPhotons, hNPhotons );
    if ( hNPhotons.isValid() ) NPhotons = *hNPhotons;
    result &= iEvent.getByLabel( tNPhotonsTot, hNPhotonsTot );
    if ( hNPhotonsTot.isValid() ) NPhotonsTot = *hNPhotonsTot;
    result &= iEvent.getByLabel( tNSuperClusters, hNSuperClusters );
    if ( hNSuperClusters.isValid() ) NSuperClusters = *hNSuperClusters;
    result &= iEvent.getByLabel( tNTMus, hNTMus );
    if ( hNTMus.isValid() ) NTMus = *hNTMus;
    result &= iEvent.getByLabel( tNTracks, hNTracks );
    if ( hNTracks.isValid() ) NTracks = *hNTracks;
    result &= iEvent.getByLabel( tNTracksTot, hNTracksTot );
    if ( hNTracksTot.isValid() ) NTracksTot = *hNTracksTot;
    result &= iEvent.getByLabel( tNVrtx, hNVrtx );
    if ( hNVrtx.isValid() ) NVrtx = *hNVrtx;
    result &= iEvent.getByLabel( tNXtals, hNXtals );
    if ( hNXtals.isValid() ) NXtals = *hNXtals;
    result &= iEvent.getByLabel( tNconv, hNconv );
    if ( hNconv.isValid() ) Nconv = *hNconv;
    result &= iEvent.getByLabel( tNgv, hNgv );
    if ( hNgv.isValid() ) Ngv = *hNgv;
    result &= iEvent.getByLabel( tPDFID1, hPDFID1 );
    if ( hPDFID1.isValid() ) PDFID1 = *hPDFID1;
    result &= iEvent.getByLabel( tPDFID2, hPDFID2 );
    if ( hPDFID2.isValid() ) PDFID2 = *hPDFID2;
    result &= iEvent.getByLabel( tPDFScalePDF, hPDFScalePDF );
    if ( hPDFScalePDF.isValid() ) PDFScalePDF = *hPDFScalePDF;
    result &= iEvent.getByLabel( tPDFx1, hPDFx1 );
    if ( hPDFx1.isValid() ) PDFx1 = *hPDFx1;
    result &= iEvent.getByLabel( tPDFx2, hPDFx2 );
    if ( hPDFx2.isValid() ) PDFx2 = *hPDFx2;
    result &= iEvent.getByLabel( tPDFxPDF1, hPDFxPDF1 );
    if ( hPDFxPDF1.isValid() ) PDFxPDF1 = *hPDFxPDF1;
    result &= iEvent.getByLabel( tPDFxPDF2, hPDFxPDF2 );
    if ( hPDFxPDF2.isValid() ) PDFxPDF2 = *hPDFxPDF2;
    result &= iEvent.getByLabel( tPFCE, hPFCE );
    if ( hPFCE.isValid() ) PFCE = *hPFCE;
    result &= iEvent.getByLabel( tPFCEt, hPFCEt );
    if ( hPFCEt.isValid() ) PFCEt = *hPFCEt;
    result &= iEvent.getByLabel( tPFCEta, hPFCEta );
    if ( hPFCEta.isValid() ) PFCEta = *hPFCEta;
    result &= iEvent.getByLabel( tPFCHSJArea, hPFCHSJArea );
    if ( hPFCHSJArea.isValid() ) PFCHSJArea = *hPFCHSJArea;
    result &= iEvent.getByLabel( tPFCHSJChEmfrac, hPFCHSJChEmfrac );
    if ( hPFCHSJChEmfrac.isValid() ) PFCHSJChEmfrac = *hPFCHSJChEmfrac;
    result &= iEvent.getByLabel( tPFCHSJChHadfrac, hPFCHSJChHadfrac );
    if ( hPFCHSJChHadfrac.isValid() ) PFCHSJChHadfrac = *hPFCHSJChHadfrac;
    result &= iEvent.getByLabel( tPFCHSJChMufrac, hPFCHSJChMufrac );
    if ( hPFCHSJChMufrac.isValid() ) PFCHSJChMufrac = *hPFCHSJChMufrac;
    result &= iEvent.getByLabel( tPFCHSJChMult, hPFCHSJChMult );
    if ( hPFCHSJChMult.isValid() ) PFCHSJChMult = *hPFCHSJChMult;
    result &= iEvent.getByLabel( tPFCHSJE, hPFCHSJE );
    if ( hPFCHSJE.isValid() ) PFCHSJE = *hPFCHSJE;
    result &= iEvent.getByLabel( tPFCHSJEt, hPFCHSJEt );
    if ( hPFCHSJEt.isValid() ) PFCHSJEt = *hPFCHSJEt;
    result &= iEvent.getByLabel( tPFCHSJEta, hPFCHSJEta );
    if ( hPFCHSJEta.isValid() ) PFCHSJEta = *hPFCHSJEta;
    result &= iEvent.getByLabel( tPFCHSJFlavour, hPFCHSJFlavour );
    if ( hPFCHSJFlavour.isValid() ) PFCHSJFlavour = *hPFCHSJFlavour;
    result &= iEvent.getByLabel( tPFCHSJHFEMfrac, hPFCHSJHFEMfrac );
    if ( hPFCHSJHFEMfrac.isValid() ) PFCHSJHFEMfrac = *hPFCHSJHFEMfrac;
    result &= iEvent.getByLabel( tPFCHSJHFHadfrac, hPFCHSJHFHadfrac );
    if ( hPFCHSJHFHadfrac.isValid() ) PFCHSJHFHadfrac = *hPFCHSJHFHadfrac;
    result &= iEvent.getByLabel( tPFCHSJIDLoose, hPFCHSJIDLoose );
    if ( hPFCHSJIDLoose.isValid() ) PFCHSJIDLoose = *hPFCHSJIDLoose;
    result &= iEvent.getByLabel( tPFCHSJL1FastJetScale, hPFCHSJL1FastJetScale );
    if ( hPFCHSJL1FastJetScale.isValid() ) PFCHSJL1FastJetScale = *hPFCHSJL1FastJetScale;
    result &= iEvent.getByLabel( tPFCHSJNConstituents, hPFCHSJNConstituents );
    if ( hPFCHSJNConstituents.isValid() ) PFCHSJNConstituents = *hPFCHSJNConstituents;
    result &= iEvent.getByLabel( tPFCHSJNeuEmfrac, hPFCHSJNeuEmfrac );
    if ( hPFCHSJNeuEmfrac.isValid() ) PFCHSJNeuEmfrac = *hPFCHSJNeuEmfrac;
    result &= iEvent.getByLabel( tPFCHSJNeuHadfrac, hPFCHSJNeuHadfrac );
    if ( hPFCHSJNeuHadfrac.isValid() ) PFCHSJNeuHadfrac = *hPFCHSJNeuHadfrac;
    result &= iEvent.getByLabel( tPFCHSJNeuMult, hPFCHSJNeuMult );
    if ( hPFCHSJNeuMult.isValid() ) PFCHSJNeuMult = *hPFCHSJNeuMult;
    result &= iEvent.getByLabel( tPFCHSJPhi, hPFCHSJPhi );
    if ( hPFCHSJPhi.isValid() ) PFCHSJPhi = *hPFCHSJPhi;
    result &= iEvent.getByLabel( tPFCHSJPhofrac, hPFCHSJPhofrac );
    if ( hPFCHSJPhofrac.isValid() ) PFCHSJPhofrac = *hPFCHSJPhofrac;
    result &= iEvent.getByLabel( tPFCHSJPt, hPFCHSJPt );
    if ( hPFCHSJPt.isValid() ) PFCHSJPt = *hPFCHSJPt;
    result &= iEvent.getByLabel( tPFCHSJPx, hPFCHSJPx );
    if ( hPFCHSJPx.isValid() ) PFCHSJPx = *hPFCHSJPx;
    result &= iEvent.getByLabel( tPFCHSJPy, hPFCHSJPy );
    if ( hPFCHSJPy.isValid() ) PFCHSJPy = *hPFCHSJPy;
    result &= iEvent.getByLabel( tPFCHSJPz, hPFCHSJPz );
    if ( hPFCHSJPz.isValid() ) PFCHSJPz = *hPFCHSJPz;
    result &= iEvent.getByLabel( tPFCHSJScale, hPFCHSJScale );
    if ( hPFCHSJScale.isValid() ) PFCHSJScale = *hPFCHSJScale;
    result &= iEvent.getByLabel( tPFCHSJcombinedSecondaryVertexBJetTags, hPFCHSJcombinedSecondaryVertexBJetTags );
    if ( hPFCHSJcombinedSecondaryVertexBJetTags.isValid() ) PFCHSJcombinedSecondaryVertexBJetTags = *hPFCHSJcombinedSecondaryVertexBJetTags;
    result &= iEvent.getByLabel( tPFCHSJcombinedSecondaryVertexMVABJetTags, hPFCHSJcombinedSecondaryVertexMVABJetTags );
    if ( hPFCHSJcombinedSecondaryVertexMVABJetTags.isValid() ) PFCHSJcombinedSecondaryVertexMVABJetTags = *hPFCHSJcombinedSecondaryVertexMVABJetTags;
    result &= iEvent.getByLabel( tPFCHSJjetBProbabilityBJetTags, hPFCHSJjetBProbabilityBJetTags );
    if ( hPFCHSJjetBProbabilityBJetTags.isValid() ) PFCHSJjetBProbabilityBJetTags = *hPFCHSJjetBProbabilityBJetTags;
    result &= iEvent.getByLabel( tPFCHSJjetProbabilityBJetTags, hPFCHSJjetProbabilityBJetTags );
    if ( hPFCHSJjetProbabilityBJetTags.isValid() ) PFCHSJjetProbabilityBJetTags = *hPFCHSJjetProbabilityBJetTags;
    result &= iEvent.getByLabel( tPFCHSJsimpleSecondaryVertexHighEffBJetTags, hPFCHSJsimpleSecondaryVertexHighEffBJetTags );
    if ( hPFCHSJsimpleSecondaryVertexHighEffBJetTags.isValid() ) PFCHSJsimpleSecondaryVertexHighEffBJetTags = *hPFCHSJsimpleSecondaryVertexHighEffBJetTags;
    result &= iEvent.getByLabel( tPFCHSJsimpleSecondaryVertexHighPurBJetTags, hPFCHSJsimpleSecondaryVertexHighPurBJetTags );
    if ( hPFCHSJsimpleSecondaryVertexHighPurBJetTags.isValid() ) PFCHSJsimpleSecondaryVertexHighPurBJetTags = *hPFCHSJsimpleSecondaryVertexHighPurBJetTags;
    result &= iEvent.getByLabel( tPFCHSJtrackCountingHighEffBJetTags, hPFCHSJtrackCountingHighEffBJetTags );
    if ( hPFCHSJtrackCountingHighEffBJetTags.isValid() ) PFCHSJtrackCountingHighEffBJetTags = *hPFCHSJtrackCountingHighEffBJetTags;
    result &= iEvent.getByLabel( tPFCHSJtrackCountingHighPurBJetTags, hPFCHSJtrackCountingHighPurBJetTags );
    if ( hPFCHSJtrackCountingHighPurBJetTags.isValid() ) PFCHSJtrackCountingHighPurBJetTags = *hPFCHSJtrackCountingHighPurBJetTags;
    result &= iEvent.getByLabel( tPFCHSNJets, hPFCHSNJets );
    if ( hPFCHSNJets.isValid() ) PFCHSNJets = *hPFCHSNJets;
    result &= iEvent.getByLabel( tPFCNCandidates, hPFCNCandidates );
    if ( hPFCNCandidates.isValid() ) PFCNCandidates = *hPFCNCandidates;
    result &= iEvent.getByLabel( tPFCPhi, hPFCPhi );
    if ( hPFCPhi.isValid() ) PFCPhi = *hPFCPhi;
    result &= iEvent.getByLabel( tPFCPt, hPFCPt );
    if ( hPFCPt.isValid() ) PFCPt = *hPFCPt;
    result &= iEvent.getByLabel( tPFCPx, hPFCPx );
    if ( hPFCPx.isValid() ) PFCPx = *hPFCPx;
    result &= iEvent.getByLabel( tPFCPy, hPFCPy );
    if ( hPFCPy.isValid() ) PFCPy = *hPFCPy;
    result &= iEvent.getByLabel( tPFCPz, hPFCPz );
    if ( hPFCPz.isValid() ) PFCPz = *hPFCPz;
    result &= iEvent.getByLabel( tPFCType, hPFCType );
    if ( hPFCType.isValid() ) PFCType = *hPFCType;
    result &= iEvent.getByLabel( tPFCVx, hPFCVx );
    if ( hPFCVx.isValid() ) PFCVx = *hPFCVx;
    result &= iEvent.getByLabel( tPFCVy, hPFCVy );
    if ( hPFCVy.isValid() ) PFCVy = *hPFCVy;
    result &= iEvent.getByLabel( tPFCVz, hPFCVz );
    if ( hPFCVz.isValid() ) PFCVz = *hPFCVz;
    result &= iEvent.getByLabel( tPFMET, hPFMET );
    if ( hPFMET.isValid() ) PFMET = *hPFMET;
    result &= iEvent.getByLabel( tPFMETSignificance, hPFMETSignificance );
    if ( hPFMETSignificance.isValid() ) PFMETSignificance = *hPFMETSignificance;
    result &= iEvent.getByLabel( tPFMETphi, hPFMETphi );
    if ( hPFMETphi.isValid() ) PFMETphi = *hPFMETphi;
    result &= iEvent.getByLabel( tPFMETpx, hPFMETpx );
    if ( hPFMETpx.isValid() ) PFMETpx = *hPFMETpx;
    result &= iEvent.getByLabel( tPFMETpy, hPFMETpy );
    if ( hPFMETpy.isValid() ) PFMETpy = *hPFMETpy;
    result &= iEvent.getByLabel( tPFSumEt, hPFSumEt );
    if ( hPFSumEt.isValid() ) PFSumEt = *hPFSumEt;
    result &= iEvent.getByLabel( tPFType1MET, hPFType1MET );
    if ( hPFType1MET.isValid() ) PFType1MET = *hPFType1MET;
    result &= iEvent.getByLabel( tPFType1METSignificance, hPFType1METSignificance );
    if ( hPFType1METSignificance.isValid() ) PFType1METSignificance = *hPFType1METSignificance;
    result &= iEvent.getByLabel( tPFType1METphi, hPFType1METphi );
    if ( hPFType1METphi.isValid() ) PFType1METphi = *hPFType1METphi;
    result &= iEvent.getByLabel( tPFType1METpx, hPFType1METpx );
    if ( hPFType1METpx.isValid() ) PFType1METpx = *hPFType1METpx;
    result &= iEvent.getByLabel( tPFType1METpy, hPFType1METpy );
    if ( hPFType1METpy.isValid() ) PFType1METpy = *hPFType1METpy;
    result &= iEvent.getByLabel( tPFType1SumEt, hPFType1SumEt );
    if ( hPFType1SumEt.isValid() ) PFType1SumEt = *hPFType1SumEt;
    result &= iEvent.getByLabel( tPUOOTnumInteractionsEarly, hPUOOTnumInteractionsEarly );
    if ( hPUOOTnumInteractionsEarly.isValid() ) PUOOTnumInteractionsEarly = *hPUOOTnumInteractionsEarly;
    result &= iEvent.getByLabel( tPUOOTnumInteractionsLate, hPUOOTnumInteractionsLate );
    if ( hPUOOTnumInteractionsLate.isValid() ) PUOOTnumInteractionsLate = *hPUOOTnumInteractionsLate;
    result &= iEvent.getByLabel( tPUWeightInTime, hPUWeightInTime );
    if ( hPUWeightInTime.isValid() ) PUWeightInTime = *hPUWeightInTime;
    result &= iEvent.getByLabel( tPUWeightTotal, hPUWeightTotal );
    if ( hPUWeightTotal.isValid() ) PUWeightTotal = *hPUWeightTotal;
    result &= iEvent.getByLabel( tPUnTrksHighPt, hPUnTrksHighPt );
    if ( hPUnTrksHighPt.isValid() ) PUnTrksHighPt = *hPUnTrksHighPt;
    result &= iEvent.getByLabel( tPUnTrksLowPt, hPUnTrksLowPt );
    if ( hPUnTrksLowPt.isValid() ) PUnTrksLowPt = *hPUnTrksLowPt;
    result &= iEvent.getByLabel( tPUnumFilled, hPUnumFilled );
    if ( hPUnumFilled.isValid() ) PUnumFilled = *hPUnumFilled;
    result &= iEvent.getByLabel( tPUnumInteractions, hPUnumInteractions );
    if ( hPUnumInteractions.isValid() ) PUnumInteractions = *hPUnumInteractions;
    result &= iEvent.getByLabel( tPUnumTrueInteractions, hPUnumTrueInteractions );
    if ( hPUnumTrueInteractions.isValid() ) PUnumTrueInteractions = *hPUnumTrueInteractions;
    result &= iEvent.getByLabel( tPUsumPtHighPt, hPUsumPtHighPt );
    if ( hPUsumPtHighPt.isValid() ) PUsumPtHighPt = *hPUsumPtHighPt;
    result &= iEvent.getByLabel( tPUsumPtLowPt, hPUsumPtLowPt );
    if ( hPUsumPtLowPt.isValid() ) PUsumPtLowPt = *hPUsumPtLowPt;
    result &= iEvent.getByLabel( tPUzPositions, hPUzPositions );
    if ( hPUzPositions.isValid() ) PUzPositions = *hPUzPositions;
    result &= iEvent.getByLabel( tPfCandEnergy, hPfCandEnergy );
    if ( hPfCandEnergy.isValid() ) PfCandEnergy = *hPfCandEnergy;
    result &= iEvent.getByLabel( tPfCandEta, hPfCandEta );
    if ( hPfCandEta.isValid() ) PfCandEta = *hPfCandEta;
    result &= iEvent.getByLabel( tPfCandHasHitInFirstPixelLayer, hPfCandHasHitInFirstPixelLayer );
    if ( hPfCandHasHitInFirstPixelLayer.isValid() ) PfCandHasHitInFirstPixelLayer = *hPfCandHasHitInFirstPixelLayer;
    result &= iEvent.getByLabel( tPfCandPdgId, hPfCandPdgId );
    if ( hPfCandPdgId.isValid() ) PfCandPdgId = *hPfCandPdgId;
    result &= iEvent.getByLabel( tPfCandPhi, hPfCandPhi );
    if ( hPfCandPhi.isValid() ) PfCandPhi = *hPfCandPhi;
    result &= iEvent.getByLabel( tPfCandPt, hPfCandPt );
    if ( hPfCandPt.isValid() ) PfCandPt = *hPfCandPt;
    result &= iEvent.getByLabel( tPfCandTrackRefPx, hPfCandTrackRefPx );
    if ( hPfCandTrackRefPx.isValid() ) PfCandTrackRefPx = *hPfCandTrackRefPx;
    result &= iEvent.getByLabel( tPfCandTrackRefPy, hPfCandTrackRefPy );
    if ( hPfCandTrackRefPy.isValid() ) PfCandTrackRefPy = *hPfCandTrackRefPy;
    result &= iEvent.getByLabel( tPfCandTrackRefPz, hPfCandTrackRefPz );
    if ( hPfCandTrackRefPz.isValid() ) PfCandTrackRefPz = *hPfCandTrackRefPz;
    result &= iEvent.getByLabel( tPfCandVx, hPfCandVx );
    if ( hPfCandVx.isValid() ) PfCandVx = *hPfCandVx;
    result &= iEvent.getByLabel( tPfCandVy, hPfCandVy );
    if ( hPfCandVy.isValid() ) PfCandVy = *hPfCandVy;
    result &= iEvent.getByLabel( tPfCandVz, hPfCandVz );
    if ( hPfCandVz.isValid() ) PfCandVz = *hPfCandVz;
    result &= iEvent.getByLabel( tPhoCaloPositionX, hPhoCaloPositionX );
    if ( hPhoCaloPositionX.isValid() ) PhoCaloPositionX = *hPhoCaloPositionX;
    result &= iEvent.getByLabel( tPhoCaloPositionY, hPhoCaloPositionY );
    if ( hPhoCaloPositionY.isValid() ) PhoCaloPositionY = *hPhoCaloPositionY;
    result &= iEvent.getByLabel( tPhoCaloPositionZ, hPhoCaloPositionZ );
    if ( hPhoCaloPositionZ.isValid() ) PhoCaloPositionZ = *hPhoCaloPositionZ;
    result &= iEvent.getByLabel( tPhoChargedHadronIso, hPhoChargedHadronIso );
    if ( hPhoChargedHadronIso.isValid() ) PhoChargedHadronIso = *hPhoChargedHadronIso;
    result &= iEvent.getByLabel( tPhoCone01ChargedHadronIsodR015dEta0pt0PFnoPU, hPhoCone01ChargedHadronIsodR015dEta0pt0PFnoPU );
    if ( hPhoCone01ChargedHadronIsodR015dEta0pt0PFnoPU.isValid() ) PhoCone01ChargedHadronIsodR015dEta0pt0PFnoPU = *hPhoCone01ChargedHadronIsodR015dEta0pt0PFnoPU;
    result &= iEvent.getByLabel( tPhoCone01ChargedHadronIsodR015dEta0pt0dz0, hPhoCone01ChargedHadronIsodR015dEta0pt0dz0 );
    if ( hPhoCone01ChargedHadronIsodR015dEta0pt0dz0.isValid() ) PhoCone01ChargedHadronIsodR015dEta0pt0dz0 = *hPhoCone01ChargedHadronIsodR015dEta0pt0dz0;
    result &= iEvent.getByLabel( tPhoCone01ChargedHadronIsodR015dEta0pt0dz1dxy01, hPhoCone01ChargedHadronIsodR015dEta0pt0dz1dxy01 );
    if ( hPhoCone01ChargedHadronIsodR015dEta0pt0dz1dxy01.isValid() ) PhoCone01ChargedHadronIsodR015dEta0pt0dz1dxy01 = *hPhoCone01ChargedHadronIsodR015dEta0pt0dz1dxy01;
    result &= iEvent.getByLabel( tPhoCone01ChargedHadronIsodR02dz02dxy01, hPhoCone01ChargedHadronIsodR02dz02dxy01 );
    if ( hPhoCone01ChargedHadronIsodR02dz02dxy01.isValid() ) PhoCone01ChargedHadronIsodR02dz02dxy01 = *hPhoCone01ChargedHadronIsodR02dz02dxy01;
    result &= iEvent.getByLabel( tPhoCone01ChargedHadronIsodR0dEta0pt0PFnoPU, hPhoCone01ChargedHadronIsodR0dEta0pt0PFnoPU );
    if ( hPhoCone01ChargedHadronIsodR0dEta0pt0PFnoPU.isValid() ) PhoCone01ChargedHadronIsodR0dEta0pt0PFnoPU = *hPhoCone01ChargedHadronIsodR0dEta0pt0PFnoPU;
    result &= iEvent.getByLabel( tPhoCone01ChargedHadronIsodR0dEta0pt0dz0, hPhoCone01ChargedHadronIsodR0dEta0pt0dz0 );
    if ( hPhoCone01ChargedHadronIsodR0dEta0pt0dz0.isValid() ) PhoCone01ChargedHadronIsodR0dEta0pt0dz0 = *hPhoCone01ChargedHadronIsodR0dEta0pt0dz0;
    result &= iEvent.getByLabel( tPhoCone01ChargedHadronIsodR0dEta0pt0dz1dxy01, hPhoCone01ChargedHadronIsodR0dEta0pt0dz1dxy01 );
    if ( hPhoCone01ChargedHadronIsodR0dEta0pt0dz1dxy01.isValid() ) PhoCone01ChargedHadronIsodR0dEta0pt0dz1dxy01 = *hPhoCone01ChargedHadronIsodR0dEta0pt0dz1dxy01;
    result &= iEvent.getByLabel( tPhoCone01NeutralHadronIsodR0dEta0pt0mvVtx, hPhoCone01NeutralHadronIsodR0dEta0pt0mvVtx );
    if ( hPhoCone01NeutralHadronIsodR0dEta0pt0mvVtx.isValid() ) PhoCone01NeutralHadronIsodR0dEta0pt0mvVtx = *hPhoCone01NeutralHadronIsodR0dEta0pt0mvVtx;
    result &= iEvent.getByLabel( tPhoCone01NeutralHadronIsomvVtx, hPhoCone01NeutralHadronIsomvVtx );
    if ( hPhoCone01NeutralHadronIsomvVtx.isValid() ) PhoCone01NeutralHadronIsomvVtx = *hPhoCone01NeutralHadronIsomvVtx;
    result &= iEvent.getByLabel( tPhoCone01PhotonIsodEta015EBdR070EEmvVtx, hPhoCone01PhotonIsodEta015EBdR070EEmvVtx );
    if ( hPhoCone01PhotonIsodEta015EBdR070EEmvVtx.isValid() ) PhoCone01PhotonIsodEta015EBdR070EEmvVtx = *hPhoCone01PhotonIsodEta015EBdR070EEmvVtx;
    result &= iEvent.getByLabel( tPhoCone01PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx, hPhoCone01PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx );
    if ( hPhoCone01PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx.isValid() ) PhoCone01PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx = *hPhoCone01PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx;
    result &= iEvent.getByLabel( tPhoCone02ChargedHadronIsodR015dEta0pt0PFnoPU, hPhoCone02ChargedHadronIsodR015dEta0pt0PFnoPU );
    if ( hPhoCone02ChargedHadronIsodR015dEta0pt0PFnoPU.isValid() ) PhoCone02ChargedHadronIsodR015dEta0pt0PFnoPU = *hPhoCone02ChargedHadronIsodR015dEta0pt0PFnoPU;
    result &= iEvent.getByLabel( tPhoCone02ChargedHadronIsodR015dEta0pt0dz0, hPhoCone02ChargedHadronIsodR015dEta0pt0dz0 );
    if ( hPhoCone02ChargedHadronIsodR015dEta0pt0dz0.isValid() ) PhoCone02ChargedHadronIsodR015dEta0pt0dz0 = *hPhoCone02ChargedHadronIsodR015dEta0pt0dz0;
    result &= iEvent.getByLabel( tPhoCone02ChargedHadronIsodR015dEta0pt0dz1dxy01, hPhoCone02ChargedHadronIsodR015dEta0pt0dz1dxy01 );
    if ( hPhoCone02ChargedHadronIsodR015dEta0pt0dz1dxy01.isValid() ) PhoCone02ChargedHadronIsodR015dEta0pt0dz1dxy01 = *hPhoCone02ChargedHadronIsodR015dEta0pt0dz1dxy01;
    result &= iEvent.getByLabel( tPhoCone02ChargedHadronIsodR02dz02dxy01, hPhoCone02ChargedHadronIsodR02dz02dxy01 );
    if ( hPhoCone02ChargedHadronIsodR02dz02dxy01.isValid() ) PhoCone02ChargedHadronIsodR02dz02dxy01 = *hPhoCone02ChargedHadronIsodR02dz02dxy01;
    result &= iEvent.getByLabel( tPhoCone02ChargedHadronIsodR0dEta0pt0PFnoPU, hPhoCone02ChargedHadronIsodR0dEta0pt0PFnoPU );
    if ( hPhoCone02ChargedHadronIsodR0dEta0pt0PFnoPU.isValid() ) PhoCone02ChargedHadronIsodR0dEta0pt0PFnoPU = *hPhoCone02ChargedHadronIsodR0dEta0pt0PFnoPU;
    result &= iEvent.getByLabel( tPhoCone02ChargedHadronIsodR0dEta0pt0dz0, hPhoCone02ChargedHadronIsodR0dEta0pt0dz0 );
    if ( hPhoCone02ChargedHadronIsodR0dEta0pt0dz0.isValid() ) PhoCone02ChargedHadronIsodR0dEta0pt0dz0 = *hPhoCone02ChargedHadronIsodR0dEta0pt0dz0;
    result &= iEvent.getByLabel( tPhoCone02ChargedHadronIsodR0dEta0pt0dz1dxy01, hPhoCone02ChargedHadronIsodR0dEta0pt0dz1dxy01 );
    if ( hPhoCone02ChargedHadronIsodR0dEta0pt0dz1dxy01.isValid() ) PhoCone02ChargedHadronIsodR0dEta0pt0dz1dxy01 = *hPhoCone02ChargedHadronIsodR0dEta0pt0dz1dxy01;
    result &= iEvent.getByLabel( tPhoCone02NeutralHadronIsodR0dEta0pt0mvVtx, hPhoCone02NeutralHadronIsodR0dEta0pt0mvVtx );
    if ( hPhoCone02NeutralHadronIsodR0dEta0pt0mvVtx.isValid() ) PhoCone02NeutralHadronIsodR0dEta0pt0mvVtx = *hPhoCone02NeutralHadronIsodR0dEta0pt0mvVtx;
    result &= iEvent.getByLabel( tPhoCone02NeutralHadronIsomvVtx, hPhoCone02NeutralHadronIsomvVtx );
    if ( hPhoCone02NeutralHadronIsomvVtx.isValid() ) PhoCone02NeutralHadronIsomvVtx = *hPhoCone02NeutralHadronIsomvVtx;
    result &= iEvent.getByLabel( tPhoCone02PhotonIsodEta015EBdR070EEmvVtx, hPhoCone02PhotonIsodEta015EBdR070EEmvVtx );
    if ( hPhoCone02PhotonIsodEta015EBdR070EEmvVtx.isValid() ) PhoCone02PhotonIsodEta015EBdR070EEmvVtx = *hPhoCone02PhotonIsodEta015EBdR070EEmvVtx;
    result &= iEvent.getByLabel( tPhoCone02PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx, hPhoCone02PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx );
    if ( hPhoCone02PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx.isValid() ) PhoCone02PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx = *hPhoCone02PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx;
    result &= iEvent.getByLabel( tPhoCone03ChargedHadronIsodR015dEta0pt0PFnoPU, hPhoCone03ChargedHadronIsodR015dEta0pt0PFnoPU );
    if ( hPhoCone03ChargedHadronIsodR015dEta0pt0PFnoPU.isValid() ) PhoCone03ChargedHadronIsodR015dEta0pt0PFnoPU = *hPhoCone03ChargedHadronIsodR015dEta0pt0PFnoPU;
    result &= iEvent.getByLabel( tPhoCone03ChargedHadronIsodR015dEta0pt0dz0, hPhoCone03ChargedHadronIsodR015dEta0pt0dz0 );
    if ( hPhoCone03ChargedHadronIsodR015dEta0pt0dz0.isValid() ) PhoCone03ChargedHadronIsodR015dEta0pt0dz0 = *hPhoCone03ChargedHadronIsodR015dEta0pt0dz0;
    result &= iEvent.getByLabel( tPhoCone03ChargedHadronIsodR015dEta0pt0dz1dxy01, hPhoCone03ChargedHadronIsodR015dEta0pt0dz1dxy01 );
    if ( hPhoCone03ChargedHadronIsodR015dEta0pt0dz1dxy01.isValid() ) PhoCone03ChargedHadronIsodR015dEta0pt0dz1dxy01 = *hPhoCone03ChargedHadronIsodR015dEta0pt0dz1dxy01;
    result &= iEvent.getByLabel( tPhoCone03ChargedHadronIsodR02dz02dxy01, hPhoCone03ChargedHadronIsodR02dz02dxy01 );
    if ( hPhoCone03ChargedHadronIsodR02dz02dxy01.isValid() ) PhoCone03ChargedHadronIsodR02dz02dxy01 = *hPhoCone03ChargedHadronIsodR02dz02dxy01;
    result &= iEvent.getByLabel( tPhoCone03ChargedHadronIsodR0dEta0pt0PFnoPU, hPhoCone03ChargedHadronIsodR0dEta0pt0PFnoPU );
    if ( hPhoCone03ChargedHadronIsodR0dEta0pt0PFnoPU.isValid() ) PhoCone03ChargedHadronIsodR0dEta0pt0PFnoPU = *hPhoCone03ChargedHadronIsodR0dEta0pt0PFnoPU;
    result &= iEvent.getByLabel( tPhoCone03ChargedHadronIsodR0dEta0pt0dz0, hPhoCone03ChargedHadronIsodR0dEta0pt0dz0 );
    if ( hPhoCone03ChargedHadronIsodR0dEta0pt0dz0.isValid() ) PhoCone03ChargedHadronIsodR0dEta0pt0dz0 = *hPhoCone03ChargedHadronIsodR0dEta0pt0dz0;
    result &= iEvent.getByLabel( tPhoCone03ChargedHadronIsodR0dEta0pt0dz1dxy01, hPhoCone03ChargedHadronIsodR0dEta0pt0dz1dxy01 );
    if ( hPhoCone03ChargedHadronIsodR0dEta0pt0dz1dxy01.isValid() ) PhoCone03ChargedHadronIsodR0dEta0pt0dz1dxy01 = *hPhoCone03ChargedHadronIsodR0dEta0pt0dz1dxy01;
    result &= iEvent.getByLabel( tPhoCone03NeutralHadronIsodR0dEta0pt0mvVtx, hPhoCone03NeutralHadronIsodR0dEta0pt0mvVtx );
    if ( hPhoCone03NeutralHadronIsodR0dEta0pt0mvVtx.isValid() ) PhoCone03NeutralHadronIsodR0dEta0pt0mvVtx = *hPhoCone03NeutralHadronIsodR0dEta0pt0mvVtx;
    result &= iEvent.getByLabel( tPhoCone03NeutralHadronIsomvVtx, hPhoCone03NeutralHadronIsomvVtx );
    if ( hPhoCone03NeutralHadronIsomvVtx.isValid() ) PhoCone03NeutralHadronIsomvVtx = *hPhoCone03NeutralHadronIsomvVtx;
    result &= iEvent.getByLabel( tPhoCone03PFCombinedIso, hPhoCone03PFCombinedIso );
    if ( hPhoCone03PFCombinedIso.isValid() ) PhoCone03PFCombinedIso = *hPhoCone03PFCombinedIso;
    result &= iEvent.getByLabel( tPhoCone03PhotonIsodEta015EBdR070EEmvVtx, hPhoCone03PhotonIsodEta015EBdR070EEmvVtx );
    if ( hPhoCone03PhotonIsodEta015EBdR070EEmvVtx.isValid() ) PhoCone03PhotonIsodEta015EBdR070EEmvVtx = *hPhoCone03PhotonIsodEta015EBdR070EEmvVtx;
    result &= iEvent.getByLabel( tPhoCone03PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx, hPhoCone03PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx );
    if ( hPhoCone03PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx.isValid() ) PhoCone03PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx = *hPhoCone03PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx;
    result &= iEvent.getByLabel( tPhoCone04ChargedHadronIsodR015dEta0pt0PFnoPU, hPhoCone04ChargedHadronIsodR015dEta0pt0PFnoPU );
    if ( hPhoCone04ChargedHadronIsodR015dEta0pt0PFnoPU.isValid() ) PhoCone04ChargedHadronIsodR015dEta0pt0PFnoPU = *hPhoCone04ChargedHadronIsodR015dEta0pt0PFnoPU;
    result &= iEvent.getByLabel( tPhoCone04ChargedHadronIsodR015dEta0pt0PFnoPUold, hPhoCone04ChargedHadronIsodR015dEta0pt0PFnoPUold );
    if ( hPhoCone04ChargedHadronIsodR015dEta0pt0PFnoPUold.isValid() ) PhoCone04ChargedHadronIsodR015dEta0pt0PFnoPUold = *hPhoCone04ChargedHadronIsodR015dEta0pt0PFnoPUold;
    result &= iEvent.getByLabel( tPhoCone04ChargedHadronIsodR015dEta0pt0dz0, hPhoCone04ChargedHadronIsodR015dEta0pt0dz0 );
    if ( hPhoCone04ChargedHadronIsodR015dEta0pt0dz0.isValid() ) PhoCone04ChargedHadronIsodR015dEta0pt0dz0 = *hPhoCone04ChargedHadronIsodR015dEta0pt0dz0;
    result &= iEvent.getByLabel( tPhoCone04ChargedHadronIsodR015dEta0pt0dz0old, hPhoCone04ChargedHadronIsodR015dEta0pt0dz0old );
    if ( hPhoCone04ChargedHadronIsodR015dEta0pt0dz0old.isValid() ) PhoCone04ChargedHadronIsodR015dEta0pt0dz0old = *hPhoCone04ChargedHadronIsodR015dEta0pt0dz0old;
    result &= iEvent.getByLabel( tPhoCone04ChargedHadronIsodR015dEta0pt0dz1dxy01, hPhoCone04ChargedHadronIsodR015dEta0pt0dz1dxy01 );
    if ( hPhoCone04ChargedHadronIsodR015dEta0pt0dz1dxy01.isValid() ) PhoCone04ChargedHadronIsodR015dEta0pt0dz1dxy01 = *hPhoCone04ChargedHadronIsodR015dEta0pt0dz1dxy01;
    result &= iEvent.getByLabel( tPhoCone04ChargedHadronIsodR02dz02dxy01, hPhoCone04ChargedHadronIsodR02dz02dxy01 );
    if ( hPhoCone04ChargedHadronIsodR02dz02dxy01.isValid() ) PhoCone04ChargedHadronIsodR02dz02dxy01 = *hPhoCone04ChargedHadronIsodR02dz02dxy01;
    result &= iEvent.getByLabel( tPhoCone04ChargedHadronIsodR0dEta0pt0PFnoPU, hPhoCone04ChargedHadronIsodR0dEta0pt0PFnoPU );
    if ( hPhoCone04ChargedHadronIsodR0dEta0pt0PFnoPU.isValid() ) PhoCone04ChargedHadronIsodR0dEta0pt0PFnoPU = *hPhoCone04ChargedHadronIsodR0dEta0pt0PFnoPU;
    result &= iEvent.getByLabel( tPhoCone04ChargedHadronIsodR0dEta0pt0PFnoPUold, hPhoCone04ChargedHadronIsodR0dEta0pt0PFnoPUold );
    if ( hPhoCone04ChargedHadronIsodR0dEta0pt0PFnoPUold.isValid() ) PhoCone04ChargedHadronIsodR0dEta0pt0PFnoPUold = *hPhoCone04ChargedHadronIsodR0dEta0pt0PFnoPUold;
    result &= iEvent.getByLabel( tPhoCone04ChargedHadronIsodR0dEta0pt0dz0, hPhoCone04ChargedHadronIsodR0dEta0pt0dz0 );
    if ( hPhoCone04ChargedHadronIsodR0dEta0pt0dz0.isValid() ) PhoCone04ChargedHadronIsodR0dEta0pt0dz0 = *hPhoCone04ChargedHadronIsodR0dEta0pt0dz0;
    result &= iEvent.getByLabel( tPhoCone04ChargedHadronIsodR0dEta0pt0dz0old, hPhoCone04ChargedHadronIsodR0dEta0pt0dz0old );
    if ( hPhoCone04ChargedHadronIsodR0dEta0pt0dz0old.isValid() ) PhoCone04ChargedHadronIsodR0dEta0pt0dz0old = *hPhoCone04ChargedHadronIsodR0dEta0pt0dz0old;
    result &= iEvent.getByLabel( tPhoCone04ChargedHadronIsodR0dEta0pt0dz1dxy01, hPhoCone04ChargedHadronIsodR0dEta0pt0dz1dxy01 );
    if ( hPhoCone04ChargedHadronIsodR0dEta0pt0dz1dxy01.isValid() ) PhoCone04ChargedHadronIsodR0dEta0pt0dz1dxy01 = *hPhoCone04ChargedHadronIsodR0dEta0pt0dz1dxy01;
    result &= iEvent.getByLabel( tPhoCone04NeutralHadronIsodR0dEta0pt0, hPhoCone04NeutralHadronIsodR0dEta0pt0 );
    if ( hPhoCone04NeutralHadronIsodR0dEta0pt0.isValid() ) PhoCone04NeutralHadronIsodR0dEta0pt0 = *hPhoCone04NeutralHadronIsodR0dEta0pt0;
    result &= iEvent.getByLabel( tPhoCone04NeutralHadronIsodR0dEta0pt0mvVtx, hPhoCone04NeutralHadronIsodR0dEta0pt0mvVtx );
    if ( hPhoCone04NeutralHadronIsodR0dEta0pt0mvVtx.isValid() ) PhoCone04NeutralHadronIsodR0dEta0pt0mvVtx = *hPhoCone04NeutralHadronIsodR0dEta0pt0mvVtx;
    result &= iEvent.getByLabel( tPhoCone04NeutralHadronIsodR0dEta0pt0nocracks, hPhoCone04NeutralHadronIsodR0dEta0pt0nocracks );
    if ( hPhoCone04NeutralHadronIsodR0dEta0pt0nocracks.isValid() ) PhoCone04NeutralHadronIsodR0dEta0pt0nocracks = *hPhoCone04NeutralHadronIsodR0dEta0pt0nocracks;
    result &= iEvent.getByLabel( tPhoCone04NeutralHadronIsodR0dEta0pt5, hPhoCone04NeutralHadronIsodR0dEta0pt5 );
    if ( hPhoCone04NeutralHadronIsodR0dEta0pt5.isValid() ) PhoCone04NeutralHadronIsodR0dEta0pt5 = *hPhoCone04NeutralHadronIsodR0dEta0pt5;
    result &= iEvent.getByLabel( tPhoCone04NeutralHadronIsodR0dEta0pt5nocracks, hPhoCone04NeutralHadronIsodR0dEta0pt5nocracks );
    if ( hPhoCone04NeutralHadronIsodR0dEta0pt5nocracks.isValid() ) PhoCone04NeutralHadronIsodR0dEta0pt5nocracks = *hPhoCone04NeutralHadronIsodR0dEta0pt5nocracks;
    result &= iEvent.getByLabel( tPhoCone04NeutralHadronIsodR7dEta0pt0, hPhoCone04NeutralHadronIsodR7dEta0pt0 );
    if ( hPhoCone04NeutralHadronIsodR7dEta0pt0.isValid() ) PhoCone04NeutralHadronIsodR7dEta0pt0 = *hPhoCone04NeutralHadronIsodR7dEta0pt0;
    result &= iEvent.getByLabel( tPhoCone04NeutralHadronIsodR7dEta0pt5, hPhoCone04NeutralHadronIsodR7dEta0pt5 );
    if ( hPhoCone04NeutralHadronIsodR7dEta0pt5.isValid() ) PhoCone04NeutralHadronIsodR7dEta0pt5 = *hPhoCone04NeutralHadronIsodR7dEta0pt5;
    result &= iEvent.getByLabel( tPhoCone04NeutralHadronIsomvVtx, hPhoCone04NeutralHadronIsomvVtx );
    if ( hPhoCone04NeutralHadronIsomvVtx.isValid() ) PhoCone04NeutralHadronIsomvVtx = *hPhoCone04NeutralHadronIsomvVtx;
    result &= iEvent.getByLabel( tPhoCone04PFCombinedIso, hPhoCone04PFCombinedIso );
    if ( hPhoCone04PFCombinedIso.isValid() ) PhoCone04PFCombinedIso = *hPhoCone04PFCombinedIso;
    result &= iEvent.getByLabel( tPhoCone04PhotonIsodEta015EBdR070EEmvVtx, hPhoCone04PhotonIsodEta015EBdR070EEmvVtx );
    if ( hPhoCone04PhotonIsodEta015EBdR070EEmvVtx.isValid() ) PhoCone04PhotonIsodEta015EBdR070EEmvVtx = *hPhoCone04PhotonIsodEta015EBdR070EEmvVtx;
    result &= iEvent.getByLabel( tPhoCone04PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx, hPhoCone04PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx );
    if ( hPhoCone04PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx.isValid() ) PhoCone04PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx = *hPhoCone04PhotonIsodR045EB070EEdEta015pt08EB1EEmvVtx;
    result &= iEvent.getByLabel( tPhoCone04PhotonIsodR0dEta0pt0, hPhoCone04PhotonIsodR0dEta0pt0 );
    if ( hPhoCone04PhotonIsodR0dEta0pt0.isValid() ) PhoCone04PhotonIsodR0dEta0pt0 = *hPhoCone04PhotonIsodR0dEta0pt0;
    result &= iEvent.getByLabel( tPhoCone04PhotonIsodR0dEta0pt5, hPhoCone04PhotonIsodR0dEta0pt5 );
    if ( hPhoCone04PhotonIsodR0dEta0pt5.isValid() ) PhoCone04PhotonIsodR0dEta0pt5 = *hPhoCone04PhotonIsodR0dEta0pt5;
    result &= iEvent.getByLabel( tPhoCone04PhotonIsodR8dEta0pt0, hPhoCone04PhotonIsodR8dEta0pt0 );
    if ( hPhoCone04PhotonIsodR8dEta0pt0.isValid() ) PhoCone04PhotonIsodR8dEta0pt0 = *hPhoCone04PhotonIsodR8dEta0pt0;
    result &= iEvent.getByLabel( tPhoCone04PhotonIsodR8dEta0pt5, hPhoCone04PhotonIsodR8dEta0pt5 );
    if ( hPhoCone04PhotonIsodR8dEta0pt5.isValid() ) PhoCone04PhotonIsodR8dEta0pt5 = *hPhoCone04PhotonIsodR8dEta0pt5;
    result &= iEvent.getByLabel( tPhoConvChi2Probability, hPhoConvChi2Probability );
    if ( hPhoConvChi2Probability.isValid() ) PhoConvChi2Probability = *hPhoConvChi2Probability;
    result &= iEvent.getByLabel( tPhoConvEoverP, hPhoConvEoverP );
    if ( hPhoConvEoverP.isValid() ) PhoConvEoverP = *hPhoConvEoverP;
    result &= iEvent.getByLabel( tPhoConvNtracks, hPhoConvNtracks );
    if ( hPhoConvNtracks.isValid() ) PhoConvNtracks = *hPhoConvNtracks;
    result &= iEvent.getByLabel( tPhoConvValidVtx, hPhoConvValidVtx );
    if ( hPhoConvValidVtx.isValid() ) PhoConvValidVtx = *hPhoConvValidVtx;
    result &= iEvent.getByLabel( tPhoE1OverE9, hPhoE1OverE9 );
    if ( hPhoE1OverE9.isValid() ) PhoE1OverE9 = *hPhoE1OverE9;
    result &= iEvent.getByLabel( tPhoE1x5, hPhoE1x5 );
    if ( hPhoE1x5.isValid() ) PhoE1x5 = *hPhoE1x5;
    result &= iEvent.getByLabel( tPhoE2x5, hPhoE2x5 );
    if ( hPhoE2x5.isValid() ) PhoE2x5 = *hPhoE2x5;
    result &= iEvent.getByLabel( tPhoE3x3, hPhoE3x3 );
    if ( hPhoE3x3.isValid() ) PhoE3x3 = *hPhoE3x3;
    result &= iEvent.getByLabel( tPhoE5x5, hPhoE5x5 );
    if ( hPhoE5x5.isValid() ) PhoE5x5 = *hPhoE5x5;
    result &= iEvent.getByLabel( tPhoEnergy, hPhoEnergy );
    if ( hPhoEnergy.isValid() ) PhoEnergy = *hPhoEnergy;
    result &= iEvent.getByLabel( tPhoEta, hPhoEta );
    if ( hPhoEta.isValid() ) PhoEta = *hPhoEta;
    result &= iEvent.getByLabel( tPhoGood, hPhoGood );
    if ( hPhoGood.isValid() ) PhoGood = *hPhoGood;
    result &= iEvent.getByLabel( tPhoH1overE, hPhoH1overE );
    if ( hPhoH1overE.isValid() ) PhoH1overE = *hPhoH1overE;
    result &= iEvent.getByLabel( tPhoH2overE, hPhoH2overE );
    if ( hPhoH2overE.isValid() ) PhoH2overE = *hPhoH2overE;
    result &= iEvent.getByLabel( tPhoHCalIso2012ConeDR03, hPhoHCalIso2012ConeDR03 );
    if ( hPhoHCalIso2012ConeDR03.isValid() ) PhoHCalIso2012ConeDR03 = *hPhoHCalIso2012ConeDR03;
    result &= iEvent.getByLabel( tPhoHasConvTrks, hPhoHasConvTrks );
    if ( hPhoHasConvTrks.isValid() ) PhoHasConvTrks = *hPhoHasConvTrks;
    result &= iEvent.getByLabel( tPhoHasPixSeed, hPhoHasPixSeed );
    if ( hPhoHasPixSeed.isValid() ) PhoHasPixSeed = *hPhoHasPixSeed;
    result &= iEvent.getByLabel( tPhoHoverE, hPhoHoverE );
    if ( hPhoHoverE.isValid() ) PhoHoverE = *hPhoHoverE;
    result &= iEvent.getByLabel( tPhoHoverE2012, hPhoHoverE2012 );
    if ( hPhoHoverE2012.isValid() ) PhoHoverE2012 = *hPhoHoverE2012;
    result &= iEvent.getByLabel( tPhoIsIso, hPhoIsIso );
    if ( hPhoIsIso.isValid() ) PhoIsIso = *hPhoIsIso;
    result &= iEvent.getByLabel( tPhoIso03, hPhoIso03 );
    if ( hPhoIso03.isValid() ) PhoIso03 = *hPhoIso03;
    result &= iEvent.getByLabel( tPhoIso03Ecal, hPhoIso03Ecal );
    if ( hPhoIso03Ecal.isValid() ) PhoIso03Ecal = *hPhoIso03Ecal;
    result &= iEvent.getByLabel( tPhoIso03Hcal, hPhoIso03Hcal );
    if ( hPhoIso03Hcal.isValid() ) PhoIso03Hcal = *hPhoIso03Hcal;
    result &= iEvent.getByLabel( tPhoIso03HcalDepth1, hPhoIso03HcalDepth1 );
    if ( hPhoIso03HcalDepth1.isValid() ) PhoIso03HcalDepth1 = *hPhoIso03HcalDepth1;
    result &= iEvent.getByLabel( tPhoIso03HcalDepth2, hPhoIso03HcalDepth2 );
    if ( hPhoIso03HcalDepth2.isValid() ) PhoIso03HcalDepth2 = *hPhoIso03HcalDepth2;
    result &= iEvent.getByLabel( tPhoIso03TrkHollow, hPhoIso03TrkHollow );
    if ( hPhoIso03TrkHollow.isValid() ) PhoIso03TrkHollow = *hPhoIso03TrkHollow;
    result &= iEvent.getByLabel( tPhoIso03TrkSolid, hPhoIso03TrkSolid );
    if ( hPhoIso03TrkSolid.isValid() ) PhoIso03TrkSolid = *hPhoIso03TrkSolid;
    result &= iEvent.getByLabel( tPhoIso03nTrksHollow, hPhoIso03nTrksHollow );
    if ( hPhoIso03nTrksHollow.isValid() ) PhoIso03nTrksHollow = *hPhoIso03nTrksHollow;
    result &= iEvent.getByLabel( tPhoIso03nTrksSolid, hPhoIso03nTrksSolid );
    if ( hPhoIso03nTrksSolid.isValid() ) PhoIso03nTrksSolid = *hPhoIso03nTrksSolid;
    result &= iEvent.getByLabel( tPhoIso04, hPhoIso04 );
    if ( hPhoIso04.isValid() ) PhoIso04 = *hPhoIso04;
    result &= iEvent.getByLabel( tPhoIso04Ecal, hPhoIso04Ecal );
    if ( hPhoIso04Ecal.isValid() ) PhoIso04Ecal = *hPhoIso04Ecal;
    result &= iEvent.getByLabel( tPhoIso04Hcal, hPhoIso04Hcal );
    if ( hPhoIso04Hcal.isValid() ) PhoIso04Hcal = *hPhoIso04Hcal;
    result &= iEvent.getByLabel( tPhoIso04HcalDepth1, hPhoIso04HcalDepth1 );
    if ( hPhoIso04HcalDepth1.isValid() ) PhoIso04HcalDepth1 = *hPhoIso04HcalDepth1;
    result &= iEvent.getByLabel( tPhoIso04HcalDepth2, hPhoIso04HcalDepth2 );
    if ( hPhoIso04HcalDepth2.isValid() ) PhoIso04HcalDepth2 = *hPhoIso04HcalDepth2;
    result &= iEvent.getByLabel( tPhoIso04TrkHollow, hPhoIso04TrkHollow );
    if ( hPhoIso04TrkHollow.isValid() ) PhoIso04TrkHollow = *hPhoIso04TrkHollow;
    result &= iEvent.getByLabel( tPhoIso04TrkSolid, hPhoIso04TrkSolid );
    if ( hPhoIso04TrkSolid.isValid() ) PhoIso04TrkSolid = *hPhoIso04TrkSolid;
    result &= iEvent.getByLabel( tPhoIso04nTrksHollow, hPhoIso04nTrksHollow );
    if ( hPhoIso04nTrksHollow.isValid() ) PhoIso04nTrksHollow = *hPhoIso04nTrksHollow;
    result &= iEvent.getByLabel( tPhoIso04nTrksSolid, hPhoIso04nTrksSolid );
    if ( hPhoIso04nTrksSolid.isValid() ) PhoIso04nTrksSolid = *hPhoIso04nTrksSolid;
    result &= iEvent.getByLabel( tPhoMCmatchexitcode, hPhoMCmatchexitcode );
    if ( hPhoMCmatchexitcode.isValid() ) PhoMCmatchexitcode = *hPhoMCmatchexitcode;
    result &= iEvent.getByLabel( tPhoMCmatchindex, hPhoMCmatchindex );
    if ( hPhoMCmatchindex.isValid() ) PhoMCmatchindex = *hPhoMCmatchindex;
    result &= iEvent.getByLabel( tPhoMatchedPFElectronCand, hPhoMatchedPFElectronCand );
    if ( hPhoMatchedPFElectronCand.isValid() ) PhoMatchedPFElectronCand = *hPhoMatchedPFElectronCand;
    result &= iEvent.getByLabel( tPhoMatchedPFPhotonCand, hPhoMatchedPFPhotonCand );
    if ( hPhoMatchedPFPhotonCand.isValid() ) PhoMatchedPFPhotonCand = *hPhoMatchedPFPhotonCand;
    result &= iEvent.getByLabel( tPhoNeutralHadronIso, hPhoNeutralHadronIso );
    if ( hPhoNeutralHadronIso.isValid() ) PhoNeutralHadronIso = *hPhoNeutralHadronIso;
    result &= iEvent.getByLabel( tPhoNewIsoPFCharged, hPhoNewIsoPFCharged );
    if ( hPhoNewIsoPFCharged.isValid() ) PhoNewIsoPFCharged = *hPhoNewIsoPFCharged;
    result &= iEvent.getByLabel( tPhoNewIsoPFNeutral, hPhoNewIsoPFNeutral );
    if ( hPhoNewIsoPFNeutral.isValid() ) PhoNewIsoPFNeutral = *hPhoNewIsoPFNeutral;
    result &= iEvent.getByLabel( tPhoNewIsoPFPhoton, hPhoNewIsoPFPhoton );
    if ( hPhoNewIsoPFPhoton.isValid() ) PhoNewIsoPFPhoton = *hPhoNewIsoPFPhoton;
    result &= iEvent.getByLabel( tPhoPassConversionVeto, hPhoPassConversionVeto );
    if ( hPhoPassConversionVeto.isValid() ) PhoPassConversionVeto = *hPhoPassConversionVeto;
    result &= iEvent.getByLabel( tPhoPhi, hPhoPhi );
    if ( hPhoPhi.isValid() ) PhoPhi = *hPhoPhi;
    result &= iEvent.getByLabel( tPhoPhotonIso, hPhoPhotonIso );
    if ( hPhoPhotonIso.isValid() ) PhoPhotonIso = *hPhoPhotonIso;
    result &= iEvent.getByLabel( tPhoPt, hPhoPt );
    if ( hPhoPt.isValid() ) PhoPt = *hPhoPt;
    result &= iEvent.getByLabel( tPhoPx, hPhoPx );
    if ( hPhoPx.isValid() ) PhoPx = *hPhoPx;
    result &= iEvent.getByLabel( tPhoPy, hPhoPy );
    if ( hPhoPy.isValid() ) PhoPy = *hPhoPy;
    result &= iEvent.getByLabel( tPhoPz, hPhoPz );
    if ( hPhoPz.isValid() ) PhoPz = *hPhoPz;
    result &= iEvent.getByLabel( tPhoR9, hPhoR9 );
    if ( hPhoR9.isValid() ) PhoR9 = *hPhoR9;
    result &= iEvent.getByLabel( tPhoS4OverS1, hPhoS4OverS1 );
    if ( hPhoS4OverS1.isValid() ) PhoS4OverS1 = *hPhoS4OverS1;
    result &= iEvent.getByLabel( tPhoSCEtaWidth, hPhoSCEtaWidth );
    if ( hPhoSCEtaWidth.isValid() ) PhoSCEtaWidth = *hPhoSCEtaWidth;
    result &= iEvent.getByLabel( tPhoSCRawEnergy, hPhoSCRawEnergy );
    if ( hPhoSCRawEnergy.isValid() ) PhoSCRawEnergy = *hPhoSCRawEnergy;
    result &= iEvent.getByLabel( tPhoSCRemovalPFIsoCharged, hPhoSCRemovalPFIsoCharged );
    if ( hPhoSCRemovalPFIsoCharged.isValid() ) PhoSCRemovalPFIsoCharged = *hPhoSCRemovalPFIsoCharged;
    result &= iEvent.getByLabel( tPhoSCRemovalPFIsoNeutral, hPhoSCRemovalPFIsoNeutral );
    if ( hPhoSCRemovalPFIsoNeutral.isValid() ) PhoSCRemovalPFIsoNeutral = *hPhoSCRemovalPFIsoNeutral;
    result &= iEvent.getByLabel( tPhoSCRemovalPFIsoPhoton, hPhoSCRemovalPFIsoPhoton );
    if ( hPhoSCRemovalPFIsoPhoton.isValid() ) PhoSCRemovalPFIsoPhoton = *hPhoSCRemovalPFIsoPhoton;
    result &= iEvent.getByLabel( tPhoSCSigmaPhiPhi, hPhoSCSigmaPhiPhi );
    if ( hPhoSCSigmaPhiPhi.isValid() ) PhoSCSigmaPhiPhi = *hPhoSCSigmaPhiPhi;
    result &= iEvent.getByLabel( tPhoScSeedSeverity, hPhoScSeedSeverity );
    if ( hPhoScSeedSeverity.isValid() ) PhoScSeedSeverity = *hPhoScSeedSeverity;
    result &= iEvent.getByLabel( tPhoSigmaEtaEta, hPhoSigmaEtaEta );
    if ( hPhoSigmaEtaEta.isValid() ) PhoSigmaEtaEta = *hPhoSigmaEtaEta;
    result &= iEvent.getByLabel( tPhoSigmaIetaIeta, hPhoSigmaIetaIeta );
    if ( hPhoSigmaIetaIeta.isValid() ) PhoSigmaIetaIeta = *hPhoSigmaIetaIeta;
    result &= iEvent.getByLabel( tPhoVx, hPhoVx );
    if ( hPhoVx.isValid() ) PhoVx = *hPhoVx;
    result &= iEvent.getByLabel( tPhoVy, hPhoVy );
    if ( hPhoVy.isValid() ) PhoVy = *hPhoVy;
    result &= iEvent.getByLabel( tPhoVz, hPhoVz );
    if ( hPhoVz.isValid() ) PhoVz = *hPhoVz;
    result &= iEvent.getByLabel( tPhoisEB, hPhoisEB );
    if ( hPhoisEB.isValid() ) PhoisEB = *hPhoisEB;
    result &= iEvent.getByLabel( tPhoisEBEEGap, hPhoisEBEEGap );
    if ( hPhoisEBEEGap.isValid() ) PhoisEBEEGap = *hPhoisEBEEGap;
    result &= iEvent.getByLabel( tPhoisEBEtaGap, hPhoisEBEtaGap );
    if ( hPhoisEBEtaGap.isValid() ) PhoisEBEtaGap = *hPhoisEBEtaGap;
    result &= iEvent.getByLabel( tPhoisEBPhiGap, hPhoisEBPhiGap );
    if ( hPhoisEBPhiGap.isValid() ) PhoisEBPhiGap = *hPhoisEBPhiGap;
    result &= iEvent.getByLabel( tPhoisEE, hPhoisEE );
    if ( hPhoisEE.isValid() ) PhoisEE = *hPhoisEE;
    result &= iEvent.getByLabel( tPhoisEEDeeGap, hPhoisEEDeeGap );
    if ( hPhoisEEDeeGap.isValid() ) PhoisEEDeeGap = *hPhoisEEDeeGap;
    result &= iEvent.getByLabel( tPhoisEERingGap, hPhoisEERingGap );
    if ( hPhoisEERingGap.isValid() ) PhoisEERingGap = *hPhoisEERingGap;
    result &= iEvent.getByLabel( tPhoisPFElectron, hPhoisPFElectron );
    if ( hPhoisPFElectron.isValid() ) PhoisPFElectron = *hPhoisPFElectron;
    result &= iEvent.getByLabel( tPhoisPFPhoton, hPhoisPFPhoton );
    if ( hPhoisPFPhoton.isValid() ) PhoisPFPhoton = *hPhoisPFPhoton;
    result &= iEvent.getByLabel( tPhoisPFlowPhoton, hPhoisPFlowPhoton );
    if ( hPhoisPFlowPhoton.isValid() ) PhoisPFlowPhoton = *hPhoisPFlowPhoton;
    result &= iEvent.getByLabel( tPhoisStandardPhoton, hPhoisStandardPhoton );
    if ( hPhoisStandardPhoton.isValid() ) PhoisStandardPhoton = *hPhoisStandardPhoton;
    result &= iEvent.getByLabel( tPhomaxEnergyXtal, hPhomaxEnergyXtal );
    if ( hPhomaxEnergyXtal.isValid() ) PhomaxEnergyXtal = *hPhomaxEnergyXtal;
    result &= iEvent.getByLabel( tPhotSCindex, hPhotSCindex );
    if ( hPhotSCindex.isValid() ) PhotSCindex = *hPhotSCindex;
    result &= iEvent.getByLabel( tPrimVtxGood, hPrimVtxGood );
    if ( hPrimVtxGood.isValid() ) PrimVtxGood = *hPrimVtxGood;
    result &= iEvent.getByLabel( tPrimVtxIsFake, hPrimVtxIsFake );
    if ( hPrimVtxIsFake.isValid() ) PrimVtxIsFake = *hPrimVtxIsFake;
    result &= iEvent.getByLabel( tPrimVtxNChi2, hPrimVtxNChi2 );
    if ( hPrimVtxNChi2.isValid() ) PrimVtxNChi2 = *hPrimVtxNChi2;
    result &= iEvent.getByLabel( tPrimVtxNdof, hPrimVtxNdof );
    if ( hPrimVtxNdof.isValid() ) PrimVtxNdof = *hPrimVtxNdof;
    result &= iEvent.getByLabel( tPrimVtxPtSum, hPrimVtxPtSum );
    if ( hPrimVtxPtSum.isValid() ) PrimVtxPtSum = *hPrimVtxPtSum;
    result &= iEvent.getByLabel( tPrimVtxRho, hPrimVtxRho );
    if ( hPrimVtxRho.isValid() ) PrimVtxRho = *hPrimVtxRho;
    result &= iEvent.getByLabel( tPrimVtxx, hPrimVtxx );
    if ( hPrimVtxx.isValid() ) PrimVtxx = *hPrimVtxx;
    result &= iEvent.getByLabel( tPrimVtxxE, hPrimVtxxE );
    if ( hPrimVtxxE.isValid() ) PrimVtxxE = *hPrimVtxxE;
    result &= iEvent.getByLabel( tPrimVtxy, hPrimVtxy );
    if ( hPrimVtxy.isValid() ) PrimVtxy = *hPrimVtxy;
    result &= iEvent.getByLabel( tPrimVtxyE, hPrimVtxyE );
    if ( hPrimVtxyE.isValid() ) PrimVtxyE = *hPrimVtxyE;
    result &= iEvent.getByLabel( tPrimVtxz, hPrimVtxz );
    if ( hPrimVtxz.isValid() ) PrimVtxz = *hPrimVtxz;
    result &= iEvent.getByLabel( tPrimVtxzE, hPrimVtxzE );
    if ( hPrimVtxzE.isValid() ) PrimVtxzE = *hPrimVtxzE;
    result &= iEvent.getByLabel( tPromptnessLevel, hPromptnessLevel );
    if ( hPromptnessLevel.isValid() ) PromptnessLevel = *hPromptnessLevel;
    result &= iEvent.getByLabel( tPtHat, hPtHat );
    if ( hPtHat.isValid() ) PtHat = *hPtHat;
    result &= iEvent.getByLabel( tQCDPartonicHT, hQCDPartonicHT );
    if ( hQCDPartonicHT.isValid() ) QCDPartonicHT = *hQCDPartonicHT;
    result &= iEvent.getByLabel( tRawMET, hRawMET );
    if ( hRawMET.isValid() ) RawMET = *hRawMET;
    result &= iEvent.getByLabel( tRawMETSignificance, hRawMETSignificance );
    if ( hRawMETSignificance.isValid() ) RawMETSignificance = *hRawMETSignificance;
    result &= iEvent.getByLabel( tRawMETemEtFrac, hRawMETemEtFrac );
    if ( hRawMETemEtFrac.isValid() ) RawMETemEtFrac = *hRawMETemEtFrac;
    result &= iEvent.getByLabel( tRawMETemEtInEB, hRawMETemEtInEB );
    if ( hRawMETemEtInEB.isValid() ) RawMETemEtInEB = *hRawMETemEtInEB;
    result &= iEvent.getByLabel( tRawMETemEtInEE, hRawMETemEtInEE );
    if ( hRawMETemEtInEE.isValid() ) RawMETemEtInEE = *hRawMETemEtInEE;
    result &= iEvent.getByLabel( tRawMETemEtInHF, hRawMETemEtInHF );
    if ( hRawMETemEtInHF.isValid() ) RawMETemEtInHF = *hRawMETemEtInHF;
    result &= iEvent.getByLabel( tRawMEThadEtFrac, hRawMEThadEtFrac );
    if ( hRawMEThadEtFrac.isValid() ) RawMEThadEtFrac = *hRawMEThadEtFrac;
    result &= iEvent.getByLabel( tRawMEThadEtInHB, hRawMEThadEtInHB );
    if ( hRawMEThadEtInHB.isValid() ) RawMEThadEtInHB = *hRawMEThadEtInHB;
    result &= iEvent.getByLabel( tRawMEThadEtInHE, hRawMEThadEtInHE );
    if ( hRawMEThadEtInHE.isValid() ) RawMEThadEtInHE = *hRawMEThadEtInHE;
    result &= iEvent.getByLabel( tRawMEThadEtInHF, hRawMEThadEtInHF );
    if ( hRawMEThadEtInHF.isValid() ) RawMEThadEtInHF = *hRawMEThadEtInHF;
    result &= iEvent.getByLabel( tRawMETphi, hRawMETphi );
    if ( hRawMETphi.isValid() ) RawMETphi = *hRawMETphi;
    result &= iEvent.getByLabel( tRawMETpx, hRawMETpx );
    if ( hRawMETpx.isValid() ) RawMETpx = *hRawMETpx;
    result &= iEvent.getByLabel( tRawMETpy, hRawMETpy );
    if ( hRawMETpy.isValid() ) RawMETpy = *hRawMETpy;
    result &= iEvent.getByLabel( tRho, hRho );
    if ( hRho.isValid() ) Rho = *hRho;
    result &= iEvent.getByLabel( tRhoForIso, hRhoForIso );
    if ( hRhoForIso.isValid() ) RhoForIso = *hRhoForIso;
    result &= iEvent.getByLabel( tRun, hRun );
    if ( hRun.isValid() ) Run = *hRun;
    result &= iEvent.getByLabel( tSCBrem, hSCBrem );
    if ( hSCBrem.isValid() ) SCBrem = *hSCBrem;
    result &= iEvent.getByLabel( tSCEnergy, hSCEnergy );
    if ( hSCEnergy.isValid() ) SCEnergy = *hSCEnergy;
    result &= iEvent.getByLabel( tSCEta, hSCEta );
    if ( hSCEta.isValid() ) SCEta = *hSCEta;
    result &= iEvent.getByLabel( tSCEtaWidth, hSCEtaWidth );
    if ( hSCEtaWidth.isValid() ) SCEtaWidth = *hSCEtaWidth;
    result &= iEvent.getByLabel( tSCNXtals, hSCNXtals );
    if ( hSCNXtals.isValid() ) SCNXtals = *hSCNXtals;
    result &= iEvent.getByLabel( tSCPhi, hSCPhi );
    if ( hSCPhi.isValid() ) SCPhi = *hSCPhi;
    result &= iEvent.getByLabel( tSCPhiWidth, hSCPhiWidth );
    if ( hSCPhiWidth.isValid() ) SCPhiWidth = *hSCPhiWidth;
    result &= iEvent.getByLabel( tSCPre, hSCPre );
    if ( hSCPre.isValid() ) SCPre = *hSCPre;
    result &= iEvent.getByLabel( tSCR9, hSCR9 );
    if ( hSCR9.isValid() ) SCR9 = *hSCR9;
    result &= iEvent.getByLabel( tSCRaw, hSCRaw );
    if ( hSCRaw.isValid() ) SCRaw = *hSCRaw;
    result &= iEvent.getByLabel( tSCX, hSCX );
    if ( hSCX.isValid() ) SCX = *hSCX;
    result &= iEvent.getByLabel( tSCXtalListStart, hSCXtalListStart );
    if ( hSCXtalListStart.isValid() ) SCXtalListStart = *hSCXtalListStart;
    result &= iEvent.getByLabel( tSCY, hSCY );
    if ( hSCY.isValid() ) SCY = *hSCY;
    result &= iEvent.getByLabel( tSCZ, hSCZ );
    if ( hSCZ.isValid() ) SCZ = *hSCZ;
    result &= iEvent.getByLabel( tSCcrackcorr, hSCcrackcorr );
    if ( hSCcrackcorr.isValid() ) SCcrackcorr = *hSCcrackcorr;
    result &= iEvent.getByLabel( tSCcrackcorrseed, hSCcrackcorrseed );
    if ( hSCcrackcorrseed.isValid() ) SCcrackcorrseed = *hSCcrackcorrseed;
    result &= iEvent.getByLabel( tSCcrackcorrseedfactor, hSCcrackcorrseedfactor );
    if ( hSCcrackcorrseedfactor.isValid() ) SCcrackcorrseedfactor = *hSCcrackcorrseedfactor;
    result &= iEvent.getByLabel( tSClocalcorr, hSClocalcorr );
    if ( hSClocalcorr.isValid() ) SClocalcorr = *hSClocalcorr;
    result &= iEvent.getByLabel( tSClocalcorrseed, hSClocalcorrseed );
    if ( hSClocalcorrseed.isValid() ) SClocalcorrseed = *hSClocalcorrseed;
    result &= iEvent.getByLabel( tSClocalcorrseedfactor, hSClocalcorrseedfactor );
    if ( hSClocalcorrseedfactor.isValid() ) SClocalcorrseedfactor = *hSClocalcorrseedfactor;
    result &= iEvent.getByLabel( tSigProcID, hSigProcID );
    if ( hSigProcID.isValid() ) SigProcID = *hSigProcID;
    result &= iEvent.getByLabel( tSigma, hSigma );
    if ( hSigma.isValid() ) Sigma = *hSigma;
    result &= iEvent.getByLabel( tSumEt, hSumEt );
    if ( hSumEt.isValid() ) SumEt = *hSumEt;
    result &= iEvent.getByLabel( tTCMET, hTCMET );
    if ( hTCMET.isValid() ) TCMET = *hTCMET;
    result &= iEvent.getByLabel( tTCMETSignificance, hTCMETSignificance );
    if ( hTCMETSignificance.isValid() ) TCMETSignificance = *hTCMETSignificance;
    result &= iEvent.getByLabel( tTCMETphi, hTCMETphi );
    if ( hTCMETphi.isValid() ) TCMETphi = *hTCMETphi;
    result &= iEvent.getByLabel( tTCMETpx, hTCMETpx );
    if ( hTCMETpx.isValid() ) TCMETpx = *hTCMETpx;
    result &= iEvent.getByLabel( tTCMETpy, hTCMETpy );
    if ( hTCMETpy.isValid() ) TCMETpy = *hTCMETpy;
    result &= iEvent.getByLabel( tTauCharge, hTauCharge );
    if ( hTauCharge.isValid() ) TauCharge = *hTauCharge;
    result &= iEvent.getByLabel( tTauChargedHadronIso, hTauChargedHadronIso );
    if ( hTauChargedHadronIso.isValid() ) TauChargedHadronIso = *hTauChargedHadronIso;
    result &= iEvent.getByLabel( tTauDecayMode, hTauDecayMode );
    if ( hTauDecayMode.isValid() ) TauDecayMode = *hTauDecayMode;
    result &= iEvent.getByLabel( tTauDecayModeFinding, hTauDecayModeFinding );
    if ( hTauDecayModeFinding.isValid() ) TauDecayModeFinding = *hTauDecayModeFinding;
    result &= iEvent.getByLabel( tTauE, hTauE );
    if ( hTauE.isValid() ) TauE = *hTauE;
    result &= iEvent.getByLabel( tTauElectronMVARejection, hTauElectronMVARejection );
    if ( hTauElectronMVARejection.isValid() ) TauElectronMVARejection = *hTauElectronMVARejection;
    result &= iEvent.getByLabel( tTauEmFraction, hTauEmFraction );
    if ( hTauEmFraction.isValid() ) TauEmFraction = *hTauEmFraction;
    result &= iEvent.getByLabel( tTauEt, hTauEt );
    if ( hTauEt.isValid() ) TauEt = *hTauEt;
    result &= iEvent.getByLabel( tTauEta, hTauEta );
    if ( hTauEta.isValid() ) TauEta = *hTauEta;
    result &= iEvent.getByLabel( tTauIsPFTau, hTauIsPFTau );
    if ( hTauIsPFTau.isValid() ) TauIsPFTau = *hTauIsPFTau;
    result &= iEvent.getByLabel( tTauIsolationMVA2raw, hTauIsolationMVA2raw );
    if ( hTauIsolationMVA2raw.isValid() ) TauIsolationMVA2raw = *hTauIsolationMVA2raw;
    result &= iEvent.getByLabel( tTauIsolationMVAraw, hTauIsolationMVAraw );
    if ( hTauIsolationMVAraw.isValid() ) TauIsolationMVAraw = *hTauIsolationMVAraw;
    result &= iEvent.getByLabel( tTauJetEta, hTauJetEta );
    if ( hTauJetEta.isValid() ) TauJetEta = *hTauJetEta;
    result &= iEvent.getByLabel( tTauJetMass, hTauJetMass );
    if ( hTauJetMass.isValid() ) TauJetMass = *hTauJetMass;
    result &= iEvent.getByLabel( tTauJetPhi, hTauJetPhi );
    if ( hTauJetPhi.isValid() ) TauJetPhi = *hTauJetPhi;
    result &= iEvent.getByLabel( tTauJetPt, hTauJetPt );
    if ( hTauJetPt.isValid() ) TauJetPt = *hTauJetPt;
    result &= iEvent.getByLabel( tTauLeadingNeuPt, hTauLeadingNeuPt );
    if ( hTauLeadingNeuPt.isValid() ) TauLeadingNeuPt = *hTauLeadingNeuPt;
    result &= iEvent.getByLabel( tTauLeadingTkEcalenergy, hTauLeadingTkEcalenergy );
    if ( hTauLeadingTkEcalenergy.isValid() ) TauLeadingTkEcalenergy = *hTauLeadingTkEcalenergy;
    result &= iEvent.getByLabel( tTauLeadingTkHcalenergy, hTauLeadingTkHcalenergy );
    if ( hTauLeadingTkHcalenergy.isValid() ) TauLeadingTkHcalenergy = *hTauLeadingTkHcalenergy;
    result &= iEvent.getByLabel( tTauLeadingTkPt, hTauLeadingTkPt );
    if ( hTauLeadingTkPt.isValid() ) TauLeadingTkPt = *hTauLeadingTkPt;
    result &= iEvent.getByLabel( tTauLooseChargedIso, hTauLooseChargedIso );
    if ( hTauLooseChargedIso.isValid() ) TauLooseChargedIso = *hTauLooseChargedIso;
    result &= iEvent.getByLabel( tTauLooseCombinedIsoDBSumPtCorr, hTauLooseCombinedIsoDBSumPtCorr );
    if ( hTauLooseCombinedIsoDBSumPtCorr.isValid() ) TauLooseCombinedIsoDBSumPtCorr = *hTauLooseCombinedIsoDBSumPtCorr;
    result &= iEvent.getByLabel( tTauLooseCombinedIsoDBSumPtCorr3Hits, hTauLooseCombinedIsoDBSumPtCorr3Hits );
    if ( hTauLooseCombinedIsoDBSumPtCorr3Hits.isValid() ) TauLooseCombinedIsoDBSumPtCorr3Hits = *hTauLooseCombinedIsoDBSumPtCorr3Hits;
    result &= iEvent.getByLabel( tTauLooseElectronMVA3Rejection, hTauLooseElectronMVA3Rejection );
    if ( hTauLooseElectronMVA3Rejection.isValid() ) TauLooseElectronMVA3Rejection = *hTauLooseElectronMVA3Rejection;
    result &= iEvent.getByLabel( tTauLooseElectronRejection, hTauLooseElectronRejection );
    if ( hTauLooseElectronRejection.isValid() ) TauLooseElectronRejection = *hTauLooseElectronRejection;
    result &= iEvent.getByLabel( tTauLooseIso, hTauLooseIso );
    if ( hTauLooseIso.isValid() ) TauLooseIso = *hTauLooseIso;
    result &= iEvent.getByLabel( tTauLooseIsoDBSumPtCorr, hTauLooseIsoDBSumPtCorr );
    if ( hTauLooseIsoDBSumPtCorr.isValid() ) TauLooseIsoDBSumPtCorr = *hTauLooseIsoDBSumPtCorr;
    result &= iEvent.getByLabel( tTauLooseIsolationMVA, hTauLooseIsolationMVA );
    if ( hTauLooseIsolationMVA.isValid() ) TauLooseIsolationMVA = *hTauLooseIsolationMVA;
    result &= iEvent.getByLabel( tTauLooseIsolationMVA2, hTauLooseIsolationMVA2 );
    if ( hTauLooseIsolationMVA2.isValid() ) TauLooseIsolationMVA2 = *hTauLooseIsolationMVA2;
    result &= iEvent.getByLabel( tTauLooseMuon2Rejection, hTauLooseMuon2Rejection );
    if ( hTauLooseMuon2Rejection.isValid() ) TauLooseMuon2Rejection = *hTauLooseMuon2Rejection;
    result &= iEvent.getByLabel( tTauLooseMuonRejection, hTauLooseMuonRejection );
    if ( hTauLooseMuonRejection.isValid() ) TauLooseMuonRejection = *hTauLooseMuonRejection;
    result &= iEvent.getByLabel( tTauMaxLepExc, hTauMaxLepExc );
    if ( hTauMaxLepExc.isValid() ) TauMaxLepExc = *hTauMaxLepExc;
    result &= iEvent.getByLabel( tTauMediumChargedIso, hTauMediumChargedIso );
    if ( hTauMediumChargedIso.isValid() ) TauMediumChargedIso = *hTauMediumChargedIso;
    result &= iEvent.getByLabel( tTauMediumCombinedIsoDBSumPtCorr, hTauMediumCombinedIsoDBSumPtCorr );
    if ( hTauMediumCombinedIsoDBSumPtCorr.isValid() ) TauMediumCombinedIsoDBSumPtCorr = *hTauMediumCombinedIsoDBSumPtCorr;
    result &= iEvent.getByLabel( tTauMediumCombinedIsoDBSumPtCorr3Hits, hTauMediumCombinedIsoDBSumPtCorr3Hits );
    if ( hTauMediumCombinedIsoDBSumPtCorr3Hits.isValid() ) TauMediumCombinedIsoDBSumPtCorr3Hits = *hTauMediumCombinedIsoDBSumPtCorr3Hits;
    result &= iEvent.getByLabel( tTauMediumElectronMVA3Rejection, hTauMediumElectronMVA3Rejection );
    if ( hTauMediumElectronMVA3Rejection.isValid() ) TauMediumElectronMVA3Rejection = *hTauMediumElectronMVA3Rejection;
    result &= iEvent.getByLabel( tTauMediumElectronRejection, hTauMediumElectronRejection );
    if ( hTauMediumElectronRejection.isValid() ) TauMediumElectronRejection = *hTauMediumElectronRejection;
    result &= iEvent.getByLabel( tTauMediumIso, hTauMediumIso );
    if ( hTauMediumIso.isValid() ) TauMediumIso = *hTauMediumIso;
    result &= iEvent.getByLabel( tTauMediumIsoDBSumPtCorr, hTauMediumIsoDBSumPtCorr );
    if ( hTauMediumIsoDBSumPtCorr.isValid() ) TauMediumIsoDBSumPtCorr = *hTauMediumIsoDBSumPtCorr;
    result &= iEvent.getByLabel( tTauMediumIsolationMVA, hTauMediumIsolationMVA );
    if ( hTauMediumIsolationMVA.isValid() ) TauMediumIsolationMVA = *hTauMediumIsolationMVA;
    result &= iEvent.getByLabel( tTauMediumIsolationMVA2, hTauMediumIsolationMVA2 );
    if ( hTauMediumIsolationMVA2.isValid() ) TauMediumIsolationMVA2 = *hTauMediumIsolationMVA2;
    result &= iEvent.getByLabel( tTauMediumMuon2Rejection, hTauMediumMuon2Rejection );
    if ( hTauMediumMuon2Rejection.isValid() ) TauMediumMuon2Rejection = *hTauMediumMuon2Rejection;
    result &= iEvent.getByLabel( tTauMediumMuonRejection, hTauMediumMuonRejection );
    if ( hTauMediumMuonRejection.isValid() ) TauMediumMuonRejection = *hTauMediumMuonRejection;
    result &= iEvent.getByLabel( tTauNObjs, hTauNObjs );
    if ( hTauNObjs.isValid() ) TauNObjs = *hTauNObjs;
    result &= iEvent.getByLabel( tTauNObjsTot, hTauNObjsTot );
    if ( hTauNObjsTot.isValid() ) TauNObjsTot = *hTauNObjsTot;
    result &= iEvent.getByLabel( tTauNeutralHadronIso, hTauNeutralHadronIso );
    if ( hTauNeutralHadronIso.isValid() ) TauNeutralHadronIso = *hTauNeutralHadronIso;
    result &= iEvent.getByLabel( tTauNumChargedHadronsIsoCone, hTauNumChargedHadronsIsoCone );
    if ( hTauNumChargedHadronsIsoCone.isValid() ) TauNumChargedHadronsIsoCone = *hTauNumChargedHadronsIsoCone;
    result &= iEvent.getByLabel( tTauNumChargedHadronsSignalCone, hTauNumChargedHadronsSignalCone );
    if ( hTauNumChargedHadronsSignalCone.isValid() ) TauNumChargedHadronsSignalCone = *hTauNumChargedHadronsSignalCone;
    result &= iEvent.getByLabel( tTauNumNeutralHadronsIsoCone, hTauNumNeutralHadronsIsoCone );
    if ( hTauNumNeutralHadronsIsoCone.isValid() ) TauNumNeutralHadronsIsoCone = *hTauNumNeutralHadronsIsoCone;
    result &= iEvent.getByLabel( tTauNumNeutralHadronsSignalCone, hTauNumNeutralHadronsSignalCone );
    if ( hTauNumNeutralHadronsSignalCone.isValid() ) TauNumNeutralHadronsSignalCone = *hTauNumNeutralHadronsSignalCone;
    result &= iEvent.getByLabel( tTauNumParticlesIsolationCone, hTauNumParticlesIsolationCone );
    if ( hTauNumParticlesIsolationCone.isValid() ) TauNumParticlesIsolationCone = *hTauNumParticlesIsolationCone;
    result &= iEvent.getByLabel( tTauNumParticlesSignalCone, hTauNumParticlesSignalCone );
    if ( hTauNumParticlesSignalCone.isValid() ) TauNumParticlesSignalCone = *hTauNumParticlesSignalCone;
    result &= iEvent.getByLabel( tTauNumPhotonsIsolationCone, hTauNumPhotonsIsolationCone );
    if ( hTauNumPhotonsIsolationCone.isValid() ) TauNumPhotonsIsolationCone = *hTauNumPhotonsIsolationCone;
    result &= iEvent.getByLabel( tTauNumPhotonsSignalCone, hTauNumPhotonsSignalCone );
    if ( hTauNumPhotonsSignalCone.isValid() ) TauNumPhotonsSignalCone = *hTauNumPhotonsSignalCone;
    result &= iEvent.getByLabel( tTauParticleIso, hTauParticleIso );
    if ( hTauParticleIso.isValid() ) TauParticleIso = *hTauParticleIso;
    result &= iEvent.getByLabel( tTauPhi, hTauPhi );
    if ( hTauPhi.isValid() ) TauPhi = *hTauPhi;
    result &= iEvent.getByLabel( tTauPhotonIso, hTauPhotonIso );
    if ( hTauPhotonIso.isValid() ) TauPhotonIso = *hTauPhotonIso;
    result &= iEvent.getByLabel( tTauPt, hTauPt );
    if ( hTauPt.isValid() ) TauPt = *hTauPt;
    result &= iEvent.getByLabel( tTauPtSumChargedParticlesIsoCone, hTauPtSumChargedParticlesIsoCone );
    if ( hTauPtSumChargedParticlesIsoCone.isValid() ) TauPtSumChargedParticlesIsoCone = *hTauPtSumChargedParticlesIsoCone;
    result &= iEvent.getByLabel( tTauPtSumPhotonsIsoCone, hTauPtSumPhotonsIsoCone );
    if ( hTauPtSumPhotonsIsoCone.isValid() ) TauPtSumPhotonsIsoCone = *hTauPtSumPhotonsIsoCone;
    result &= iEvent.getByLabel( tTauPx, hTauPx );
    if ( hTauPx.isValid() ) TauPx = *hTauPx;
    result &= iEvent.getByLabel( tTauPy, hTauPy );
    if ( hTauPy.isValid() ) TauPy = *hTauPy;
    result &= iEvent.getByLabel( tTauPz, hTauPz );
    if ( hTauPz.isValid() ) TauPz = *hTauPz;
    result &= iEvent.getByLabel( tTauTightChargedIso, hTauTightChargedIso );
    if ( hTauTightChargedIso.isValid() ) TauTightChargedIso = *hTauTightChargedIso;
    result &= iEvent.getByLabel( tTauTightCombinedIsoDBSumPtCorr, hTauTightCombinedIsoDBSumPtCorr );
    if ( hTauTightCombinedIsoDBSumPtCorr.isValid() ) TauTightCombinedIsoDBSumPtCorr = *hTauTightCombinedIsoDBSumPtCorr;
    result &= iEvent.getByLabel( tTauTightCombinedIsoDBSumPtCorr3Hits, hTauTightCombinedIsoDBSumPtCorr3Hits );
    if ( hTauTightCombinedIsoDBSumPtCorr3Hits.isValid() ) TauTightCombinedIsoDBSumPtCorr3Hits = *hTauTightCombinedIsoDBSumPtCorr3Hits;
    result &= iEvent.getByLabel( tTauTightElectronMVA3Rejection, hTauTightElectronMVA3Rejection );
    if ( hTauTightElectronMVA3Rejection.isValid() ) TauTightElectronMVA3Rejection = *hTauTightElectronMVA3Rejection;
    result &= iEvent.getByLabel( tTauTightElectronRejection, hTauTightElectronRejection );
    if ( hTauTightElectronRejection.isValid() ) TauTightElectronRejection = *hTauTightElectronRejection;
    result &= iEvent.getByLabel( tTauTightIso, hTauTightIso );
    if ( hTauTightIso.isValid() ) TauTightIso = *hTauTightIso;
    result &= iEvent.getByLabel( tTauTightIsoDBSumPtCorr, hTauTightIsoDBSumPtCorr );
    if ( hTauTightIsoDBSumPtCorr.isValid() ) TauTightIsoDBSumPtCorr = *hTauTightIsoDBSumPtCorr;
    result &= iEvent.getByLabel( tTauTightIsolationMVA, hTauTightIsolationMVA );
    if ( hTauTightIsolationMVA.isValid() ) TauTightIsolationMVA = *hTauTightIsolationMVA;
    result &= iEvent.getByLabel( tTauTightIsolationMVA2, hTauTightIsolationMVA2 );
    if ( hTauTightIsolationMVA2.isValid() ) TauTightIsolationMVA2 = *hTauTightIsolationMVA2;
    result &= iEvent.getByLabel( tTauTightMuon2Rejection, hTauTightMuon2Rejection );
    if ( hTauTightMuon2Rejection.isValid() ) TauTightMuon2Rejection = *hTauTightMuon2Rejection;
    result &= iEvent.getByLabel( tTauTightMuonRejection, hTauTightMuonRejection );
    if ( hTauTightMuonRejection.isValid() ) TauTightMuonRejection = *hTauTightMuonRejection;
    result &= iEvent.getByLabel( tTauVLooseChargedIso, hTauVLooseChargedIso );
    if ( hTauVLooseChargedIso.isValid() ) TauVLooseChargedIso = *hTauVLooseChargedIso;
    result &= iEvent.getByLabel( tTauVLooseCombinedIsoDBSumPtCorr, hTauVLooseCombinedIsoDBSumPtCorr );
    if ( hTauVLooseCombinedIsoDBSumPtCorr.isValid() ) TauVLooseCombinedIsoDBSumPtCorr = *hTauVLooseCombinedIsoDBSumPtCorr;
    result &= iEvent.getByLabel( tTauVLooseIso, hTauVLooseIso );
    if ( hTauVLooseIso.isValid() ) TauVLooseIso = *hTauVLooseIso;
    result &= iEvent.getByLabel( tTauVLooseIsoDBSumPtCorr, hTauVLooseIsoDBSumPtCorr );
    if ( hTauVLooseIsoDBSumPtCorr.isValid() ) TauVLooseIsoDBSumPtCorr = *hTauVLooseIsoDBSumPtCorr;
    result &= iEvent.getByLabel( tTauVTightElectronMVA3Rejection, hTauVTightElectronMVA3Rejection );
    if ( hTauVTightElectronMVA3Rejection.isValid() ) TauVTightElectronMVA3Rejection = *hTauVTightElectronMVA3Rejection;
    result &= iEvent.getByLabel( tTauVz, hTauVz );
    if ( hTauVz.isValid() ) TauVz = *hTauVz;
    result &= iEvent.getByLabel( tTrkEta, hTrkEta );
    if ( hTrkEta.isValid() ) TrkEta = *hTrkEta;
    result &= iEvent.getByLabel( tTrkGood, hTrkGood );
    if ( hTrkGood.isValid() ) TrkGood = *hTrkGood;
    result &= iEvent.getByLabel( tTrkNChi2, hTrkNChi2 );
    if ( hTrkNChi2.isValid() ) TrkNChi2 = *hTrkNChi2;
    result &= iEvent.getByLabel( tTrkNHits, hTrkNHits );
    if ( hTrkNHits.isValid() ) TrkNHits = *hTrkNHits;
    result &= iEvent.getByLabel( tTrkPhi, hTrkPhi );
    if ( hTrkPhi.isValid() ) TrkPhi = *hTrkPhi;
    result &= iEvent.getByLabel( tTrkPt, hTrkPt );
    if ( hTrkPt.isValid() ) TrkPt = *hTrkPt;
    result &= iEvent.getByLabel( tTrkPtSum, hTrkPtSum );
    if ( hTrkPtSum.isValid() ) TrkPtSum = *hTrkPtSum;
    result &= iEvent.getByLabel( tTrkPtSumPhi, hTrkPtSumPhi );
    if ( hTrkPtSumPhi.isValid() ) TrkPtSumPhi = *hTrkPtSumPhi;
    result &= iEvent.getByLabel( tTrkPtSumx, hTrkPtSumx );
    if ( hTrkPtSumx.isValid() ) TrkPtSumx = *hTrkPtSumx;
    result &= iEvent.getByLabel( tTrkPtSumy, hTrkPtSumy );
    if ( hTrkPtSumy.isValid() ) TrkPtSumy = *hTrkPtSumy;
    result &= iEvent.getByLabel( tTrkVtxDxy, hTrkVtxDxy );
    if ( hTrkVtxDxy.isValid() ) TrkVtxDxy = *hTrkVtxDxy;
    result &= iEvent.getByLabel( tTrkVtxDz, hTrkVtxDz );
    if ( hTrkVtxDz.isValid() ) TrkVtxDz = *hTrkVtxDz;
    result &= iEvent.getByLabel( tVrtxChi2, hVrtxChi2 );
    if ( hVrtxChi2.isValid() ) VrtxChi2 = *hVrtxChi2;
    result &= iEvent.getByLabel( tVrtxIsFake, hVrtxIsFake );
    if ( hVrtxIsFake.isValid() ) VrtxIsFake = *hVrtxIsFake;
    result &= iEvent.getByLabel( tVrtxNdof, hVrtxNdof );
    if ( hVrtxNdof.isValid() ) VrtxNdof = *hVrtxNdof;
    result &= iEvent.getByLabel( tVrtxNtrks, hVrtxNtrks );
    if ( hVrtxNtrks.isValid() ) VrtxNtrks = *hVrtxNtrks;
    result &= iEvent.getByLabel( tVrtxSumPt, hVrtxSumPt );
    if ( hVrtxSumPt.isValid() ) VrtxSumPt = *hVrtxSumPt;
    result &= iEvent.getByLabel( tVrtxX, hVrtxX );
    if ( hVrtxX.isValid() ) VrtxX = *hVrtxX;
    result &= iEvent.getByLabel( tVrtxXE, hVrtxXE );
    if ( hVrtxXE.isValid() ) VrtxXE = *hVrtxXE;
    result &= iEvent.getByLabel( tVrtxY, hVrtxY );
    if ( hVrtxY.isValid() ) VrtxY = *hVrtxY;
    result &= iEvent.getByLabel( tVrtxYE, hVrtxYE );
    if ( hVrtxYE.isValid() ) VrtxYE = *hVrtxYE;
    result &= iEvent.getByLabel( tVrtxZ, hVrtxZ );
    if ( hVrtxZ.isValid() ) VrtxZ = *hVrtxZ;
    result &= iEvent.getByLabel( tVrtxZE, hVrtxZE );
    if ( hVrtxZE.isValid() ) VrtxZE = *hVrtxZE;
    result &= iEvent.getByLabel( tVtxdiphoh2gglobe, hVtxdiphoh2gglobe );
    if ( hVtxdiphoh2gglobe.isValid() ) Vtxdiphoh2gglobe = *hVtxdiphoh2gglobe;
    result &= iEvent.getByLabel( tVtxdiphomva, hVtxdiphomva );
    if ( hVtxdiphomva.isValid() ) Vtxdiphomva = *hVtxdiphomva;
    result &= iEvent.getByLabel( tVtxdiphoproductrank, hVtxdiphoproductrank );
    if ( hVtxdiphoproductrank.isValid() ) Vtxdiphoproductrank = *hVtxdiphoproductrank;
    result &= iEvent.getByLabel( tWeight, hWeight );
    if ( hWeight.isValid() ) Weight = *hWeight;
    result &= iEvent.getByLabel( tXtalEtaWidth, hXtalEtaWidth );
    if ( hXtalEtaWidth.isValid() ) XtalEtaWidth = *hXtalEtaWidth;
    result &= iEvent.getByLabel( tXtalFront1X, hXtalFront1X );
    if ( hXtalFront1X.isValid() ) XtalFront1X = *hXtalFront1X;
    result &= iEvent.getByLabel( tXtalFront1Y, hXtalFront1Y );
    if ( hXtalFront1Y.isValid() ) XtalFront1Y = *hXtalFront1Y;
    result &= iEvent.getByLabel( tXtalFront1Z, hXtalFront1Z );
    if ( hXtalFront1Z.isValid() ) XtalFront1Z = *hXtalFront1Z;
    result &= iEvent.getByLabel( tXtalFront2X, hXtalFront2X );
    if ( hXtalFront2X.isValid() ) XtalFront2X = *hXtalFront2X;
    result &= iEvent.getByLabel( tXtalFront2Y, hXtalFront2Y );
    if ( hXtalFront2Y.isValid() ) XtalFront2Y = *hXtalFront2Y;
    result &= iEvent.getByLabel( tXtalFront2Z, hXtalFront2Z );
    if ( hXtalFront2Z.isValid() ) XtalFront2Z = *hXtalFront2Z;
    result &= iEvent.getByLabel( tXtalFront3X, hXtalFront3X );
    if ( hXtalFront3X.isValid() ) XtalFront3X = *hXtalFront3X;
    result &= iEvent.getByLabel( tXtalFront3Y, hXtalFront3Y );
    if ( hXtalFront3Y.isValid() ) XtalFront3Y = *hXtalFront3Y;
    result &= iEvent.getByLabel( tXtalFront3Z, hXtalFront3Z );
    if ( hXtalFront3Z.isValid() ) XtalFront3Z = *hXtalFront3Z;
    result &= iEvent.getByLabel( tXtalFront4X, hXtalFront4X );
    if ( hXtalFront4X.isValid() ) XtalFront4X = *hXtalFront4X;
    result &= iEvent.getByLabel( tXtalFront4Y, hXtalFront4Y );
    if ( hXtalFront4Y.isValid() ) XtalFront4Y = *hXtalFront4Y;
    result &= iEvent.getByLabel( tXtalFront4Z, hXtalFront4Z );
    if ( hXtalFront4Z.isValid() ) XtalFront4Z = *hXtalFront4Z;
    result &= iEvent.getByLabel( tXtalPhiWidth, hXtalPhiWidth );
    if ( hXtalPhiWidth.isValid() ) XtalPhiWidth = *hXtalPhiWidth;
    result &= iEvent.getByLabel( tXtalX, hXtalX );
    if ( hXtalX.isValid() ) XtalX = *hXtalX;
    result &= iEvent.getByLabel( tXtalY, hXtalY );
    if ( hXtalY.isValid() ) XtalY = *hXtalY;
    result &= iEvent.getByLabel( tXtalZ, hXtalZ );
    if ( hXtalZ.isValid() ) XtalZ = *hXtalZ;
    result &= iEvent.getByLabel( tgenInfoEta, hgenInfoEta );
    if ( hgenInfoEta.isValid() ) genInfoEta = *hgenInfoEta;
    result &= iEvent.getByLabel( tgenInfoId, hgenInfoId );
    if ( hgenInfoId.isValid() ) genInfoId = *hgenInfoId;
    result &= iEvent.getByLabel( tgenInfoM, hgenInfoM );
    if ( hgenInfoM.isValid() ) genInfoM = *hgenInfoM;
    result &= iEvent.getByLabel( tgenInfoMo1, hgenInfoMo1 );
    if ( hgenInfoMo1.isValid() ) genInfoMo1 = *hgenInfoMo1;
    result &= iEvent.getByLabel( tgenInfoMo2, hgenInfoMo2 );
    if ( hgenInfoMo2.isValid() ) genInfoMo2 = *hgenInfoMo2;
    result &= iEvent.getByLabel( tgenInfoNMo, hgenInfoNMo );
    if ( hgenInfoNMo.isValid() ) genInfoNMo = *hgenInfoNMo;
    result &= iEvent.getByLabel( tgenInfoPhi, hgenInfoPhi );
    if ( hgenInfoPhi.isValid() ) genInfoPhi = *hgenInfoPhi;
    result &= iEvent.getByLabel( tgenInfoPromptFlag, hgenInfoPromptFlag );
    if ( hgenInfoPromptFlag.isValid() ) genInfoPromptFlag = *hgenInfoPromptFlag;
    result &= iEvent.getByLabel( tgenInfoPt, hgenInfoPt );
    if ( hgenInfoPt.isValid() ) genInfoPt = *hgenInfoPt;
    result &= iEvent.getByLabel( tgenInfoPx, hgenInfoPx );
    if ( hgenInfoPx.isValid() ) genInfoPx = *hgenInfoPx;
    result &= iEvent.getByLabel( tgenInfoPy, hgenInfoPy );
    if ( hgenInfoPy.isValid() ) genInfoPy = *hgenInfoPy;
    result &= iEvent.getByLabel( tgenInfoPz, hgenInfoPz );
    if ( hgenInfoPz.isValid() ) genInfoPz = *hgenInfoPz;
    result &= iEvent.getByLabel( tgenInfoStatus, hgenInfoStatus );
    if ( hgenInfoStatus.isValid() ) genInfoStatus = *hgenInfoStatus;
    result &= iEvent.getByLabel( tgvNTkHi, hgvNTkHi );
    if ( hgvNTkHi.isValid() ) gvNTkHi = *hgvNTkHi;
    result &= iEvent.getByLabel( tgvNTkLo, hgvNTkLo );
    if ( hgvNTkLo.isValid() ) gvNTkLo = *hgvNTkLo;
    result &= iEvent.getByLabel( tgvSumPtHi, hgvSumPtHi );
    if ( hgvSumPtHi.isValid() ) gvSumPtHi = *hgvSumPtHi;
    result &= iEvent.getByLabel( tgvSumPtLo, hgvSumPtLo );
    if ( hgvSumPtLo.isValid() ) gvSumPtLo = *hgvSumPtLo;
    result &= iEvent.getByLabel( tnGenParticles, hnGenParticles );
    if ( hnGenParticles.isValid() ) nGenParticles = *hnGenParticles;
    result &= iEvent.getByLabel( tpdfW, hpdfW );
    if ( hpdfW.isValid() ) pdfW = *hpdfW;
    result &= iEvent.getByLabel( tpdfWsum, hpdfWsum );
    if ( hpdfWsum.isValid() ) pdfWsum = *hpdfWsum;
    result &= iEvent.getByLabel( tprocess, hprocess );
    if ( hprocess.isValid() ) process = *hprocess;
    result &= iEvent.getByLabel( tsignMu, hsignMu );
    if ( hsignMu.isValid() ) signMu = *hsignMu;
    result &= iEvent.getByLabel( txSMS, hxSMS );
    if ( hxSMS.isValid() ) xSMS = *hxSMS;
    result &= iEvent.getByLabel( txbarSMS, hxbarSMS );
    if ( hxbarSMS.isValid() ) xbarSMS = *hxbarSMS;



    int nElectrons = this->NEles;
    bool ele_cond = false;
    if( this->min_nelectrons != -100){
      if( min_nelectrons > 0 )
	ele_cond = (nElectrons > min_nelectrons);
      else
	ele_cond = ( nElectrons == abs(min_nelectrons) );
    }

    int nMuons = this->NMus;
    bool mu_cond = false;
    if( this->min_nmuons != -100){
      if( min_nmuons > 0 )
	mu_cond = ( nMuons > min_nmuons );
      else
	mu_cond = ( nMuons == abs(min_nmuons) );
    }

    int nTaus = this->TauNObjs ;
    bool tau_cond = false;
    if( this->min_ntaus != -100){
      if( min_ntaus > 0 )
	tau_cond = ( nTaus > min_ntaus) ;
      else
	tau_cond = ( nTaus == abs( min_ntaus ));
    }

    int nLeptons = nElectrons + nMuons + nTaus ; 
    bool lept_cond = false;
    if( this->min_nleptons != -100){
      if( min_nleptons > 0)
	lept_cond = ( nLeptons > min_nleptons );
      else
	lept_cond = ( nLeptons == abs( min_nleptons ) );
    }

    int nJets = this->NJets ; //
    bool jet_cond = true;
    if( this->min_njets != -100 ){
      if( min_njets > 0 )
	jet_cond = ( nJets > min_njets );
      else
	jet_cond = ( nJets == abs( min_njets ) );
    }
    

    double HT = 0.0 ; //???
    for( int i = 0 ; i < nJets ; i++ )  {
      if(PFCHSJPt[i] > 50 && fabs(PFCHSJEta[i])<3.0){
	HT += PFCHSJPt[i] ;
      }
    }
    bool ht_cond = true;
    if( this->min_HT != 0.0 ){
      ht_cond = ( HT > min_HT ) ;
    } 

    bool hlt_result = (HLTPaths.size() == 0);
    for( vector<int>::const_iterator hlt_index = HLTIndices.begin() ; hlt_index!=HLTIndices.end();hlt_index++){
      hlt_result |= ( HLTResults[ *hlt_index ] );
    }
    

    return ( ( ele_cond || mu_cond || tau_cond || lept_cond ) || ( jet_cond && ht_cond ) ) && hlt_result  ;

}

// ------------ method called once each job just before starting event loop  ------------
void 
NTupleSelector::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void 
NTupleSelector::endJob() {
}

// ------------ method called when starting to processes a run  ------------
bool 
NTupleSelector::beginRun(edm::Run& run, edm::EventSetup const&)
{ 


  bool result = true;

  // Get all run handles and assign to members
  result &= run.getByLabel( tExtXSecLO, hExtXSecLO );
  if ( hExtXSecLO.isValid() ) ExtXSecLO = *hExtXSecLO;
  result &= run.getByLabel( tExtXSecNLO, hExtXSecNLO );
  if ( hExtXSecNLO.isValid() ) ExtXSecNLO = *hExtXSecNLO;
  result &= run.getByLabel( tHLTLabels, hHLTLabels );
  if ( hHLTLabels.isValid() ) HLTLabels = *hHLTLabels;
  result &= run.getByLabel( tHLTNames, hHLTNames );
  if ( hHLTNames.isValid() ) HLTNames = *hHLTNames;
  result &= run.getByLabel( tIntXSec, hIntXSec );
  if ( hIntXSec.isValid() ) IntXSec = *hIntXSec;
  result &= run.getByLabel( tL1PhysMenu, hL1PhysMenu );
  if ( hL1PhysMenu.isValid() ) L1PhysMenu = *hL1PhysMenu;
  result &= run.getByLabel( tMaxElEta, hMaxElEta );
  if ( hMaxElEta.isValid() ) MaxElEta = *hMaxElEta;
  result &= run.getByLabel( tMaxGenJetEta, hMaxGenJetEta );
  if ( hMaxGenJetEta.isValid() ) MaxGenJetEta = *hMaxGenJetEta;
  result &= run.getByLabel( tMaxGenLeptEta, hMaxGenLeptEta );
  if ( hMaxGenLeptEta.isValid() ) MaxGenLeptEta = *hMaxGenLeptEta;
  result &= run.getByLabel( tMaxGenPhotEta, hMaxGenPhotEta );
  if ( hMaxGenPhotEta.isValid() ) MaxGenPhotEta = *hMaxGenPhotEta;
  result &= run.getByLabel( tMaxJEta, hMaxJEta );
  if ( hMaxJEta.isValid() ) MaxJEta = *hMaxJEta;
  result &= run.getByLabel( tMaxMuEta, hMaxMuEta );
  if ( hMaxMuEta.isValid() ) MaxMuEta = *hMaxMuEta;
  result &= run.getByLabel( tMaxNConv, hMaxNConv );
  if ( hMaxNConv.isValid() ) MaxNConv = *hMaxNConv;
  result &= run.getByLabel( tMaxNEBhits, hMaxNEBhits );
  if ( hMaxNEBhits.isValid() ) MaxNEBhits = *hMaxNEBhits;
  result &= run.getByLabel( tMaxNEles, hMaxNEles );
  if ( hMaxNEles.isValid() ) MaxNEles = *hMaxNEles;
  result &= run.getByLabel( tMaxNGenJet, hMaxNGenJet );
  if ( hMaxNGenJet.isValid() ) MaxNGenJet = *hMaxNGenJet;
  result &= run.getByLabel( tMaxNGenLep, hMaxNGenLep );
  if ( hMaxNGenLep.isValid() ) MaxNGenLep = *hMaxNGenLep;
  result &= run.getByLabel( tMaxNGenPho, hMaxNGenPho );
  if ( hMaxNGenPho.isValid() ) MaxNGenPho = *hMaxNGenPho;
  result &= run.getByLabel( tMaxNJets, hMaxNJets );
  if ( hMaxNJets.isValid() ) MaxNJets = *hMaxNJets;
  result &= run.getByLabel( tMaxNMus, hMaxNMus );
  if ( hMaxNMus.isValid() ) MaxNMus = *hMaxNMus;
  result &= run.getByLabel( tMaxNPfCand, hMaxNPfCand );
  if ( hMaxNPfCand.isValid() ) MaxNPfCand = *hMaxNPfCand;
  result &= run.getByLabel( tMaxNPhotons, hMaxNPhotons );
  if ( hMaxNPhotons.isValid() ) MaxNPhotons = *hMaxNPhotons;
  result &= run.getByLabel( tMaxNPileup, hMaxNPileup );
  if ( hMaxNPileup.isValid() ) MaxNPileup = *hMaxNPileup;
  result &= run.getByLabel( tMaxNSC, hMaxNSC );
  if ( hMaxNSC.isValid() ) MaxNSC = *hMaxNSC;
  result &= run.getByLabel( tMaxNTrks, hMaxNTrks );
  if ( hMaxNTrks.isValid() ) MaxNTrks = *hMaxNTrks;
  result &= run.getByLabel( tMaxNVrtx, hMaxNVrtx );
  if ( hMaxNVrtx.isValid() ) MaxNVrtx = *hMaxNVrtx;
  result &= run.getByLabel( tMaxNXtals, hMaxNXtals );
  if ( hMaxNXtals.isValid() ) MaxNXtals = *hMaxNXtals;
  result &= run.getByLabel( tMaxPhotonEta, hMaxPhotonEta );
  if ( hMaxPhotonEta.isValid() ) MaxPhotonEta = *hMaxPhotonEta;
  result &= run.getByLabel( tMaxTrkEta, hMaxTrkEta );
  if ( hMaxTrkEta.isValid() ) MaxTrkEta = *hMaxTrkEta;
  result &= run.getByLabel( tMaxTrkNChi2, hMaxTrkNChi2 );
  if ( hMaxTrkNChi2.isValid() ) MaxTrkNChi2 = *hMaxTrkNChi2;
  result &= run.getByLabel( tMinEBRechitE, hMinEBRechitE );
  if ( hMinEBRechitE.isValid() ) MinEBRechitE = *hMinEBRechitE;
  result &= run.getByLabel( tMinElPt, hMinElPt );
  if ( hMinElPt.isValid() ) MinElPt = *hMinElPt;
  result &= run.getByLabel( tMinGenJetPt, hMinGenJetPt );
  if ( hMinGenJetPt.isValid() ) MinGenJetPt = *hMinGenJetPt;
  result &= run.getByLabel( tMinGenLeptPt, hMinGenLeptPt );
  if ( hMinGenLeptPt.isValid() ) MinGenLeptPt = *hMinGenLeptPt;
  result &= run.getByLabel( tMinGenPhotPt, hMinGenPhotPt );
  if ( hMinGenPhotPt.isValid() ) MinGenPhotPt = *hMinGenPhotPt;
  result &= run.getByLabel( tMinJEMfrac, hMinJEMfrac );
  if ( hMinJEMfrac.isValid() ) MinJEMfrac = *hMinJEMfrac;
  result &= run.getByLabel( tMinJPt, hMinJPt );
  if ( hMinJPt.isValid() ) MinJPt = *hMinJPt;
  result &= run.getByLabel( tMinMuPt, hMinMuPt );
  if ( hMinMuPt.isValid() ) MinMuPt = *hMinMuPt;
  result &= run.getByLabel( tMinPhotonPt, hMinPhotonPt );
  if ( hMinPhotonPt.isValid() ) MinPhotonPt = *hMinPhotonPt;
  result &= run.getByLabel( tMinRawJPt, hMinRawJPt );
  if ( hMinRawJPt.isValid() ) MinRawJPt = *hMinRawJPt;
  result &= run.getByLabel( tMinSCraw, hMinSCraw );
  if ( hMinSCraw.isValid() ) MinSCraw = *hMinSCraw;
  result &= run.getByLabel( tMinTrkNHits, hMinTrkNHits );
  if ( hMinTrkNHits.isValid() ) MinTrkNHits = *hMinTrkNHits;
  result &= run.getByLabel( tMinTrkPt, hMinTrkPt );
  if ( hMinTrkPt.isValid() ) MinTrkPt = *hMinTrkPt;
  result &= run.getByLabel( tPileUpData, hPileUpData );
  if ( hPileUpData.isValid() ) PileUpData = *hPileUpData;
  result &= run.getByLabel( tPileUpMC, hPileUpMC );
  if ( hPileUpMC.isValid() ) PileUpMC = *hPileUpMC;

  HLTIndices.clear();


  string_vector_itr first_iterator = HLTNames.begin();
  cout << run.run() << "-------------------------------------------" << HLTNames.size() << endl;
  for( vector<string>::const_iterator hlt = HLTNames.begin() ; hlt != HLTNames.end() ; hlt++ )
    {
      int index = distance( first_iterator , hlt );
      cout << "\t" << index << ":" << *hlt << endl;
    }
  

  for( vector<string>::const_iterator hlt_regexp = HLTPaths.begin() ; hlt_regexp != HLTPaths.end() ; hlt_regexp++ )
    {
      cout << "\t" <<  *hlt_regexp << " : " << endl;
      vector< string_vector_itr > matches =  edm::regexMatch( HLTNames , *hlt_regexp ) ;
      for( vector< string_vector_itr >::const_iterator hlt_itr = matches.begin() ; hlt_itr != matches.end() ;hlt_itr++){
	int index = distance( first_iterator , *hlt_itr );
	HLTIndices.push_back( index );
	cout << "\t\t" << *(*hlt_itr) << "  : " << index << endl;
      }
    }
  return result;
}

// ------------ method called when ending the processing of a run  ------------
bool 
NTupleSelector::endRun(edm::Run&, edm::EventSetup const&)
{
  return true;
}

// ------------ method called when starting to processes a luminosity block  ------------
bool 
NTupleSelector::beginLuminosityBlock(edm::LuminosityBlock&, edm::EventSetup const&)
{
  return true;
}

// ------------ method called when ending the processing of a luminosity block  ------------
bool 
NTupleSelector::endLuminosityBlock(edm::LuminosityBlock&, edm::EventSetup const&)
{
  return true;
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
NTupleSelector::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  //The following says we do not know what parameters are allowed so do no validation
  // Please change this to state exactly what you do use, even if it is no parameters
  edm::ParameterSetDescription desc;
  desc.setUnknown();
  descriptions.addDefault(desc);
}
//define this as a plug-in
DEFINE_FWK_MODULE(NTupleSelector);
